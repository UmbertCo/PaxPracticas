...
public partial class Consultas_webDescargaComprobantes : System.Web.UI.Page
{
    ...
    private DataTable fnObtenerComprobantesProveedorDescargaThread(DateTime pdFechaIni, DateTime pdFechaFin, string psRfcEmisor, string psRfcReceptor, string sVersion, int nIdUsuario, int nTipo, string sLista, string sListaComprobantes)
    {
        ...
        try
        {
            using (SqlConnection con = new SqlConnection(Utilerias.Encriptacion.Base64.DesencriptarBase64(ConfigurationManager.ConnectionStrings[conSucursal].ConnectionString)))
            {
                using (SqlCommand cmd = con.CreateCommand())
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.CommandText = "usp_cfd_Comprobantes_Proveedor_sel";

                    cmd.Parameters.Add(new SqlParameter("dDesde", pdFechaIni));
                    cmd.Parameters.Add(new SqlParameter("dHasta", pdFechaFin));

                    if (!string.IsNullOrEmpty(psRfcEmisor))
                        cmd.Parameters.Add(new SqlParameter("sRFCEmisor", psRfcEmisor));
                    if (!string.IsNullOrEmpty(psRfcReceptor))
                        cmd.Parameters.Add(new SqlParameter("sRFCReceptor", psRfcReceptor));
                    if (!string.IsNullOrEmpty(sVersion))
                    ...
                    cmd.Parameters.Add(new SqlParameter("sListaComp", sListaComprobantes)); ;
    ...
    private DataTable fnObtenerComprobantesDescargaThread(DateTime pdFechaIni, DateTime pdFechaFin, string psRfcEmisor, string psRfcReceptor, string sVersion, int nIdUsuario, int nTipo, string sLista, string sListaComprobantes)
    {
        ...
        try
        {
            using (SqlConnection con = new SqlConnection(Utilerias.Encriptacion.Base64.DesencriptarBase64(ConfigurationManager.ConnectionStrings[conSucursal].ConnectionString)))
            {
                ...
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.CommandText = "usp_cfd_Comprobantes_sel";
