...
public partial class Operacion_Comprobantes_webValidacionComprobantes : System.Web.UI.Page
{
    /// <summary>
                ...
                try
                {
                    ...
                    if (fuPdf.HasFile)
                    {
                       ...
                       /* foreach (DataRow row in tabla.Rows)
                        {
                            if (row["nombre_completo"].ToString() == fuComprobantes.FileName.ToString())
                            {
                    ...
                    if (!string.IsNullOrEmpty(row["nombre_xml"].ToString()))
                    {
                            ...
                            == Path.GetFileNameWithoutExtension(row["nombre_pdf"].ToString()))
                        {
                            ...
                            if (Path.GetExtension(row["nombre_xml"].ToString()).ToLower() == ".xml")
                            {
                                ...
                                //MemoryStream ms = new MemoryStream((byte[])row["xml"]);
                                //doc.Load(ms);
            ...
            foreach (ZipEntry zipEntry in archivoZip)
            {
                ...
                if (Path.GetExtension(zipEntry.Name).ToLower() == ".xml")
                {
                    ...
                    if (!existe)
                    {
                        ...
                        xXml.Load(strXml);
                        xXml.LoadXml(xXml.InnerXml.ToString().Replace("<?xml version=\"1.0\" encoding=\"UTF-8\"?>", ""));*/
    ...
    protected void btnValidar_Click(object sender, EventArgs e)
    {
        ...
        if ((usu.email == null) || (usu.email == ""))
        {
            lMensaje.Text = Resources.resCorpusCFDIEs.varRegistrarCorreo;
        }
        else
        {
            ValidarComprobantes();
