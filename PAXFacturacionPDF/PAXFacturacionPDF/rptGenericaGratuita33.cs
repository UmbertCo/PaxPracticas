...
namespace PAXFacturacionPDF
{
    public partial class rptGenericaGratuita33 : DevExpress.XtraReports.UI.XtraReport
    {
        ...
        private void xrBarCodeQRGratuita_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            ...
            string sSello = ((XRTableCell)FindControl("CellSelloDigitalEmisor", true)).Text;
            sSello = sSello.Substring(sSello.Length - 8, 8);

            sCadenaCodigo = "https://verificacfdi.facturaelectronica.sat.gob.mx/default.aspx?re=" + ((XRTableCell)FindControl("CellRFCEmisor", true)).Text.Replace("RFC:", "").Trim()
        ...
        {
            XRTableCell CellMetodoPago = (XRTableCell)sender;
            if (CellMetodoPago.Text != "")
            {
                switch (CellMetodoPago.Text)
                {
                    case "PUE":
                        (sender as XRTableCell).Text = CellMetodoPago.Text + "(Pago en una sola exhibición)";
                        break;
                    case "PIP":
                        (sender as XRTableCell).Text = CellMetodoPago.Text + "(Pago inicial y parcialidades)";
                        break;
                    case "PPD":
                        (sender as XRTableCell).Text = CellMetodoPago.Text + "(Pago en parcialidades o diferido)";
                        break;
                }
            }
        }
        ...
        private void CellRegimenEmisor_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            ...
            if (CellRegimenEmisor.Text != "")
            {
                DataTable dtTiposRegimenFiscal = new DataTable();
                clsOperacionTipoRegimen cRegimenFiscal = new clsOperacionTipoRegimen();

                try
                {
                    dtTiposRegimenFiscal = cRegimenFiscal.fnObtenerTiposRegimenFiscal();
                    foreach (DataRow dr in dtTiposRegimenFiscal.Rows)
                    {
                        if (dr["Clave"].ToString() == CellRegimenEmisor.Text)
                        {
                            (sender as XRTableCell).Text = "Régimen " + dr["Clave"].ToString() + "(" + dr["Descripcion"].ToString() + ")";
                            break;
                        }
                    }
                }
        ...
        private void CellFormaPago_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            ...
            if (CellFormaPago.Text != "")
            {
                try
                {
                    clsOperacionFormaPago cFormaPago = new clsOperacionFormaPago();

                    dtFormaPago = cFormaPago.fnFormaPago();
                    foreach (DataRow dr in dtFormaPago.Rows)
                    {
                        if (dr["c_FormaPago"].ToString() == CellFormaPago.Text)
                        {
                            (sender as XRTableCell).Text = dr["c_FormaPago"].ToString() + "(" + dr["Descripcion"].ToString() + ")";
                            break;
                        }
                    }
                }
        ...
        private void CellUsoCFDI_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            ...
            DataTable dtUsoCFDI = new DataTable();
            clsOperacionUsoCFDI cUsoCFDI = new clsOperacionUsoCFDI();
            if (CellFormaPago.Text != "")
            {
                dtUsoCFDI = cUsoCFDI.fnCargarUsoCFDI("NA");
                foreach (DataRow dr in dtUsoCFDI.Rows)
                {
                    if (dr["c_UsoCFDI"].ToString() == CellUsoCFDI.Text)
                    {
                        (sender as XRTableCell).Text = "Uso CFDI: " + dr["c_UsoCFDI"].ToString() + "(" + dr["Descripcion"].ToString() + ")";
                        break;
                    }
                }
            }
