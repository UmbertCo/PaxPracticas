...
public class clsOperacionConsultaPdf
{
    ...
    private void fnTotales(XmlNamespaceManager nsmComprobante, XPathNavigator navPie, StaticContainer Pie, int nNumPag, int nTotPag, string sColor)
	{
        ...
        if (nNumPag == nTotPag)//Si es ultima pagina
        {
            if (descuento != string.Empty)//si tiene Descuento
            {
        ...
        Pie.AddAligned(Pie.rWidth / 7, RepObj.rAlignCenter, Pie.rHeight - verPadding, RepObj.rAlignTop, new RepString(fPropNormal, ConectorPAXGenericoPDF.Properties.Settings.Default["urlHostComercial"].ToString()));
	...
	private string fnTextoImporte(string psValor, string psMoneda)
	{
        ...
        languaje = new CultureInfo("es-Mx"); //System.Configuration.ConfigurationManager.AppSettings["DefaultLanguage"].ToString());
    ...
    private RepImage fnMarcaAguaCorpus()
    {
        ...
        try
        {
            ...
            string fichero = ruta + "marca_agua_corpus.png";
...
public class Detalle
{
	...
	{
        try { cantidad = navDetalle.SelectSingleNode("@cantidad", nsmComprobante).Value; }
        catch { cantidad = "0"; }
        ...
        try { descripcion = navDetalle.SelectSingleNode("@descripcion", nsmComprobante).Value; }
        catch { descripcion = string.Empty; }
        try { valorUnitario = navDetalle.SelectSingleNode("@valorUnitario", nsmComprobante).Value; }
        catch { valorUnitario = "0"; }
        try { importe = navDetalle.SelectSingleNode("@importe", nsmComprobante).Value; }
        catch { importe = "0"; }
	}
}
...
public class Transformacion
{
    ...
    public static string fnConstruirCadenaTimbrado(IXPathNavigable xml, string psNombreArchivoXSLT)
    {
        ...
        try
        {
            ...
            trans.Load(XmlReader.Create(new StringReader(ConectorPAXGenericoPDF.Properties.Settings.Default[psNombreArchivoXSLT].ToString())));
