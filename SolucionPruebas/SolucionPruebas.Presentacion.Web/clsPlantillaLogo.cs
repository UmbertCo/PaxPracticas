...
public class clsPlantillaLogo
{
    ...
    private StaticContainer fnContarPaginas(List<DetalleLogo> detalles)
    {
        ...
        int maxRenglones = (int)((areaDetalle.rHeight / (fPropNormal.rSize + 2))) - 2;
        ...
        for (int i = 0; renglonActual <= maxRenglones; i++)
        {
            ...
            {
                if (renglones > maxRenglones - renglonActual + 1)
                { }
                //break;
            }
            else
            {
                if (renglones > maxRenglones - renglonActual + 1) //Si es hoja nueva e verifica si el concepto cabe en su totalidad 
                {
                    rengTotal = Convert.ToInt32((renglones - maxRenglones - renglonActual + 1));
