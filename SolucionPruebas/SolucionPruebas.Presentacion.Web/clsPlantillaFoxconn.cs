...
public class clsPlantillaFoxconn
{
    ...
    public clsPlantillaFoxconn(XmlDocument pxComprobante)
    {
    ...
    public void fnGenerarPDF(string scolor)
    {
        ...
        List<DetalleFoxconn> detallesNum = fnObtenerDetalles();
        ...
        List<DetalleFoxconn> detalles = fnObtenerDetalles();
        ...
        while (bSeguir)
        {
            ...
            StaticContainer scPanel = fnCrearPanelFactura_Invoice(ref Encabezado, ColorT);
            ...
            //Agregar el Panel Factura Invoice
            pagina.Add(Encabezado.rWidth - scPanel.rWidth, (Encabezado.rHeight - fPropBlanca.rSize * 2) - 83, scPanel);

            //Marco codigo de barras Bidimensional

            fnCrearPanelRedondeado(Pie, 0, tamCodigo, tamCodigo, tamCodigo, grosorPen, radioCurva, false, ColorT);

            if (pagina.iPageNo == nTotPag)//Si es ultima pagina
                pagina.Add(margenPagina + 5, RT.rPointFromMM(altoPagina) - margenPagina - Pie.rHeight + (tamCodigo * 2) - 5, GenerarCodigoBidimensional());
            ...
            ms = fnImagenCliente("Foxconn_logo.jpg");

            //Creamos el área de detalle
            if (ms.Length > 0)
            {
                RepImage image = new RepImage(ms, 150, 41);
                pagina.Add(margenPagina + 12, margenPagina + 42, image);
            }
    ...
    /// <returns></returns>
    private StaticContainer fnContarPaginas(List<DetalleFoxconn> detalles)
    {
        ...
        DetalleFoxconn[] copiaDetalles = detalles.ToArray();
        DetalleFoxconn d;
        double posRenglon;
            ...
            if (d.aduana != null || !String.IsNullOrEmpty(d.RfcT))
            {
                ...
                if (!String.IsNullOrEmpty(d.RfcT))
                {
                    ...
                    if ((d.RetencionesT != null && d.RetencionesT.Count > 0) || (d.TrasladosT != null && d.TrasladosT.Count > 0)) //Impuestos tercero
                    {
                        ...
                        {
                            foreach (ComprobanteImpuestosRetencionTFoxconn retencion in d.RetencionesT)
                            {
                                switch (retencion.impuesto)
                                {
                                    case ComprobanteImpuestosRetencionImpuestoTFoxconn.IVA: sDetalleTerceros.Append("IVA Ret: "); break;
                                    case ComprobanteImpuestosRetencionImpuestoTFoxconn.ISR: sDetalleTerceros.Append("ISR: "); break;
                                }
                                sDetalleTerceros.Append(TransformacionFoxconn.fnFormatoCurrency(retencion.importe.ToString()) + " ");
                            }
                        }

                        if (d.TrasladosT != null && d.TrasladosT.Count > 0)
                        {
                            foreach (ComprobanteImpuestosTrasladoTFoxconn traslado in d.TrasladosT)
                            {
                                switch (traslado.impuesto)
                                {
                                    case ComprobanteImpuestosTrasladoImpuestoTFoxconn.IVA: sDetalleTerceros.Append("IVA Tras "); break;
                                    case ComprobanteImpuestosTrasladoImpuestoTFoxconn.IEPS: sDetalleTerceros.Append("IEPS "); break;
                                }
                                sDetalleTerceros.Append(TransformacionFoxconn.fnFormatoPorcentaje(traslado.tasa.ToString()) + ": ");
                                sDetalleTerceros.Append(TransformacionFoxconn.fnFormatoCurrency(traslado.importe.ToString()) + " ");
                            }
                        }
                    }
    ...
    /// <returns></returns>
    private StaticContainer fnCrearDetalle(List<DetalleFoxconn> detalles, System.Drawing.Color sColor)
    {
        StaticContainer areaDetalle = new StaticContainer(RT.rPointFromMM(anchoPagina) - margenPagina * 2, RT.rPointFromMM(altoPagina - altoEncabezado - altoPie) - margenPagina * 2);
        ...
        for (int i = 0; renglonActual <= maxRenglones; i++)
        {
            ...
            else
            {
                ...
                Col3 = fnAgregarMultilineaDetalle(areaDetalle, d.descripcion + " " + fnObtenerConceptoAddenda(i + 1), fPropNormal, posColumna3, posRenglon, 42, true);
            }
    ...
    private List<DetalleFoxconn> fnObtenerDetalles()
    {
        XmlNamespaceManager nsmComprobante = new XmlNamespaceManager(gxComprobante.NameTable);
        nsmComprobante.AddNamespace("cfdi", "http://www.sat.gob.mx/cfd/3");

        List<DetalleFoxconn> detalles = new List<DetalleFoxconn>();
        ...
        {
            detalles.Add(new DetalleFoxconn(navDetalles.Current, nsmComprobante));
            ...
            if (nodenavigator.HasChildren)//Si contiene nodo hijo
            {
                ...
                //    detalles.Add(new DetalleMyers(navDetallesAduana.Current, nsmComprobante));
                ...
                if (navDetallesAduana != null)
                    detalles.Add(new DetalleFoxconn(navDetallesAduana, nsmComprobante));
                ...
                if (navComplTerceros != null)
                {
                    ...
                    if (navTerceros != null)
                        detalles.Add(new DetalleFoxconn(navTerceros, nsmComprobanteComplTerceros));
                }
            }
        }
    ...
    private void fnTotales(XmlNamespaceManager nsmComprobante, XPathNavigator navPie, StaticContainer Pie, int nNumPag, int nTotPag, string sColor)
    {
        ...
        string subtotal, total, moneda, sello, timbre, formaDePago, metodoPago, Regimenfiscal, version, sNumCtaPago, descuento, sTotalAddenda, sSalesOrder, sDelivery;
        subtotal = total = moneda = sello = timbre = formaDePago = metodoPago = Regimenfiscal = version = sNumCtaPago = descuento = sTotalAddenda = sSalesOrder = sDelivery = string.Empty;
        ...
        XPathNavigator navFoxconn = xdFoxconn.CreateNavigator();

        try { sRetIva = navFoxconn.SelectSingleNode("/Foxconn/Impuesto/@importe").Value; }
        catch { }


        List<ImpuestoFoxconn> impuestos = new List<ImpuestoFoxconn>();
        List<ImpuestoCompFoxconn> impuestosComp = new List<ImpuestoCompFoxconn>();
        ...
        if (nNumPag == nTotPag)//Si es ultima pagina
        {
            ...
            {
                impuestos.Add(new ImpuestoFoxconn(navImpuestos.Current, nsmComprobante));
            }
        ...
        try
        {
            ...
            {
                impuestosComp.Add(new ImpuestoCompFoxconn(navComplemento.Current, nsmComprobante));
            }
		...
		try { sTotalAddenda = navFoxconn.SelectSingleNode("Foxconn/TotalTexto/@qty", nsmComprobante).Value; }
        catch { }
        try { sSalesOrder = navFoxconn.SelectSingleNode("Foxconn/OrdenVenta/@salesOrder", nsmComprobante).Value; }
        catch { }
        try { sDelivery = navFoxconn.SelectSingleNode("Foxconn/Envio/@delivery", nsmComprobante).Value; }
        catch { }
        ...
        if (nNumPag == nTotPag)//Si es ultima pagina
        {
            ...
            Pie.AddAligned(Pie.rWidth - horPadding, RepObj.rAlignRight, verPadding, RepObj.rAlignTop, new RepString(fPropNormal, TransformacionFoxconn.fnFormatoCurrency(subtotal)));
        ...
        if (nNumPag == nTotPag)//Si es ultima pagina
        {
            if (descuento != string.Empty)
            {
                ...
                Pie.AddAligned(Pie.rWidth - horPadding, RepObj.rAlignRight, verPadding + altoRenglon * renglon, RepObj.rAlignTop, new RepString(fPropNormal, TransformacionFoxconn.fnFormatoCurrency(descuento)));
                renglon++;
            }
            foreach (ImpuestoFoxconn i in impuestos)
            {
                if (i.Nombre != "IEPS")
                {
                    Pie.AddAligned(horPadding + posPanelTotales, RepObj.rAlignLeft, verPadding + altoRenglon * renglon, RepObj.rAlignTop, new RepString(fPropNormal, i.Nombre + " " + TransformacionFoxconn.fnFormatoPorcentaje(i.Tasa)));
                    Pie.AddAligned(Pie.rWidth - horPadding, RepObj.rAlignRight, verPadding + altoRenglon * renglon, RepObj.rAlignTop, new RepString(fPropNormal, TransformacionFoxconn.fnFormatoCurrency(i.Importe)));
                    renglon++;
                }
            }
            ...
            try
            {
                ...
                foreach (ImpuestoCompFoxconn i in impuestosComp)
                {
                    Pie.AddAligned(horPadding + posPanelTotales, RepObj.rAlignLeft, verPadding + altoRenglon * renglon, RepObj.rAlignTop, new RepString(fPropNormal, i.Nombre + " " + TransformacionFoxconn.fnFormatoPorcentaje(i.Tasa)));
                    Pie.AddAligned(Pie.rWidth - horPadding, RepObj.rAlignRight, verPadding + altoRenglon * renglon, RepObj.rAlignTop, new RepString(fPropNormal, TransformacionFoxconn.fnFormatoCurrency(i.Importe)));
                    renglon++;
            ...
            if (!sRetIva.Equals(""))
            {
                ...
                Pie.AddAligned(horPadding + posPanelTotales, RepObj.rAlignLeft, verPadding+altoRenglon*renglon, RepObj.rAlignTop, new RepString(fPropNormal, "IvaRet"));
                Pie.AddAligned(Pie.rWidth - horPadding, RepObj.rAlignRight, verPadding + altoRenglon * renglon, RepObj.rAlignTop, new RepString(fPropNormal, sRetIva));
            ...
            Pie.AddAligned(Pie.rWidth - horPadding, RepObj.rAlignRight, tamCodigo - altoRenglon, RepObj.rAlignTop, new RepString(fPropNormal, TransformacionFoxconn.fnFormatoCurrency(total)));
        }
        ...
        if (nNumPag == nTotPag)//Si es ultima pagina
        {
            ...
            Pie.Add(horPadding, altoRenglon, new RepString(fPropNegrita, "Total con letra:"));
            fnAgregarMultilinea(Pie, sTotalAddenda, fPropChica, horPadding, altoRenglon * 1, 80, true);

            fnAgregarMultilinea(Pie, "Forma de Pago:", fPropNegrita, horPadding, altoRenglon * 3, 80, true);
            fnAgregarMultilinea(Pie, formaDePago, fPropChica, horPadding, altoRenglon * 4, 80, true);
            ...
            if (version == "3.2") //Si es version 3.2 muestra el siguiente contenido
            {
                ...
                if (!string.IsNullOrEmpty(metodoPago))
                {
                    ...
                    fnAgregarMultilinea(Pie, metodoPago, fPropChica, horPadding + 140, altoRenglon * 4, 80, true);

                    //Número de cuenta
                    if (!string.IsNullOrEmpty(sNumCtaPago))
                    {
                        fnAgregarMultilinea(Pie, "Número de Cuenta:", fPropNegrita, horPadding + 260, altoRenglon * 3, 80, true);
                        fnAgregarMultilinea(Pie, sNumCtaPago, fPropChica, horPadding + 260, altoRenglon * 4, 80, true);
                    }
                }
                else
                {
                    ...
                    if (!string.IsNullOrEmpty(sNumCtaPago))
                    {
                        fnAgregarMultilinea(Pie, "Número de Cuenta:", fPropNegrita, horPadding + 140, altoRenglon * 3, 80, true);
                        fnAgregarMultilinea(Pie, sNumCtaPago, fPropChica, horPadding + 140, altoRenglon * 4, 80, true);
                    }
                }
            }
			
			fnAgregarMultilinea(Pie, "Sales order:", fPropNegrita, horPadding, altoRenglon * 5, 80, true);
            fnAgregarMultilinea(Pie, sSalesOrder, fPropChica, horPadding, altoRenglon * 6, 80, true);

            fnAgregarMultilinea(Pie, "Delivery:", fPropNegrita, horPadding + 140, altoRenglon * 5, 80, true);
            fnAgregarMultilinea(Pie, sDelivery, fPropChica, horPadding + 140, altoRenglon * 6, 80, true);
        }
        fnCrearPanelRedondeado(Pie, 0, 0, posPanelTotales, tamCodigo, grosorPen, radioCurva, false, ColorT);

		//Sección de datos bancarios de la Addenda
        fnDatosBancarios(Pie, horPadding, altoRenglon);
        //Dibujamos el panel de los datos bancarios
        fnCrearPanelRedondeado(Pie, tamCodigo, tamCodigo, Pie.rWidth - tamCodigo, tamCodigo, grosorPen, radioCurva, false, ColorT);
        ...
        if (nNumPag == nTotPag)//Si es ultima pagina
        {
            ...
            renglon = fnAgregarMultilinea(Pie, TransformacionFoxconn.fnConstruirCadenaTimbrado(gxComprobante.CreateNavigator().SelectSingleNode("/cfdi:Comprobante/cfdi:Complemento/tfd:TimbreFiscalDigital", nsmComprobante).CreateNavigator(), "cadenaoriginal_3_2")
                , fPropChica, horPadding, (tamCodigo * 2) + altoRenglon * renglon, 130, false);
        ...
        fnCrearPanelRedondeado(Pie, 0, (tamCodigo * 2), Pie.rWidth, Pie.rHeight - (tamCodigo * 2) - altoRenglon, grosorPen, radioCurva, false, ColorT);

        //Agrega www.paxfacturacion.com
        fPropNormal = new FontProp(fuenteNormal, 4);
        Pie.AddAligned(Pie.rWidth / 7, RepObj.rAlignCenter, Pie.rHeight - verPadding, RepObj.rAlignTop, new RepString(fPropNormal, Settings.Default.urlHostComercial));
    ...
    /// <summary>
    /// Función que se encarga de imprimir el area de datos bancarios de la addenda
    /// </summary>
    /// <param name="Pie">Contenedor de Pie</param>
    /// <param name="horPadding">Margen horinzontal</param>
    /// <param name="altoRenglon">Alto del renglon</param>
    private void fnDatosBancarios(StaticContainer Pie, double horPadding, double altoRenglon)
    {
        double renglon = 0;
        string sResultado = string.Empty;
        string sAddendaConceptos = string.Empty;
        char[] aCadena = new[] { ' ' };
        double verPadding = Pie.rHeight * 0.02;
        try
        {
            //leemos el nodo adenda
            XmlNodeList addenda = gxComprobante.GetElementsByTagName("cfdi:Addenda");
            if (addenda.Count.Equals(0))
                return;

            XmlNodeList NodoFoxconn = gxComprobante.GetElementsByTagName("Foxconn");
            if (NodoFoxconn.Count.Equals(0))
                return;

            XmlNodeList xnlNumeroCuenta = null;
            XmlNodeList xnlNombreCuenta = null;
            XmlNodeList xnlNombreBanco = null;
            XmlNodeList xnlClaveSwift = null;
            XmlNodeList xnlDireccionBanco = null;
            XmlNodeList xnlContacto = null;

            foreach (XmlElement nodo in NodoFoxconn)
            {
                try { xnlNumeroCuenta = ((XmlElement)NodoFoxconn[0]).GetElementsByTagName("NumeroCuenta"); }
                catch { }

                try { xnlNombreCuenta = ((XmlElement)NodoFoxconn[0]).GetElementsByTagName("NombreCuenta"); }
                catch { }

                try { xnlNombreBanco = ((XmlElement)NodoFoxconn[0]).GetElementsByTagName("NombreBanco"); }
                catch { }

                try { xnlClaveSwift = ((XmlElement)NodoFoxconn[0]).GetElementsByTagName("ClaveSwift"); }
                catch { }

                try { xnlDireccionBanco = ((XmlElement)NodoFoxconn[0]).GetElementsByTagName("DireccionBanco"); }
                catch { }

                try { xnlContacto = ((XmlElement)NodoFoxconn[0]).GetElementsByTagName("Contacto"); }
                catch { }
            }

            renglon = 1;
            Pie.Add(tamCodigo + horPadding, tamCodigo + altoRenglon * renglon++, new RepString(fPropNormal, "Please wire to:"));
            try { Pie.Add(tamCodigo + horPadding, tamCodigo + altoRenglon * renglon++, new RepString(fPropChica, "Account Number: " + xnlNumeroCuenta[0].Attributes.GetNamedItem("Account_Number").Value)); }
            catch { }

            try { Pie.Add(tamCodigo + horPadding, tamCodigo + altoRenglon * renglon++, new RepString(fPropChica, "Account Name: " + xnlNombreCuenta[0].Attributes.GetNamedItem("Account_Name").Value)); }
            catch { }

            try { Pie.Add(tamCodigo + horPadding, tamCodigo + altoRenglon * renglon++, new RepString(fPropChica, "Bank Name: " + xnlNombreBanco[0].Attributes.GetNamedItem("Bank_Name").Value)); }
            catch { }

            try { Pie.Add(tamCodigo + horPadding, tamCodigo + altoRenglon * renglon++, new RepString(fPropChica, "Swift Code: " + xnlClaveSwift[0].Attributes.GetNamedItem("Swift_Code").Value)); }
            catch { }

            try { Pie.Add(tamCodigo + horPadding, tamCodigo + altoRenglon * renglon++, new RepString(fPropChica, "Bank Add: " + xnlDireccionBanco[0].Attributes.GetNamedItem("Bank_Address").Value)); }
            catch { }

            try { Pie.Add(tamCodigo + horPadding, tamCodigo + altoRenglon * renglon++, new RepString(fPropChica, xnlContacto[0].Attributes.GetNamedItem("contact").Value)); }
            catch { }
        }
        catch
        { }
    ...
    private StaticContainer fnCrearEncabezado(System.Drawing.Color sColor)
    {
        ...
        fnShipTo_EnviadoA(ref Encabezado);
    ...
    private void fnDatosEmisor(XmlNamespaceManager nsmComprobante, XPathNavigator navEncabezado, StaticContainer Encabezado, System.Drawing.Color sColor, ref double Renglon)
    {
        ...
        double nPosXEncabezado = (Encabezado.rWidth * 0.4);

        double nPosYEncabezado = 0;

        i = fnAgregarMultilinea(Encabezado, razonSocial, fPropTitulo, leftPadding + nPosXEncabezado, nPosYEncabezado + posRenglon, 40, true);
        fPropTitulo.rSize = 6;
        Renglon += 0.5;
        Renglon += i;
        if (i > 1)
        ...
        if (exCalle != string.Empty)
        {
            i = fnAgregarMultilinea(Encabezado, rfc, fPropNormal, leftPadding + nPosXEncabezado, nPosYEncabezado + posRenglon + tamRenglon * Renglon, 96, true);
            Renglon += i;
            if (i > 1)
                Renglon -= 0.5;
            i = fnAgregarMultilinea(Encabezado, "EXPEDIDO EN", fPropNormal, leftPadding + nPosXEncabezado, nPosYEncabezado + posRenglon + tamRenglon * Renglon, 96, true);
            Renglon += i;
            if (i > 1)
                Renglon -= 0.5;
            i = fnAgregarMultilinea(Encabezado, exDireccion, fPropNormal, leftPadding + nPosXEncabezado, nPosYEncabezado + posRenglon + tamRenglon * Renglon, 96, true);
            Renglon += i;
            if (i > 1)
                Renglon -= 0.5;
            i = fnAgregarMultilinea(Encabezado, exUbicacion, fPropNormal, leftPadding + nPosXEncabezado, nPosYEncabezado + posRenglon + tamRenglon * Renglon, 96, true);
            Renglon += i;
            if (i > 1)
                Renglon -= 0.5;
            i = fnAgregarMultilinea(Encabezado, exUbicacion2, fPropNormal, leftPadding + nPosXEncabezado, nPosYEncabezado + posRenglon + tamRenglon * Renglon, 96, true);
            Renglon += i;
            if (i > 1)
                Renglon -= 0.5;
        ...
        {
            i = fnAgregarMultilinea(Encabezado, direccion, fPropNormal, leftPadding + nPosXEncabezado, nPosYEncabezado + posRenglon + tamRenglon * Renglon, 96, true);
            Renglon += i;
            if (i > 1)
                Renglon -= 0.5;

            i = fnAgregarMultilinea(Encabezado, ubicacion2, fPropNormal, leftPadding + nPosXEncabezado, nPosYEncabezado + posRenglon + tamRenglon * Renglon, 96, true);
            Renglon += i;

            if (i > 1) Renglon -= 0.5;
            i = fnAgregarMultilinea(Encabezado, "Tel. (614)2-36-3000 Fax. (614)2-36-3005", fPropNormal, leftPadding + nPosXEncabezado, nPosYEncabezado + posRenglon + tamRenglon * Renglon, 96, true);
            Renglon += i;

            fnAgregarMultilinea(Encabezado, rfc, fPropNormal, Encabezado.rWidth * 0.05, Encabezado.rHeight * 0.47 - 7 - fPropNegrita.rSize, 96, true);
        ...
        if (!string.IsNullOrEmpty(referencia))
        {
            ...
            //i = fnAgregarMultilinea(Encabezado, Refer, fPropNormal, leftPadding + nPosXEncabezado, nPosYEncabezado + posRenglon + tamRenglon * Renglon, 96, true);
            //Renglon += i;
            //if (i > 1)
            //    Renglon -= 0.5;
            if (version == "3.2")
            {
                string sLugarExp = "Lugar Expedicion: " + estado + " " + pais + " ";
                ...
                i = fnAgregarMultilinea(Encabezado, sLugarExp + sRegFis, fPropNormal, leftPadding + nPosXEncabezado, nPosYEncabezado + posRenglon + tamRenglon * Renglon, 96, true);
                Renglon += i;
                //if (i > 1)
                //    Renglon -= 0.5;
                //i = fnAgregarMultilinea(Encabezado, Regimenfiscal, fPropNormal, leftPadding + nPosXEncabezado, nPosYEncabezado + posRenglon + tamRenglon * Renglon, 96, true);
                //Renglon += i;
                //if (i > 1)
                //    Renglon -= 0.5;
            }
        ...
        else
        {
            if (version == "3.2")
            {
                ...
                i = fnAgregarMultilinea(Encabezado, sRegFis, fPropNormal, leftPadding + nPosXEncabezado, nPosYEncabezado + posRenglon + tamRenglon * Renglon, 96, true);
                Renglon += i;
                if (i > 1)
                    Renglon -= 0.5;
                i = fnAgregarMultilinea(Encabezado, Regimenfiscal, fPropNormal, leftPadding + nPosXEncabezado, nPosYEncabezado + posRenglon + tamRenglon * Renglon, 96, true);
    ...
    private void fnDatosReceptor(XmlNamespaceManager nsmComprobante, XPathNavigator navEncabezado, StaticContainer Encabezado, double Renglon)
    {
        ...
        try { sTaxID = navFoxconn.SelectSingleNode("/Foxconn/TaxID/@taxid").Value; }
        catch { }
        ...
        fnAgregarMultilinea(Encabezado, "VENDIDO A/SOLD TO", fPropNegrita, Encabezado.rWidth * 0.05, nRenglon, 96, true);

        nRenglon += 1 + fPropNegrita.rSize;
        fnAgregarMultilinea(Encabezado, razonSocial, fPropNormal, Encabezado.rWidth * 0.05, nRenglon, 96, true);
        ...
        nRenglon += 1 + fPropNegrita.rSize;
        fnAgregarMultilinea(Encabezado, rfc, fPropNormal, Encabezado.rWidth * 0.05, nRenglon, 96, true);
    ...
    private StaticContainer fnCrearPanelFactura_Invoice(ref StaticContainer Encabezado, Color cColor)
    {
        XmlNamespaceManager nsmComprobante = new XmlNamespaceManager(gxComprobante.NameTable);
        nsmComprobante.AddNamespace("cfdi", "http://www.sat.gob.mx/cfd/3");
        nsmComprobante.AddNamespace("tfd", "http://www.sat.gob.mx/TimbreFiscalDigital");
        ...
        double nLabelX = 180, nLabelY = 13;
        double nContenidoX = 180, nContenidoY = 83;

        double nMargen_Panel = 3;

        StaticContainer scPanel = new StaticContainer(scPanelW, scPanelH);

        string sSerie = "", sFolio = "", sFecha = "", sFechaTimb = "", sCertificado = "";

        string sPedimento = "", sPO = "", sTerminos = "", sInvoice = "", sDocNro = "";



        try { sSerie = navEncabezado.SelectSingleNode("/cfdi:Comprobante/@serie", nsmComprobante).Value; }
        catch { }
        try { sFolio = navEncabezado.SelectSingleNode("/cfdi:Comprobante/@folio", nsmComprobante).Value; }
        catch { }
        try { sFecha = navEncabezado.SelectSingleNode("/cfdi:Comprobante/@fecha", nsmComprobante).Value; }
        catch { }
        try { sFechaTimb = navEncabezado.SelectSingleNode("/cfdi:Comprobante/cfdi:Complemento/tfd:TimbreFiscalDigital/@FechaTimbrado", nsmComprobante).Value; }
        catch { }
        ...
        try
        { sPedimento = navFoxconn.SelectSingleNode("/Foxconn/Pedimento/@pedimento").Value; }
        catch { }

        try { sPO = navFoxconn.SelectSingleNode("/Foxconn/OrdenCompra/@po").Value; }
        catch { }

        try { sTerminos = navFoxconn.SelectSingleNode("/Foxconn/Termino/@terminos").Value; }
        catch { }

        try { sInvoice = navFoxconn.SelectSingleNode("/Foxconn/Factura/@Invoice").Value; }
        catch { }

        try { sDocNro = navFoxconn.SelectSingleNode("/Foxconn/NumeroDocumento/@docNro").Value; }
        catch { }

        RepString[] rpCadenas = new RepString[8];

        RepString rsFecha = new RepString(fPropNormal, "Fecha/Date:         " + sFecha);
        rpCadenas[0] = rsFecha;
        RepString rsSerieFolio = new RepString(fPropNormal, "Serie/Folio:          " + sSerie);
        rpCadenas[1] = rsSerieFolio;
        RepString rsDocN = new RepString(fPropNormal, "Doc. Nro.:            " + sDocNro);
        rpCadenas[2] = rsDocN;
        RepString rsCertificado = new RepString(fPropNormal, "Certificado No.:    " + sCertificado);
        rpCadenas[3] = rsCertificado;
        RepString rsPedimento = new RepString(fPropNormal, "Pedimento No.:    " + sPedimento);
        rpCadenas[4] = rsPedimento;
        RepString rsPO = new RepString(fPropNormal, "O.C/P.O.:             " + sPO);
        rpCadenas[5] = rsPO;
        RepString rsTerminos = new RepString(fPropNormal, "Terminos/Terms: " + sTerminos);
        rpCadenas[6] = rsTerminos;
        RepString rsInvoice = new RepString(fPropNormal, "Invoice:                " + sInvoice);
        rpCadenas[7] = rsInvoice;
        RepString rsTitulo = new RepString(fPropNegrita, "FACTURA/INVOICE");


        fnAgregarMultilinea(scPanel, rsTitulo.sText, rsTitulo.fontProp, nMargen_Panel, nMargen_Panel + fPropNormal.rSize, (int)rsCertificado.rWidth, true);

        int renglon = 0;

        foreach (RepString rpCadena in rpCadenas)
        {

            fnAgregarMultilinea(scPanel, rpCadena.sText, rpCadena.fontProp, nMargen_Panel, nMargen_Panel + fPropNormal.rSize + renglon + nLabelY, (int)rsCertificado.rWidth, true);
            renglon += 10;
        }

        fnCrearPanelRedondeado(scPanel, 0, nLabelY, nContenidoX, nContenidoY, 1, 1, false, cColor);
        fnCrearPanelRedondeado(scPanel, 0, 0, nLabelX, nLabelY, 1, 1, false, cColor);
    ...
    private void fnShipTo_EnviadoA(ref StaticContainer Encabezado)
    {
        ...
        string sNombreEnvio = "", sdireccionEnvio;
        ...
        double nRenglon = Encabezado.rHeight * 0.47;
        fnAgregarMultilinea(Encabezado, "EMBARCAR A/SHIP TO", fPropNegrita, Encabezado.rWidth * 0.35, nRenglon, 96, true);

        nRenglon += fPropNegrita.rSize + 0.5;
        fnAgregarMultilinea(Encabezado, sNombreEnvio, fPropChica, Encabezado.rWidth * 0.35, nRenglon, 46, true);
    ...
    private string fnTextoImporte(string psValor, string psMoneda)
    {
        ...
        NumaletFoxconn parser = new NumaletFoxconn();
        parser.LetraCapital = true;

        switch (psMoneda)
        {
            case "MXN":
                parser.TipoMoneda = NumaletFoxconn.Moneda.Peso;
                break;
            case "USD":
                parser.TipoMoneda = NumaletFoxconn.Moneda.Dolar;
                break;
            case "XEU":
                parser.TipoMoneda = NumaletFoxconn.Moneda.Euro;
                break;
        }
    ...
    private RepImage fnMarcaAguaCorpus()
    {
        ...
        try
        {
            string fichero = Settings.Default["imagenes"].ToString() + "marca_agua_corpus.png";
    ...
    private RepImage fnImagenPAX()
    {
        ...
        try
        {
            string fichero = Settings.Default["imagenes"].ToString() + "logo_pax.png";

            //string fichero = "pax.jpg";
            Image image = Image.FromFile(fichero);
            image.Save(ms, ImageFormat.Jpeg);
        }
        catch (Exception ex)
    ...
    /// <summary>
    /// Funcion que inicializa el Nodelist Addenda para poder accedera los valores
    /// </summary>
    private void fnObtenerAdenda()
    {

        try
        {
            //leemos el nodo adenda
            gaAddenda = gxComprobante.GetElementsByTagName("cfdi:Addenda");
        }
        catch { }
    ...
    /// <summary>
    /// Función que se encarga de obtener el detalle de los conceptos de la addenda
    /// </summary>
    /// <param name="nNumeroConcepto">Numero del concepto</param>
    /// <returns></returns>
    private string fnObtenerConceptoAddenda(int nNumeroConcepto)
    {
        string sResultado = string.Empty;
        string sAddendaConceptos = string.Empty;
        string[] aConcepto = null;
        char[] aCadena = new[] { ' ' };
        try
        {


            if (gaAddenda.Count.Equals(0))
            {
                return sResultado;
            }
            ...
            //string valor = ((XmlElement)NodoFoxconn[0]).GetElementsByTagName("NumeroParte" + nNumeroConcepto);


            foreach (XmlElement nodo in NodoFoxconn)
            {
                XmlNodeList addendaConcepto = ((XmlElement)NodoFoxconn[0]).GetElementsByTagName("NumeroParte" + nNumeroConcepto);
                if (!addendaConcepto.Count.Equals(0))
                {
                    if (!addendaConcepto[0].Attributes.Count.Equals(0))
                    {
                        sAddendaConceptos = addendaConcepto[0].Attributes.GetNamedItem("as" + nNumeroConcepto).Value;
                        aConcepto = sAddendaConceptos.Split(aCadena);
                        int nContador = 0;
                        foreach (string item in aConcepto)
                        {
                            if (nContador.Equals(0))
                            {
...
public class ImpuestoFoxconn
{
    ...
    public string TextoImpuesto
    {
        get
        {
            return Nombre + " " + TransformacionFoxconn.fnFormatoPorcentaje(Tasa) + " " + TransformacionFoxconn.fnFormatoCurrency(Importe);
        }
    }
    ...
    public ImpuestoFoxconn(XPathNavigator navImpuesto, XmlNamespaceManager nsmComprobante)
    {
...
/// tanto para traslados como para retenciones
/// </summary>
public class ImpuestoCompFoxconn
{
    public string Nombre { get; set; }
    public string Tasa { get; set; }
    public string Importe { get; set; }
    ...
    /// <param name="navPie">Navegador XML con los valores de los impuestos</param>
    /// <param name="nsmComprobante"></param>
    public ImpuestoCompFoxconn(XPathNavigator navImpuesto, XmlNamespaceManager nsmComprobante)
    {
        try
        {
...
public class DetalleFoxconn
{
    ...
    private List<ComprobanteImpuestosRetencionTFoxconn> _retencionesT;
    public List<ComprobanteImpuestosRetencionTFoxconn> RetencionesT
    {
        get { return _retencionesT; }
        set { _retencionesT = value; }
    }

    private List<ComprobanteImpuestosTrasladoTFoxconn> _trasladosT;
    public List<ComprobanteImpuestosTrasladoTFoxconn> TrasladosT
    {
        get { return _trasladosT; }
        set { _trasladosT = value; }
    }

    private t_UbicacionFiscalTFoxconn _ubicacionFiscalT;
    public t_UbicacionFiscalTFoxconn UbicacionFiscalT
    {
        get { return _ubicacionFiscalT; }
        set { _ubicacionFiscalT = value; }
    }

    private t_InformacionAduaneraTFoxconn _informacionAduaneraT;
    public t_InformacionAduaneraTFoxconn InformacionAduaneraT
    {
    ...
    /// <param name="nsmComprobante">Manejador de nombres de espacio</param>
    public DetalleFoxconn(XPathNavigator navDetalle, XmlNamespaceManager nsmComprobante)
    {
        try
        {
        ...
        try
        {
            List<ComprobanteImpuestosRetencionTFoxconn> impuestosRetencion = new List<ComprobanteImpuestosRetencionTFoxconn>();
            List<ComprobanteImpuestosTrasladoTFoxconn> impuestosTraslado = new List<ComprobanteImpuestosTrasladoTFoxconn>();
            ...
            {
                ComprobanteImpuestosRetencionTFoxconn impRet = new ComprobanteImpuestosRetencionTFoxconn();
                ...
                try
                {
                    ...
                    if (!String.IsNullOrEmpty(sImpuestoRet))
                    {
                        ...
                        {
                            case "IVA": impRet.impuesto = ComprobanteImpuestosRetencionImpuestoTFoxconn.IVA; break;
                            case "ISR": impRet.impuesto = ComprobanteImpuestosRetencionImpuestoTFoxconn.ISR; break;
                        }
                    }
            ...
            {
                ComprobanteImpuestosTrasladoTFoxconn impTras = new ComprobanteImpuestosTrasladoTFoxconn();
                ...
                try
                {
                    ...
                    if (!String.IsNullOrEmpty(sImpuestoTras))
                    {
                        ...
                        {
                            case "IVA": impTras.impuesto = ComprobanteImpuestosTrasladoImpuestoTFoxconn.IVA; break;
                            case "IEPS": impTras.impuesto = ComprobanteImpuestosTrasladoImpuestoTFoxconn.IEPS; break;
                        }
                    }
            ...
            try
            {
                t_UbicacionFiscalTFoxconn ubicacionTerceros = new t_UbicacionFiscalTFoxconn();
            ...
            try
            {
                t_InformacionAduaneraTFoxconn aduaneraTerceros = new t_InformacionAduaneraTFoxconn();
...
public partial class ComprobanteImpuestosRetencionTFoxconn
{
    private ComprobanteImpuestosRetencionImpuestoTFoxconn impuestoField;
    private decimal importeField;

    public ComprobanteImpuestosRetencionImpuestoTFoxconn impuesto
    {
...
public enum ComprobanteImpuestosRetencionImpuestoTFoxconn
{
    ISR, IVA,
}

public partial class ComprobanteImpuestosTrasladoTFoxconn
{
    private ComprobanteImpuestosTrasladoImpuestoTFoxconn impuestoField;
    private decimal tasaField;
    private decimal importeField;

    public ComprobanteImpuestosTrasladoImpuestoTFoxconn impuesto
    {
        get { return this.impuestoField; }
        set { this.impuestoField = value; }
    }
...
public enum ComprobanteImpuestosTrasladoImpuestoTFoxconn
{
    IVA, IEPS,
}

public partial class t_UbicacionFiscalTFoxconn
{
...
public partial class t_InformacionAduaneraTFoxconn
{
...
/// </summary>
public class TransformacionFoxconn
{
...
public sealed class NumaletFoxconn
{
    ...
    public NumaletFoxconn(Boolean ConvertirDecimales, String MascaraSalidaDecimal, String SeparadorDecimalSalida, Boolean LetraCapital)
    {
    ...
    {
        return NumaletFoxconn.ToString(Numero, CultureInfo.CurrentCulture);
    }
