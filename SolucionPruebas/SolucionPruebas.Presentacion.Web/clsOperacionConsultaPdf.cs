...
public class clsOperacionConsultaPdf
{
    ...
    private StaticContainer fnContarPaginas(List<Detalle> detalles)
    {
        ...
        for (int i = 0; renglonActual <= maxRenglones; i++)
        {
            ...
            else
            {
                ...
                Col3 = fnAgregarMultilineaDetalle(areaDetalle, fnDescripcionTamanioColumna(d.descripcion, 42,renglonActual, maxRenglones - 1), fPropNormal, posColumna3, posRenglon, 42, true);
            }
            ...
            if (renglones > maxRenglones - renglonActual)
            {
                detalles[detalles.IndexOf(d)].descripcion = fnSiguienteDescripcionTamanioColumna(d.descripcion, 42, renglonActual, maxRenglones - 1);
        ...
        for (int i = 0; renglonActual <= maxRenglones; i++)
		{
            ...
            if (d.aduana != null || !String.IsNullOrEmpty(d.RfcT))
            {
                ...
                if (!String.IsNullOrEmpty(d.RfcT))
                {
                    ...
                    Col3 = fnAgregarMultilineaDetalle(areaDetalle, sDetalleTerceros.ToString(), fPropNormal, posColumna3, posRenglon, 55, true);
                }        
            }
            else
            {
                if (noConcepto.Equals(d.noIdentificacion) && !string.IsNullOrEmpty(d.noIdentificacion))
                {
                    if (renglones > maxRenglones - renglonActual)
                    {
	...
	private string fnTextoImporte(string psValor, string psMoneda)
	{
        ...
        languaje = new CultureInfo(SolucionPruebas.Presentacion.Web.Settings.Default.DefaultLanguage);
