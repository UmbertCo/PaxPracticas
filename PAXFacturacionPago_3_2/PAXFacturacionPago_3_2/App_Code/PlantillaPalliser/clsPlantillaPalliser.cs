...
/// </summary>
public class clsPlantillaPalliser
{
    #region atributos
    ...
    //tamaños en puntos
    private const double altoEncabezado = 78;//60;
    ...
    public clsPlantillaPalliser(XmlDocument pxComprobante)
	{
    ...
    public void fnGenerarPDF(int id_contribuyente, int sid_rfc, string scolor)
	{
		...
		while (bSeguir)
		{
            ...
            //Se carga logo Palliser
            ms = fnImagenCliente("Palliser_logo_07_red.jpg");
            if (ms.Length > 0)
            {
                double dAlt = 20;
                double dAnc = 120;
                ...
                double dXimg = 160;//35
                double dYimg = -540;//530
                Pie.Add(margenPagina + dXimg, margenPagina + dYimg, image);
            }

			//Creamos el área de detalle
            double dPosDetalleX = 120;//120
            double dPosDetalleY = 350;//500
            //pagina.Add(margenPagina + dPosDetalleX, margenPagina + dPosDetalleY, fnMarcaAguaPalliser());
	...
	/// <returns></returns>
    private StaticContainer fnCrearDetalle(List<Detalle> detalles, System.Drawing.Color sColor)
	{
		StaticContainer areaDetalle = new StaticContainer(RT.rPointFromMM(anchoPagina) - margenPagina * 2, RT.rPointFromMM(altoPagina - altoEncabezado - altoPie) - margenPagina * 2);
		...
		while (navDetalles.MoveNext())
		{
            ...
            if (nodenavigator.HasChildren)//Si contiene nodo hijo
            {
                ...
                if (navDetallesAduana.Count > 0)//Si contiene Informacion Aduanal se agrega al concepto
                    detalles.Add(new Detalle(navDetallesAduana.Current, nsmComprobante));
            }
		}
    ...
    private void fnTotales(XmlNamespaceManager nsmComprobante, XPathNavigator navPie, StaticContainer Pie, int nNumPag, int nTotPag, string sColor)
	{
        ...
        #region  Datos de Addenda

        //Declaracion de variables 

        string sInvoice_Totals_DueDate, sInvoice_Discount, sInvoice_Totals_English, sInvoice_Totals_CargoDelEmbarque, sInvoice_Totals_CargoMisc, sPaymentTerms, sInvoice_DiscountIfPayByDate, sInvoice_Totals_Spanish, sInvoice_Totals_Currency;

        sInvoice_Totals_DueDate = sInvoice_Discount = sInvoice_Totals_English = sInvoice_Totals_CargoDelEmbarque = sInvoice_Totals_CargoMisc = sPaymentTerms = sInvoice_DiscountIfPayByDate = sInvoice_Totals_Spanish = sInvoice_Totals_Currency = string.Empty;

        XmlNodeList adenda = gxComprobante.GetElementsByTagName("cfdi:Addenda");
        //Acceso al nodo de totales
        XmlNodeList lista = ((XmlElement)adenda[0]).GetElementsByTagName("Totals");
        int c = 0;
        //Recorremos el nodo
        foreach (XmlElement nodo in lista)
        {
            XmlNodeList Invoice_Totals_DueDate = nodo.GetElementsByTagName("Invoice_Totals_DueDate");
            sInvoice_Totals_DueDate = Invoice_Totals_DueDate[c].InnerText;

            XmlNodeList Invoice_Discount = nodo.GetElementsByTagName("Invoice_Discount");
            sInvoice_Discount = Invoice_Discount[c].InnerText;

            XmlNodeList Invoice_Totals_Spanish = nodo.GetElementsByTagName("Invoice_Totals_Spanish");
            sInvoice_Totals_Spanish = Invoice_Totals_Spanish[c].InnerText;

            XmlNodeList Invoice_Totals_English = nodo.GetElementsByTagName("Invoice_Totals_English");
            sInvoice_Totals_English = Invoice_Totals_English[c].InnerText;

            XmlNodeList Invoice_Totals_Currency = nodo.GetElementsByTagName("Invoice_Totals_Currency");
            sInvoice_Totals_Currency = Invoice_Totals_Currency[c].InnerText;

            XmlNodeList Invoice_Totals_CargoDelEmbarque = nodo.GetElementsByTagName("Invoice_Totals_CargoDelEmbarque");
            sInvoice_Totals_CargoDelEmbarque = Invoice_Totals_CargoDelEmbarque[c].InnerText;

            XmlNodeList Invoice_Totals_CargoMisc = nodo.GetElementsByTagName("Invoice_Totals_CargoMisc");
            sInvoice_Totals_CargoMisc = Invoice_Totals_CargoMisc[c].InnerText;

            XmlNodeList PaymentTerms = nodo.GetElementsByTagName("PaymentTerms");
            sPaymentTerms = PaymentTerms[c].InnerText;

            XmlNodeList Invoice_DiscountIfPayByDate = nodo.GetElementsByTagName("Invoice_DiscountIfPayByDate");
            sInvoice_DiscountIfPayByDate = Invoice_DiscountIfPayByDate[c].InnerText;
        }
        ...
        if (nNumPag == nTotPag)//Si es ultima pagina
        {
            ...
            Pie.AddAligned(Pie.rWidth - horPadding, RepObj.rAlignRight, verPadding, RepObj.rAlignTop, new RepString(fPropNormal, Transformacion.fnFormatoCurrency(subtotal)));
        }
        ...
        if (nNumPag == nTotPag)//Si es ultima pagina
        {
            ...
            Pie.AddAligned(Pie.rWidth - horPadding, RepObj.rAlignRight, tamCodigo - altoRenglon, RepObj.rAlignTop, new RepString(fPropNormal, Transformacion.fnFormatoCurrency(total)));
        }
        ...
        if (nNumPag == nTotPag)//Si es ultima pagina
        {
            ...
            double dlargo = 270;
            Pie.Add(0, altoRenglon - 50, new RepLine(new PenProp(PDF, grosorPen, ColorT), posPanelTotales - tamCodigo + dlargo, 0));

            //*************************************** Mostramos datos de la andenda *************************************************************************//
            /*<Totals */
            double dpoXtotales = 295;
            /*<Invoice_Totals_DueDate>*/renglon += fnAgregarMultilinea(Pie, "Pagar antes de: ", fPropNegrita, tamCodigo + horPadding + dpoXtotales, altoRenglon - 35, 80, true);
            dpoXtotales = 430;
            fnAgregarMultilinea(Pie,sInvoice_Totals_DueDate, fPropNormal, tamCodigo + horPadding + dpoXtotales, altoRenglon - 35, 80, true);
            double salto = 1;
            //dpoXtotales = 120;                //Concatenamos los campos <Invoice_Discount> + <Invoice_DiscountIfPayByDate>
            //fnAgregarMultilinea(Pie, "Pagar: ",fPropNegrita,tamCodigo + horPadding, altoRenglon, 80, true);
            //dpoXtotales = 45;
            //fnAgregarMultilinea(Pie,sInvoice_Discount, fPropNormal, tamCodigo + horPadding + dpoXtotales, altoRenglon, 80, true);
            //dpoXtotales = 150;
            //fnAgregarMultilinea(Pie, "si paga antes de ", fPropNegrita, tamCodigo + horPadding + dpoXtotales, altoRenglon, 80, true);
            //dpoXtotales = 230;
            //fnAgregarMultilinea(Pie,sInvoice_DiscountIfPayByDate, fPropNormal, tamCodigo + horPadding + dpoXtotales, altoRenglon, 80, true);
            dpoXtotales = 150;
            //fnAgregarMultilinea(Pie, "Remitente:" + " " + "12315.35" + " " + "si fue pagado por " + "07/14/12", fPropNormal, tamCodigo + horPadding, altoRenglon, 80, true);
            salto += fnAgregarMultilinea(Pie, "CARGO DEL EMBARQUE", fPropNegrita, tamCodigo + horPadding + dpoXtotales, altoRenglon * salto, 80, true);
            /*<Invoice_Totals_CargoDelEmbarque>*/
            salto += fnAgregarMultilinea(Pie,sInvoice_Totals_CargoDelEmbarque, fPropNormal, tamCodigo + horPadding + dpoXtotales, altoRenglon * salto, 80, true);
            /*</Invoice_Totals_CargoDelEmbarque>*/
            salto += fnAgregarMultilinea(Pie, "CARGO MISCELANEOS", fPropNegrita, tamCodigo + horPadding + dpoXtotales, altoRenglon * salto, 80, true);
            /*<Invoice_Totals_CargoMisc>*/
            salto += fnAgregarMultilinea(Pie,sInvoice_Totals_CargoMisc, fPropNormal, tamCodigo + horPadding + dpoXtotales, altoRenglon * salto, 80, true);
            /*</Invoice_Totals_CargoMisc>*/
            /*<PaymentTerms>*/fnAgregarMultilinea(Pie, "CONDICIONES", fPropNegrita, tamCodigo + horPadding + dpoXtotales, altoRenglon * salto, 80, true);
            dpoXtotales = 230;
            fnAgregarMultilinea(Pie,sPaymentTerms,fPropNormal, tamCodigo + horPadding + dpoXtotales, altoRenglon * salto, 80, true);
            dpoXtotales = 120;                //Concatenamos los campos <Invoice_Discount> + <Invoice_DiscountIfPayByDate>
            double dAlto = 6.4;
            fnAgregarMultilinea(Pie, "Pagar: ", fPropNegrita, tamCodigo + horPadding, altoRenglon * dAlto , 80, true);
            dpoXtotales = 45;
            fnAgregarMultilinea(Pie, sInvoice_Discount, fPropNormal, tamCodigo + horPadding + dpoXtotales, altoRenglon * dAlto, 80, true);
            dpoXtotales = 150;
            fnAgregarMultilinea(Pie, "si paga antes de ", fPropNegrita, tamCodigo + horPadding + dpoXtotales, altoRenglon * dAlto, 80, true);
            dpoXtotales = 230;
            fnAgregarMultilinea(Pie, sInvoice_DiscountIfPayByDate, fPropNormal, tamCodigo + horPadding + dpoXtotales, altoRenglon * dAlto, 80, true);

            double dvalor = 35;
            /*<Invoice_Totals_English>*/fnAgregarMultilinea(Pie,sInvoice_Totals_English, fPropNormal,5, altoRenglon * 2.8 - dvalor, 80, true);

            /*</Totals>*/
            salto += 2.4;
            fnAgregarMultilinea(Pie,"Esta Factura será pagada en la ciudad de Saltillo, Coahuila", fPropNegrita, tamCodigo + horPadding , altoRenglon * salto, 80, true);
            salto += 0.8;
            fnAgregarMultilinea(Pie,"Debo y pagaré el importe de esta factura, la cual si no es pagada dentro del término estipulado causará interés del _________% mensual",fPropChica, tamCodigo + horPadding, altoRenglon * salto, 80, true);
            dpoXtotales = 380;
            salto += 0.5;
            fnAgregarMultilinea(Pie, sInvoice_Totals_Currency, fPropNegrita, tamCodigo + horPadding + dpoXtotales, altoRenglon * salto, 80, true);
            ...
            Pie.Add(5,altoRenglon - dvalor, new RepString(fPropNegrita, "Total con letra:"));
            fnAgregarMultilinea(Pie, sInvoice_Totals_Spanish,fPropChica,5, altoRenglon * 1.7 - dvalor, 80, true);///*fnTextoImporte(Transformacion.fnFormatoRedondeo(total)*/
            double daltoReglon = 1;
            fnAgregarMultilinea(Pie, "Forma de Pago:", fPropNegrita, tamCodigo + horPadding, altoRenglon * daltoReglon, 80, true);
            daltoReglon = 2;
            fnAgregarMultilinea(Pie, formaDePago, fPropChica, tamCodigo + horPadding, altoRenglon * daltoReglon, 80, true);
            ...
            if (version == "3.2") //Si es version 3.2 muestra el siguiente contenido
            {
                ...
                {
                    daltoReglon = 3;
                    fnAgregarMultilinea(Pie, "Método de Pago:", fPropNegrita, tamCodigo + horPadding, altoRenglon * daltoReglon, 80, true);
                    daltoReglon = 3.8;
                    fnAgregarMultilinea(Pie, metodoPago, fPropChica, tamCodigo + horPadding, altoRenglon * daltoReglon, 80, true);
                    ...
                    {
                        daltoReglon = 4.7;
                        fnAgregarMultilinea(Pie, "Número de Cuenta:", fPropNegrita, tamCodigo + horPadding, altoRenglon * daltoReglon, 80, true);
                        daltoReglon = 5.5;
                        fnAgregarMultilinea(Pie, sNumCtaPago, fPropChica, tamCodigo + horPadding, altoRenglon * daltoReglon, 80, true);
                    }
                }
                else
                {
	...
	private void fnDatosEmisor(XmlNamespaceManager nsmComprobante, XPathNavigator navEncabezado, StaticContainer Encabezado, System.Drawing.Color sColor, ref double Renglon)
	{
        string razonSocial, rfc, calle, noExterior, noInterior, colonia, municipio, estado, pais, codigoPostal, serie, folio, fecha, fechaTimb,
            noCertificadoEmisor, referencia, Localidad, Regimenfiscal, version, estadolug, noAutorizacion, fechaAutorizacion, leyenda, exCalle, exNoInterior, exNoExterior,
            exColonia, exMunicipio, exEstado, exPais, exCodigoPostal, exLocalidad, tipoDeComprobante;
        razonSocial = rfc = calle = noExterior = noInterior = colonia = municipio = estado = pais = codigoPostal = fecha = serie = folio =
            noCertificadoEmisor = referencia = Localidad = Regimenfiscal = version = fechaTimb = estadolug = noAutorizacion = fechaAutorizacion = leyenda =
        exCalle = exNoInterior = exNoExterior = exColonia = exMunicipio = exEstado = exPais = exCodigoPostal = exLocalidad = tipoDeComprobante = string.Empty;
        ...
        try { tipoDeComprobante = navEncabezado.SelectSingleNode("/cfdi:Comprobante/@tipoDeComprobante", nsmComprobante).Value; }
        catch{}
        ...
        #region Adenda Palliser

        //Declaracion de variables

        string sCustNum, sInvoiceNum, sPONum, sComOrderNum, sCarrierDescription, sTextLineDescription, sCurrentBatchNum, sWaADDR0, sWaADDR5, sshADDR0, sshADDR1, sshADDR2, sshADDR3, sshADDR4, sshADDR5, sadfolio, sShipmentNum;
        sCustNum = sInvoiceNum = sPONum = sComOrderNum = sCarrierDescription = sTextLineDescription = sCurrentBatchNum = sWaADDR0 = sWaADDR5 = sshADDR0 = sshADDR1 = sshADDR2 = sshADDR3 = sshADDR4 = sshADDR5 = sadfolio = sShipmentNum=string.Empty;

        //leemos el nodo adenda
        XmlNodeList adenda = gxComprobante.GetElementsByTagName("cfdi:Addenda");
        XmlNodeList lista = ((XmlElement)adenda[0]).GetElementsByTagName("Header");
        try
        {
            int c = 0;
            //recorremos el nodo Hearder
            foreach (XmlElement nodo in lista)
            {
                XmlNodeList CustNum = nodo.GetElementsByTagName("CustNum");
                //Asignamos el valor correspondiente del nodo a la variable definida
                sCustNum = CustNum[c].InnerText;

                XmlNodeList InvoiceNum = nodo.GetElementsByTagName("InvoiceNum");
                //Asignamos el valor correspondiente del nodo a la variable definida
                sInvoiceNum = InvoiceNum[c].InnerText;

                XmlNodeList PONum = nodo.GetElementsByTagName("PONum");
                //Asignamos el valor correspondiente del nodo a la variable definida
                sPONum = PONum[c].InnerText;

                XmlNodeList ComOrderNum = nodo.GetElementsByTagName("ComOrderNum");
                //Asignamos el valor correspondiente del nodo a la variable definida
                sComOrderNum = ComOrderNum[c].InnerText;

                XmlNodeList CarrierDescription = nodo.GetElementsByTagName("CarrierDescription");
                //Asignamos el valor correspondiente del nodo a la variable definida
                sCarrierDescription = CarrierDescription[c].InnerText;

                XmlNodeList TexLineDescription = nodo.GetElementsByTagName("TextLineDescription");
                //Asignamos el valor correspondiente del nodo a la variable definida
                sTextLineDescription = TexLineDescription[c].InnerText;

                XmlNodeList CurrentBatchNum = nodo.GetElementsByTagName("CurrentBatchNum");
                sCurrentBatchNum = CurrentBatchNum[c].InnerText;

                XmlNodeList adfolio = nodo.GetElementsByTagName("folio");
                sadfolio = adfolio[c].InnerText;

                XmlNodeList ShipmentNum =  nodo.GetElementsByTagName ("ShipmentNum");
                sShipmentNum = ShipmentNum[c].InnerText;

            }
            //Accedemos al nodo Warehouse
            lista = ((XmlElement)adenda[0]).GetElementsByTagName("Warehouse");
            foreach (XmlElement nodo in lista)//recorremos el nodo 
            {
                XmlNodeList waADDR0 = nodo.GetElementsByTagName("ADDR0");
                //Asignamos el valor correspondiente del nodo a la variable definida
                sWaADDR0 = waADDR0[c].InnerText;
                XmlNodeList WaADDR5 = nodo.GetElementsByTagName("ADDR5");
                sWaADDR5 = WaADDR5[c].InnerText;
            }
            //Accedemos al nodo ShipTo
            lista = ((XmlElement)adenda[0]).GetElementsByTagName("ShipTo");
            foreach (XmlElement nodo in lista)
            {
                XmlNodeList shADDR0 = nodo.GetElementsByTagName("ADDR0");
                XmlNodeList shADDR1 = nodo.GetElementsByTagName("ADDR1");
                XmlNodeList shADDR2 = nodo.GetElementsByTagName("ADDR2");
                XmlNodeList shADDR3 = nodo.GetElementsByTagName("ADDR3");
                XmlNodeList shADDR4 = nodo.GetElementsByTagName("ADDR4");
                XmlNodeList shADDR5 = nodo.GetElementsByTagName("ADDR5");
                //Asignamos el valor correspondiente del nodo a la variable definida
                sshADDR0 = shADDR0[c].InnerText;
                sshADDR1 = shADDR1[c].InnerText;
                sshADDR2 = shADDR2[c].InnerText;
                sshADDR3 = shADDR3[c].InnerText;
                sshADDR4 = shADDR4[c].InnerText;
                sshADDR5 = shADDR5[c].InnerText;
            }
            lista = ((XmlElement)adenda[0]).GetElementsByTagName("");
        ...
        /*************************************************** Enviar pago A:  *****************************************************/
        int nTamCaracter = 40 ;
        //Renglon = fnAgregarMultilinea(Encabezado, "ENVIAR PAGO A: ", fPropTitulo, leftPadding, posRenglon, 50, true);

        //<SolBy>
        /*<ADDR1>*/Renglon += fnAgregarMultilinea(Encabezado,razonSocial, fPropNormal, leftPadding, posRenglon, nTamCaracter, true);
        ///*<ADDR2>*/Renglon += fnAgregarMultilinea(Encabezado,"S. DE RL. DE C.V. ", fPropNormal, leftPadding, posRenglon + tamRenglon * Renglon, nTamCaracter, true);
        /*<ADDR3>*/Renglon += fnAgregarMultilinea(Encabezado,direccion, fPropNormal, leftPadding, posRenglon + tamRenglon * Renglon, nTamCaracter, true);
        nTamCaracter = 20;
        /*<ADDR4>*/Renglon += fnAgregarMultilinea(Encabezado,ubicacion, fPropNormal, leftPadding, posRenglon + tamRenglon * Renglon, nTamCaracter, true);
        nTamCaracter = 40;
        ///*<ADDR5>*/Renglon += fnAgregarMultilinea(Encabezado,estado, fPropNormal, leftPadding, posRenglon + tamRenglon * Renglon, nTamCaracter, true);
        /*<ADDR6>*/Renglon += fnAgregarMultilinea(Encabezado,rfc, fPropNormal, leftPadding, posRenglon + tamRenglon * Renglon, nTamCaracter, true);
        //</SolBy>"
        
        /************************************************** Expedido en **********************************************************************/
        //double dPX = 280;
        sep = 5;
        posRazon = fPropTitulo.rSize + sep;
        tamRenglon = fPropNormal.rSize + sep;
        posRenglon = posRazon + sep;
        Renglon = 2;

        //Renglon += fnAgregarMultilineaCentrada(Encabezado, "ESTA MERCANCIA HA SIDO SURTIDA POR: ", fPropNegrita, leftPadding + dPX, posRenglon, 40);
        //Renglon += fnAgregarMultilineaCentrada(Encabezado, "PALLISER DE MEXICO S DE RL DE CV ", fPropNormal, leftPadding + dPX, posRenglon + tamRenglon * Renglon, 40);
        //Renglon += fnAgregarMultilineaCentrada(Encabezado, "BLVD ISIDRO LOPEZ Z.5182 ", fPropNormal, leftPadding + dPX, posRenglon + tamRenglon * Renglon, 40);
        //Renglon += fnAgregarMultilineaCentrada(Encabezado, "SALTILLO, COAHUILA ", fPropNormal, leftPadding + dPX, posRenglon + tamRenglon * Renglon, 40);
        //Renglon += fnAgregarMultilineaCentrada(Encabezado, "CP25220 MEX ", fPropNormal, leftPadding + dPX, posRenglon + tamRenglon * Renglon, 40);
        //Renglon += fnAgregarMultilineaCentrada(Encabezado, "RFC PHI830429MG6 ", fPropNormal, leftPadding + dPX, posRenglon + tamRenglon * Renglon, 40);

        double dPX = 190;
        fPropNegrita.rSize = 4;
        fPropNormal.rSize = 4;
        Renglon += fnAgregarMultilinea(Encabezado, "Expedido en :", fPropNegrita, leftPadding + dPX, posRenglon + tamRenglon * Renglon, 40, true);

        /*<cfdi:Emisor nombre></cfdi>*/
        //Renglon += fnAgregarMultilinea(Encabezado, razonSocial + " " + "S DE RL DE CV ", fPropNormal, leftPadding + dPX, posRenglon + tamRenglon * Renglon, 40, true);
        ///*<cfdi:ExpedidoEn>*/Renglon += fnAgregarMultilinea(Encabezado, "S DE RL DE CV ", fPropNormal, leftPadding + dPX, posRenglon + tamRenglon * Renglon, 40, true);
        /**/
        Renglon += fnAgregarMultilinea(Encabezado, exDireccion, fPropNormal, leftPadding + dPX, posRenglon + tamRenglon * Renglon, 40, true);
        Renglon += fnAgregarMultilinea(Encabezado, exUbicacion, fPropNormal, leftPadding + dPX, posRenglon + tamRenglon * Renglon, 40, true);
        fPropNegrita.rSize = 6;
        dPX = 160;
        Renglon += fnAgregarMultilinea(Encabezado, Regimenfiscal, fPropNegrita, leftPadding + dPX, posRenglon + tamRenglon * Renglon, 40, true);
        /*<cfdi:Emisor RFC></cfdi>*/
        //Renglon += fnAgregarMultilinea(Encabezado, rfc, fPropNormal, leftPadding + dPX, posRenglon + tamRenglon * Renglon, 40, true);
        ...
        Renglon += fnAgregarMultilinea(Encabezado, "ENVIADO ", fPropNegrita, leftPadding + dPosX, posRenglon + tamRenglon * Renglon, 1, true);
        dPosX = 230;
        Renglon = 7;
        nTamCaracter = 35;
        /*<ShipTo>*/
        /*<ADDR0>*/Renglon += fnAgregarMultilinea(Encabezado,sshADDR0, fPropNormal, leftPadding + dPosX, posRenglon + tamRenglon * Renglon, nTamCaracter, true);
        Renglon = 7;
        dPosX = 280;
        /*<Header */
        /*<ShipmentNum>*/Renglon += fnAgregarMultilinea(Encabezado,sShipmentNum, fPropNormal, leftPadding + dPosX, posRenglon + tamRenglon * Renglon, nTamCaracter, true);
        /*</Header*/
        dPosX = 230;
        /*<ShipTo>*/
        /*<ADDR1>*/Renglon += fnAgregarMultilinea(Encabezado,sshADDR1, fPropNormal, leftPadding + dPosX, posRenglon + tamRenglon * Renglon, nTamCaracter, true);
        /*<ADDR2>*/Renglon += fnAgregarMultilinea(Encabezado,sshADDR2, fPropNormal, leftPadding + dPosX, posRenglon + tamRenglon * Renglon, nTamCaracter, true);
        /*<ADDR3>*/Renglon += fnAgregarMultilinea(Encabezado,sshADDR3, fPropNormal, leftPadding + dPosX, posRenglon + tamRenglon * Renglon, nTamCaracter, true);
        /*<ADDR4>*/Renglon += fnAgregarMultilinea(Encabezado,sshADDR4, fPropNormal, leftPadding + dPosX, posRenglon + tamRenglon * Renglon, nTamCaracter, true);
        /*<ADDR5>*/Renglon += fnAgregarMultilinea(Encabezado,sshADDR5, fPropNormal, leftPadding + dPosX, posRenglon + tamRenglon * Renglon, nTamCaracter, true);
        /*</ShipTo>*/
        posRenglon = 170;

        /*<Header>*/
        Renglon=fnAgregarMultilinea(Encabezado, "CLIENTE ", fPropNormal, leftPadding, posRenglon , nTamCaracter, true);
        /*<CustNum>*/Renglon += fnAgregarMultilinea(Encabezado,sCustNum, fPropNormal, leftPadding, posRenglon + tamRenglon * Renglon, nTamCaracter, true);
        dPosX = 190;
        Renglon = 0;
        Renglon=fnAgregarMultilinea(Encabezado, "#ORDEN ", fPropNormal, leftPadding + dPosX, posRenglon , nTamCaracter, true);
        /*<PONum>*/Renglon += fnAgregarMultilinea(Encabezado,sPONum, fPropNormal, leftPadding + dPosX, posRenglon + tamRenglon * Renglon, nTamCaracter, true);
        dPosX =368;
        Renglon = 0;
        Renglon= fnAgregarMultilinea(Encabezado, "NUESTRA ORDEN ENVIO ", fPropNormal, leftPadding + dPosX, posRenglon, nTamCaracter, true);
        /*<ComOrderNum>*/Renglon += fnAgregarMultilinea(Encabezado,sComOrderNum, fPropNormal, leftPadding + dPosX, posRenglon + tamRenglon * Renglon, nTamCaracter, true);
        //salto de linea 
        dPosX = 60;//135;
        Renglon= 2.6;
        nTamCaracter = 90;
        fnAgregarMultilinea(Encabezado, "Transportistas : ", fPropNormal, leftPadding + dPosX, posRenglon + tamRenglon * Renglon, nTamCaracter, true);
        dPosX += 65;                            //Concatenamos los campos <CarrierDescription> + <CurrentBatchNum>
        fnAgregarMultilinea(Encabezado, sCarrierDescription + " " + sCurrentBatchNum + " " + sTextLineDescription, fPropNormal, leftPadding + dPosX, posRenglon + tamRenglon * Renglon, nTamCaracter, true);
        ...
        fnCrearPanelRedondeado(Encabezado, Encabezado.rWidth - 189.5, 0, fAnchoPanel + 75, fAltoPanel + 13, grosorPen, radioCurva, false, sColor);
        //Panel inferior de encabezado
        //double dpfPosY = 145;
        //double dfAnchoP = 457.8;
        //fnCrearPanelRedondeado(Encabezado, Encabezado.rWidth- 572,dpfPosY,fAnchoPanel + dfAnchoP, fAltoPanel - 3, grosorPen, radioCurva, false, sColor);
        //dpfPosY = 177.8;
        //fnCrearPanelRedondeado(Encabezado, Encabezado.rWidth - 572,dpfPosY, fAnchoPanel + dfAnchoP, fAltoPanel - 2.5, grosorPen, radioCurva, false, sColor);
        //double dTamano = 37.5;
        
        posX = posX + 23;
        if (tipoDeComprobante == "egreso")
            tipoDeComprobante = "Nota de crédito";
        else
            tipoDeComprobante = "Factura";
        fnAgregarMultilineaCentrada(Encabezado, tipoDeComprobante, fPropNegrita, posX + fAnchoPanel / 2, fAltoPanel / 3, 15);
        posX = posX - 90 ;
        Renglon = fnAgregarMultilineaCentrada(Encabezado, "COM# ", fPropNegrita, posX + fAnchoPanel / 3, fAltoPanel / 3, 15);
        fnAgregarMultilineaCentrada(Encabezado,"ALMACEN ", fPropNegrita, posX + fAnchoPanel / 2.5, fAltoPanel * Renglon, 15);
        ...
        leftPadding = Encabezado.rWidth * 0.02;
        ...
        if (!string.IsNullOrEmpty(sUUID))
        {
            Encabezado.AddAligned(posX, RepObj.rAlignRight,
                fAltoPanel + fAltoPanel /2, RepObj.rAlignTop,
                new RepString(fPropNegrita, "Folio Fiscal:"));
            Encabezado.AddAligned(posX, RepObj.rAlignRight,
                fAltoPanel + fAltoPanel / 1.2, RepObj.rAlignTop,
        ...
        if (!string.IsNullOrEmpty(serie) && !string.IsNullOrEmpty(folio))
        {
            ...
            Encabezado.AddAligned(posX, RepObj.rAlignRight,
                    fAltoPanel-nPosY, RepObj.rAlignTop,
        ...
        /*<Header*/
        /*<InvoiceNum>*/
        Encabezado.AddAligned(posX-100, RepObj.rAlignRight,
                    fAltoPanel - nPosY, RepObj.rAlignTop,
                new RepString(fPropRoja,sInvoiceNum));   
        /*</InvoiceNum>*/
        /*</Header*/
        /************************ Datos de ALMACEN ***********************************************/
        
        /*<Warehouse>*/       //Concatenamos los campos <ADDR0> + <ADDR5>        
        Encabezado.AddAligned(posX , RepObj.rAlignRight,
                    fAltoPanel + 6, RepObj.rAlignTop,
                new RepString(fPropRoja,sWaADDR0 + " "+ sWaADDR5));
        /*</Warehouse>*/
	...
	private void fnDatosReceptor(XmlNamespaceManager nsmComprobante, XPathNavigator navEncabezado, StaticContainer Encabezado, double Renglon)
	{
        ...
        /**********************************************************************************************************************************/
        Renglon = 7;
        Renglon += fnAgregarMultilinea(Encabezado, "FACTURADO ", fPropNegrita, leftPadding, posRenglon + tamRenglon * Renglon, 1, true);
        Renglon = 7;
        int nTamCaracter = 35;
        double dPosX = 10;
        int nRenglon=0;
        Renglon += nRenglon=fnAgregarMultilinea(Encabezado, razonSocial, fPropNormal, leftPadding + dPosX, posRenglon + tamRenglon * Renglon, nTamCaracter, true);
        if (nRenglon > 1)
            Renglon = 8.5;
        Renglon += fnAgregarMultilinea(Encabezado,calle, fPropNormal, leftPadding + dPosX, posRenglon + tamRenglon * Renglon, nTamCaracter, true);
        Renglon += fnAgregarMultilinea(Encabezado,Localidad, fPropNormal, leftPadding + dPosX, posRenglon + tamRenglon * Renglon, nTamCaracter, true);
        Renglon += fnAgregarMultilinea(Encabezado,pais, fPropNormal, leftPadding + dPosX, posRenglon + tamRenglon * Renglon, nTamCaracter, true);
        Renglon += fnAgregarMultilinea(Encabezado,rfc, fPropNormal, leftPadding + dPosX, posRenglon + tamRenglon * Renglon, nTamCaracter, true);
        //Renglon += fnAgregarMultilinea(Encabezado, "PHI830429MG6 ", fPropNormal, leftPadding + dPosX, posRenglon + tamRenglon * Renglon, nTamCaracter, true);
        //Renglon += fnAgregarMultilinea(Encabezado, "EL PALACIO DE HIERRO, SA DE CV ", fPropNormal, leftPadding + dPosX, posRenglon + tamRenglon * Renglon, nTamCaracter, true);
        //Renglon += fnAgregarMultilinea(Encabezado, "CASA PALACIO ", fPropNormal, leftPadding + dPosX, posRenglon + tamRenglon * Renglon, nTamCaracter, true);
        //Renglon += fnAgregarMultilinea(Encabezado, "DURANGO #230 COL. ROMA ", fPropNormal, leftPadding + dPosX, posRenglon + tamRenglon * Renglon, nTamCaracter, true);
        //Renglon += fnAgregarMultilinea(Encabezado, "DELEGACION CUAHTEMOC ", fPropNormal, leftPadding + dPosX, posRenglon + tamRenglon * Renglon, nTamCaracter, true);
        //Renglon += fnAgregarMultilinea(Encabezado, "MEXICO, D.F. C.P. MX 06700 ", fPropNormal, leftPadding + dPosX, posRenglon + tamRenglon * Renglon, nTamCaracter, true);
        //Renglon += fnAgregarMultilinea(Encabezado, "PHI830429MG6 ", fPropNormal, leftPadding + dPosX, posRenglon + tamRenglon * Renglon, nTamCaracter, true);
	}
    ...
    private void fnAgregarBordeRedondeado(StaticContainer poObjeto, double pfGrosor, double pfRadioCurva, System.Drawing.Color sColor)
    {
        ...
        //Linea superior
        double dLugarY = -190.5;
        double dTamLinea = 375;
        double dLugarX = dTamLinea + 3;
        poObjeto.Add(pfRadioCurva + dLugarX, dLugarY, new RepLine(pen, poObjeto.rWidth - lArc - dTamLinea, 0));

        //Linea inferior 
        dLugarY = -62;
        dTamLinea = -7.3;
        dLugarX = dTamLinea + 3.3;
        poObjeto.Add(pfRadioCurva + dLugarX, dLugarY, new RepLine(pen, poObjeto.rWidth - lArc - dTamLinea, 0));
    ...
    /// </summary>
    /// <returns></returns>
    private RepImage fnMarcaAguaPalliser()
    {
        MemoryStream ms = new MemoryStream();
        try
        {
            string fichero = HttpContext.Current.Server.MapPath("~/Imagenes/Palliser_logo_07_red.jpg");
        ...
        double drWidth = 320;
        double drHeight = 60;
        return new RepImage(ms,drWidth,drHeight);
    }
...
public class clsErrorLogPalliser
{
    //private static clsInicioSesionUsuario gUsuario;
    public clsErrorLogPalliser()
    {
    }
...
public class clsComunPDF
{
    public static string ObtenerParamentro(string sParametro)
    {
        ...
        using (SqlConnection con = new SqlConnection(PAXCrypto.CryptoAES.DesencriptaAES64(ConfigurationManager.ConnectionStrings["conControl"].ConnectionString)))
        {
            ...
            {
                con.Close();
                clsErrorLogPalliser.fnNuevaEntrada(ex, clsErrorLogPalliser.TipoErroresLog.Conexion);
            }
        }
        return sRetorno;
    }
