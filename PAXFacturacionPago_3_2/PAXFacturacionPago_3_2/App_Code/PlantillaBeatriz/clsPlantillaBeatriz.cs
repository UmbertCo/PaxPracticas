...
/// </summary>
    ...
    public class clsPlantillaBeatriz 
    {
        #region atributos
        ...
        //tamaños en puntos
        private const double altoEncabezado = 95;
        ...
        public clsPlantillaBeatriz(XmlDocument pxComprobante)
        {
            try
            {
        ...
        public void fnGenerarPDF(int id_contribuyente, int sid_rfc, string scolor)
        {
            ...
            //Letra titulo
            fuenteTitulo = new FontDef(PDF, FontDef.StandardFont.TimesRoman);//TimesRoman
            ...
            while (bSeguir)
            {
                ...
                Color ColorT = ColorTranslator.FromHtml("#A6A6A6");
                ...
                fPropNormal.rSize = 6;
                pagina.AddAligned(RT.rPointFromMM(anchoPagina) - margenPagina, RepObj.rAlignRight, margenPagina + 720, RepObj.rAlignCenter, new RepString(fPropNormal, pagina.iPageNo.ToString() + " de " + nTotPag));
                ...
                //    double sWidth = 70;
                //    double sHeight = 694;
                //    RepImage image = new RepImage(ms, sWidth, sHeight);
                //    double sY = 702;
                //    pagina.Add(margenPagina, margenPagina + sY, image);
                //}

                //Se carga imagen logo pie
                ms = fnImagenClienteDir("LogoBeatrizV.jpg");
                if (ms.Length > 0)
                {
                    double sWidth = 60;
                    double sHeight = 400;
                ...
                //double dpie = 755;
                pagina.Add(margenPagina, margenPagina + 720, fnImagenPAX());
        ...
        {
            StaticContainer areaDetalle = new StaticContainer(RT.rPointFromMM(anchoPagina) - margenPagina * 2, RT.rPointFromMM(altoPagina - altoEncabezado - altoPie - RT.rPointFromMM(.5)) - margenPagina * 2);

            fnAgregarBordeRedondeado(areaDetalle, grosorPen, radioCurva, Color.Red);
            ...
            double dSeparador = 0.22;//Separador de titulos
            double dIncremento = 0;//Incremento

            //******** Definimos la posicion de detalles ********//
            double posColumna1 = areaDetalle.rWidth * dSeparador;                            //CANTIDAD 0.104 incremento de separador de titulos de detalle
            dIncremento = 0.104;
            dSeparador += dIncremento;
            double posColumna2 = areaDetalle.rWidth * dSeparador;                            //UNIDAD DE MEDIDA
            dIncremento = 0.128;
            dSeparador += dIncremento;
            double posColumna3 = areaDetalle.rWidth * dSeparador;// 0.39;                    //descripcion
            dIncremento = 0.38;
            dSeparador += dIncremento;
            double posColumna4 = areaDetalle.rWidth * dSeparador;// 0.68;                    //Precio
            dIncremento = 0.155;
            dSeparador += dIncremento;
            double posColumna5 = areaDetalle.rWidth * dSeparador;                            //Importe
            //double posColumna5 = Encabezado.rWidth * 0.81;                                 //Importe
            ...
            double posColumna3 = areaDetalle.rWidth * 0.30; //0.2;
            ...
            for (int i = 0; renglonActual <= maxRenglones; i++)
            {
                ...
                //primero verificamos si la descripción cabrá en el espacio restante
                int nTamReglon = 42;
                renglones = fnContarMultilinea(areaDetalle, d.descripcion, fPropNormal, posColumna3, posRenglon, nTamReglon, true);
        ...
        private void fnTotales(XmlNamespaceManager nsmComprobante, XPathNavigator navPie, StaticContainer Pie, int nNumPag, int nTotPag, string sColor)
        {
            ...
            fPropNormal = new FontProp(fuenteNormal, tamFuenteNormal);
            Pie.AddAligned(Pie.rWidth / 2, RepObj.rAlignCenter, tamCodigo + daltoY, RepObj.rAlignTop, new RepString(fPropNormal, leyendaPDF));

            //Agrega www.paxfacturacion.com
            fPropNormal = new FontProp(fuenteNormal, 4);
            Pie.AddAligned(Pie.rWidth / 7, RepObj.rAlignCenter, tamCodigo + daltoY, RepObj.rAlignTop, new RepString(fPropNormal, clsComunLogo.ObtenerParamentro("urlHostComercial")));
            ...
            //********************************************************************************************************************//
            //********************************** Paneles Inferiones Pie **********************************************************//
            //********************************************************************************************************************//
            double drWidthPanel = 110;
            double drHeight = 18.5;
            double dDisPanel = 40; //Distancia de los paneles
            double dalto = 73;
            Color cPaneles = ColorTranslator.FromHtml("#EA090B");
            fnCrearPanelRedondeado(Pie, 0, Pie.rHeight - tamCodigo + dalto, drWidthPanel, drHeight, grosorPen, radioCurva, true, cPaneles);
            double dsuma = 80;
            dDisPanel += dsuma;
            fnCrearPanelRedondeado(Pie, dDisPanel, Pie.rHeight - tamCodigo + dalto, drWidthPanel, drHeight, grosorPen, radioCurva, true, cPaneles);
            dsuma = 120;
            dDisPanel += dsuma;
            fnCrearPanelRedondeado(Pie, dDisPanel, Pie.rHeight - tamCodigo + dalto, drWidthPanel, drHeight, grosorPen, radioCurva, true, cPaneles);
            dsuma = 120;
        ...
        private StaticContainer fnCrearEncabezado(System.Drawing.Color sColor)
        {
            ...
            //********************************************************************************************************************//
            //********************************** Paneles Top Encabezado **********************************************************//
            //********************************************************************************************************************//
            double drWidthPanel = 110;
            double drHeight = 18.5;
            double dDisPanel = 40; //Distancia de los paneles
            Color cPaneles = ColorTranslator.FromHtml("#EA090B");
            fnCrearPanelRedondeado(Encabezado, 0, 0, drWidthPanel, drHeight, grosorPen, radioCurva, true, cPaneles);
            double dsuma = 80;
            dDisPanel += dsuma;
            fnCrearPanelRedondeado(Encabezado, dDisPanel, 0, drWidthPanel, drHeight, grosorPen, radioCurva, true, cPaneles);
            dsuma = 120;
            dDisPanel += dsuma;
            fnCrearPanelRedondeado(Encabezado, dDisPanel, 0, drWidthPanel, drHeight, grosorPen, radioCurva, true, cPaneles);
            dsuma = 120;
        ...
        private void fnDibujarTitulosDetalle(StaticContainer Encabezado, System.Drawing.Color sColor)
        {
            ...
            double altoBarra = fPropBlanca.rSize * 2.7;
            //double dAnchoTitulos = 510;
            //Encabezado.rWidth = dAnchoTitulos;//510
            //Panel de los Titulos
            fnCrearPanelRedondeado(Encabezado, Encabezado.rWidth - fPropBlanca.rSize * 80, Encabezado.rHeight - altoBarra, Encabezado.rWidth - Encabezado.rHeight + 178, altoBarra, grosorPen, radioCurva, false, sColor);
            ...
            double dSeparador = 0.185;//Separador de titulos
            double dIncremento = 0;//Incremento
            int nTaRenglon = 10;//Tamaño del reglon para el multilinia

            //******** Definimos la posicion de los titulos ********//
            double posColumna1 = Encabezado.rWidth * dSeparador;                            //CANTIDAD 0.104 incremento de separador de titulos de detalle
            dIncremento = 0.104;
            dSeparador += dIncremento;
            double posColumna2 = Encabezado.rWidth * dSeparador;                            //UNIDAD DE MEDIDA
            dIncremento = 0.28;
            dSeparador += dIncremento;
            double posColumna3 = Encabezado.rWidth * dSeparador;// 0.39;                    //descripcion
            dIncremento = 0.2;
            dSeparador += dIncremento;
            double posColumna4 = Encabezado.rWidth * dSeparador;// 0.68;                    //Precio
            dIncremento = 0.15;
            dSeparador += dIncremento;
            double posColumna5 = Encabezado.rWidth * dSeparador;                            //Importe

            //******** Agregado de Titulos ********//
            Encabezado.AddAligned(posColumna1, RepObj.rAlignLeft, puntoMedio, RepObj.rAlignCenter, new RepString(fPropNegrita, "CANTIDAD"));
            //Encabezado.AddAligned(posColumna1, RepObj.rAlignLeft, puntoMedio, RepObj.rAlignCenter, new RepString(fPropNegrita, "CLAVE"));
            //fnAgregarMultilinea(Encabezado, "UNIDAD DE MEDIDA", fPropNegrita, posColumna2, puntoMedio, nTaRenglon, true);
            Encabezado.AddAligned(posColumna2, RepObj.rAlignLeft, puntoMedio, RepObj.rAlignCenter, new RepString(fPropNegrita, "UNIDAD DE MEDIDA"));
            Encabezado.AddAligned(posColumna3, RepObj.rAlignLeft, puntoMedio, RepObj.rAlignCenter, new RepString(fPropNegrita, "DESCRIPCIÓN"));
            Encabezado.AddAligned(posColumna4, RepObj.rAlignLeft, puntoMedio, RepObj.rAlignCenter, new RepString(fPropNegrita, "PRECIO"));
            Encabezado.AddAligned(posColumna5, RepObj.rAlignLeft, puntoMedio, RepObj.rAlignCenter, new RepString(fPropNegrita, "TOTAL"));
        }
        ...
        private void fnDatosEmisor(XmlNamespaceManager nsmComprobante, XPathNavigator navEncabezado, StaticContainer Encabezado, System.Drawing.Color sColor, ref double Renglon)
        {
            ...
            razonSocial = rfc = calle = noExterior = noInterior = colonia = municipio = estado = pais = codigoPostal = fecha = serie = folio = noCertificadoEmisor =
                referencia = Localidad = version = Regimenfiscal = fechaTimb = estadolug = noAutorizacion = fechaAutorizacion = leyenda = string.Empty;
            ...
            if (adenda.Count > 0)
            {
                XmlNodeList lista = ((XmlElement)adenda[0]).GetElementsByTagName("CargoExpress");

                foreach (XmlElement nodo in lista)
                {
                    XmlNodeList xFeguia = nodo.GetElementsByTagName("FechaGuia");
                    XmlNodeList xFepago = nodo.GetElementsByTagName("Fechapago");
                    XmlNodeList xNoguia = nodo.GetElementsByTagName("Noguia");
                    XmlNodeList xOrigen = nodo.GetElementsByTagName("Origen");
                    XmlNodeList xOrigenCompra = nodo.GetElementsByTagName("OrigenCompra");
                    XmlNodeList xDestino = nodo.GetElementsByTagName("Destino");
                    XmlNodeList xUnidad = nodo.GetElementsByTagName("Unidad");

                    try
                    {
                        sFeguia = xFeguia[0].InnerText;
                    }
                    catch { }

                    char[] cCad = { '/' };
                    string[] sCad = sFeguia.Split(cCad);
                    string sNombreMes = string.Empty;

                    if (sCad.Length > 0)
                    {
                        string smes = sCad[1].ToString();
                        switch (smes)
                        {
                            case "01":
                                sNombreMes = "Enero";
                        ...
                        sFeguia = sNombreMes + " " + sCad[0].ToString() + "," + sCad[2].ToString();
                    }
                    try
                    {
                        sFepago = xFepago[0].InnerText;
                    }
                    catch { }

                    sCad = sFepago.Split(cCad);
                    ...
                    if (sCad.Length > 0)
                    {
                        ...
                        sFepago = sNombreMes + " " + sCad[0].ToString() + "," + sCad[2].ToString();
                    }
                    try
                    {
                        sNoGuia = xNoguia[0].InnerText;
                    }
                    catch { }
                    try
                    {
                        sOrdenCompra = xOrigenCompra[0].InnerText;
                    }
                    catch { }
                    try
                    {
                        sOrigen = xOrigen[0].InnerText;
                    }
                    catch { }
                    try
                    {
                        sDestino = xDestino[0].InnerText;
                    }
                    catch { }
                    try
                    {
                        sUnidad = xUnidad[0].InnerText;
                    }
                    catch { }
                }
            }
            ...
            Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignLeft, posRenglon - dry, RepObj.rAlignTop, new RepString(fPropTitulo, razonSocial));// "Beatriz Eugenia Ruiz Ochoa"));
            //Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignLeft, posRenglon - 9, RepObj.rAlignTop, new RepString(fPropTitulo, razonSocial));
            ...
            //salto de renglon 
            dposRen += dcalRen;
            Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, "Col. " + colonia));
            //salto de renglon 
            dposRen += dcalRen;
            Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, ubicacion));
            //salto de renglon 
            dposRen += dcalRen;
            Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, ubicacion2));
            //salto de renglon 
            dposRen += dcalRen;
            Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, "Tel y Fax: (614) 541 5525"));
            //salto de renglon 
            dposRen += dcalRen;
            Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, rfc));
            //salto de renglon 
            dposRen += dcalRen;
            //Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, "REGIMEN FISCAL: " + Regimenfiscal));
            //dposRen += dcalRen;
            #endregion

            //Panel para datos de Receptor
            fnCrearPanelRedondeado(Encabezado, Encabezado.rWidth - fPropBlanca.rSize * 80, posRenglon + tamRenglon * dposRen + 3, Encabezado.rWidth - Encabezado.rHeight + 178, Encabezado.rHeight - fPropBlanca.rSize * 35, grosorPen, radioCurva, false, sColor);
            ...
            dposRen += dcalRen;
            Color ColorT = ColorTranslator.FromHtml("#A6A6A6");
            fPropNormal.color = ColorT;
            Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, "Fecha de embarque :"));
            dPX = 175;
            fPropNormal.color = Color.Black;
            Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, sFeguia));
            //salto de renglon 
            dPX = 92;
            dcalRen = 1;
            dposRen += dcalRen;
            ColorT = ColorTranslator.FromHtml("#A6A6A6");
            fPropNormal.color = ColorT;
            Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, "Fecha de pago :"));
            dPX = 175;
            fPropNormal.color = Color.Black;
            Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, sFepago));
            //salto de renglon 
            dPX = 92;
            dposRen += dcalRen;
            ColorT = ColorTranslator.FromHtml("#A6A6A6");
            fPropNormal.color = ColorT;
            Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, "No. Guía :"));
            dPX = 175;
            fPropNormal.color = Color.Black;
            Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, sNoGuia));
            //salto de renglon 
            dPX = 92;
            dposRen += dcalRen;
            ColorT = ColorTranslator.FromHtml("#A6A6A6");
            fPropNormal.color = ColorT;
            Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, "Orden de Compra :"));
            dPX = 175;
            fPropNormal.color = Color.Black;
            Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, sOrdenCompra));
            //salto de renglon 
            dPX = 92;
            dcalRen = 2.7999999999999998;
            dposRen += dcalRen;
            ColorT = ColorTranslator.FromHtml("#A6A6A6");
            fPropNormal.color = ColorT;
            Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, "Origen :"));
            dPX = 175;
            fPropNormal.color = Color.Black;
            Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, sOrigen));
            //salto de renglon 
            dPX = 92;
            dcalRen = 1;
            dposRen += dcalRen;
            ColorT = ColorTranslator.FromHtml("#A6A6A6");
            fPropNormal.color = ColorT;
            Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, "Destino :"));
            dPX = 175;
            fPropNormal.color = Color.Black;
            Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, sDestino));
            //salto de renglon 
            dPX = 92;
            dposRen += dcalRen;
            ColorT = ColorTranslator.FromHtml("#A6A6A6");
            fPropNormal.color = ColorT;
            Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, "Unidad :"));
            dPX = 175;
            fPropNormal.color = Color.Black;
            Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, sUnidad));


            #endregion

            #region NoAutorizacion
            ...
            try { sRegimen = navEncabezado.SelectSingleNode("/cfdi:Comprobante/cfdi:Emisor/cfdi:RegimenFiscal/@Regimen", nsmComprobante).Value; }
            catch { }
            ...
            double fAltoPanel = Encabezado.rHeight / 11;//6;
            double fAnchoPanel = Encabezado.rWidth / 5;
            double posX = 0;
            fPropTitulo.rSize = 7.5;

            posX = 3;

            //Panel Datos Fiscales
            fnCrearPanelRedondeado(Encabezado, Encabezado.rWidth - fPropBlanca.rSize * 42, Encabezado.rHeight - fPropBlanca.rSize * 21.7, Encabezado.rWidth - Encabezado.rHeight - 50, Encabezado.rHeight - fPropBlanca.rSize * 26, grosorPen, radioCurva, false, sColor);

            //Paneles Addenda 
            fnCrearPanelRedondeado(Encabezado, Encabezado.rWidth - fPropBlanca.rSize * 80, Encabezado.rHeight - fPropBlanca.rSize * 21.7, Encabezado.rWidth - Encabezado.rHeight - 75, Encabezado.rHeight - fPropBlanca.rSize * 35.5, grosorPen, radioCurva, false, sColor);
            fnCrearPanelRedondeado(Encabezado, Encabezado.rWidth - fPropBlanca.rSize * 80, Encabezado.rHeight - fPropBlanca.rSize * 12.3, Encabezado.rWidth - Encabezado.rHeight - 75, Encabezado.rHeight - fPropBlanca.rSize * 35.4, grosorPen, radioCurva, false, sColor);
            ...
            if (!string.IsNullOrEmpty(serie) && !string.IsNullOrEmpty(folio))
            {
                Encabezado.AddAligned(posX, RepObj.rAlignLeft,
                        posRenglon - dry, RepObj.rAlignTop,
                    new RepString(fPropTitulo, "Serie y Folio:"));
                double sposXseFo = 65;
                Encabezado.AddAligned(posX + sposXseFo, RepObj.rAlignLeft,
                        posRenglon - dry, RepObj.rAlignTop,
                    new RepString(fPropTitulo, serie + folio));
            }

            posX = Encabezado.rWidth - leftPadding + 5.5;

            //Tipo de Documento
            //fnAgregarMultilineaCentrada(Encabezado, TipoDocumento, fPropNegrita, posX - 457.6 + fAnchoPanel / 6, fAltoPanel / 2 + 4, 15);

            fAltoPanel = Encabezado.rHeight / 2.2;

            fAltoPanel += fAltoPanel - RT.rPointFromMM(5);
            double enter = 1.62;
            ...
            if (!string.IsNullOrEmpty(sUUID))
            {
                Encabezado.AddAligned(posX, RepObj.rAlignRight,
                    fAltoPanel / enter, RepObj.rAlignTop,
            ...
            {
                Encabezado.AddAligned(posX, RepObj.rAlignRight,
                    fAltoPanel + 40, RepObj.rAlignTop,
                    new RepString(fPropNegrita, "Fecha y hora de certificación:"));
                Encabezado.AddAligned(posX, RepObj.rAlignRight,
                    fAltoPanel + 48, RepObj.rAlignTop,
            ...
            {
                Encabezado.AddAligned(posX, RepObj.rAlignRight,
                    fAltoPanel + 58, RepObj.rAlignTop,
                    new RepString(fPropNegrita, "Lugar, fecha y hora de emisión:"));
                Encabezado.AddAligned(posX, RepObj.rAlignRight,
                    fAltoPanel + 66, RepObj.rAlignTop,
            ...
            {
                Encabezado.AddAligned(posX, RepObj.rAlignRight,
                    fAltoPanel + 78, RepObj.rAlignTop,
                    new RepString(fPropNegrita, "Regimen Fiscal: "+ sRegimen));
            }

            #endregion

            fPropTitulo.color = Color.Black;
            fPropNormal = new FontProp(fuenteNormal, tamFuenteNormal);
            fPropNegrita = new FontProp(fuenteNormal, tamFuenteNormal, Color.Black);
            fPropNegrita.bBold = true;
            fPropRoja = new FontProp(fuenteNormal, tamFuenteNormal, Color.Red);
        ...
        private void fnDatosReceptor(XmlNamespaceManager nsmComprobante, XPathNavigator navEncabezado, StaticContainer Encabezado, double Renglon)
        {
            ...
            try { codigoPostal = "C.P. " + navEncabezado.SelectSingleNode("/cfdi:Comprobante/cfdi:Receptor/cfdi:Domicilio/@codigoPostal", nsmComprobante).Value; }
            catch { }
            ...
            double leftPadding = Encabezado.rWidth - fPropBlanca.rSize * 78;
            double sep = 5;//10
            ...
            //i = fnAgregarMultilinea(Encabezado, "Recibimos de ", fPropNegrita, leftPadding, posRenglon + tamRenglon * Renglon, 90, true);
            i = fnAgregarMultilinea(Encabezado, razonSocial, fPropNormal, leftPadding, posRenglon + tamRenglon * Renglon++, 90, true);
            Renglon += i;
            if (i > 1)
                Renglon -= 1.5;
            else
                Renglon -= 1;
            //i = fnAgregarMultilinea(Encabezado, "RFC ", fPropNegrita, leftPadding, posRenglon + tamRenglon * Renglon, 90, true);
            //Renglon += i;
            //if (i > 1)
            //    Renglon -= 1.5;
            //else
            //    Renglon -= 1;
            i = fnAgregarMultilinea(Encabezado, rfc, fPropNormal, leftPadding, posRenglon + tamRenglon * Renglon++, 90, true);

            //i = fnAgregarMultilinea(Encabezado, "Con Domicilio en ", fPropNegrita, leftPadding, posRenglon + tamRenglon * Renglon, 90, true);
            //Renglon += i;
            //if (i > 1)
            //    Renglon -= 1.5;
            //else
            ...
            if (i > 1)
                Renglon -= 0.5;
            i = fnAgregarMultilinea(Encabezado, colonia, fPropNormal, leftPadding, posRenglon + tamRenglon * Renglon, 90, true);
            Renglon += i;
            if (i > 1)
                Renglon -= 0.5;
            i = fnAgregarMultilinea(Encabezado, codigoPostal + " " + ubicacion, fPropNormal, leftPadding, posRenglon + tamRenglon * Renglon, 90, true);
        ...
        private void fnAgregarBordeRedondeado(StaticContainer poObjeto, double pfGrosor, double pfRadioCurva, System.Drawing.Color sColor)
        {
            ...
            //esquina superior izquierda
            poObjeto.Add(pfRadioCurva + 87, lArc, new RepArc(pen, pfRadioCurva, 180, 90));

            //esquina superior derecha
            poObjeto.Add(poObjeto.rWidth - lArc, lArc, new RepArc(pen, pfRadioCurva, 270, 90));

            //esquina inferior izquierda
            poObjeto.Add(pfRadioCurva + 87, poObjeto.rHeight, new RepArc(pen, pfRadioCurva, 90, 90));
            ...
            //añadimos los bordes rectos
            //borde superior
            poObjeto.Add(pfRadioCurva + 90, 0, new RepLine(pen, poObjeto.rWidth - 95, 0));

            //borde inferior
            poObjeto.Add(pfRadioCurva + 90, poObjeto.rHeight, new RepLine(pen, poObjeto.rWidth - 95, 0));

            ////borde izquierdo
            poObjeto.Add(pfRadioCurva + 87, pfRadioCurva, new RepLine(pen, 0, pfRadioCurva * 2 - poObjeto.rHeight));
        ...
        private RepImage GenerarCodigoBidimensional()
        {
            ...
            //Tamalo codigo Bidimensional 2.75
            return new RepImage(ms, tamCodigo - 7.5, tamCodigo - 7.5);
        }
        ...
        public Report ObtenerPlantilla()
        {
            return PDF;
        }
    }
