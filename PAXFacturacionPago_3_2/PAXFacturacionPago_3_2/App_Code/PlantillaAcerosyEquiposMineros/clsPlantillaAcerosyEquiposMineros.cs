...
/// </summary>
public class clsAcerosyEquiposMineros
{
    #region atributos
    ...
    //tamaños en puntos
    private const double altoEncabezado = 60;//60
    private double altoPie = 120;//70
    ...
    public clsAcerosyEquiposMineros(XmlDocument pxComprobante)
    {
        try
        {
    ...
    public void fnGenerarPDF(int id_contribuyente, int sid_rfc, string scolor)
    {
        ...
        while (bSeguir)
        {
            ...
            if (ms.Length > 0)
            {
                double drWidth = 90;//100
                double drHeight = 72;//80
                RepImage image = new RepImage(ms, drWidth, drHeight);
                double dMargPosX = 12;
                double dMargPosY = 80; //85;
                pagina.Add(margenPagina + dMargPosX, margenPagina + dMargPosY, image);
            }
            ...
            if (pagina.iPageNo == nTotPag)//si es ultima pagina
            {
                ...
                ms = fnImagenClienteCarp("CIFelicianoArreola.jpg");
                if (ms.Length > 0)
                {
                    double dAlt = 98;
                    double dAnc = 65;
            ...
            //Agrega marca de agua de Aceros
            double drX = 150;
            double drY = 390;
            pagina.Add(margenPagina + drX, margenPagina + drY, fnMarcaAguaAcerosyEquiposMineros());
    ...
    public void fnGenerarPDFSave(int id_contribuyente, int sid_rfc, string scolor, string ruta)
    {
        ...
        while (bSeguir)
        {
            ...
            if (ms.Length > 0)
            {
                double drWidth = 100;//100
                double drHeight = 80;//80
                RepImage image = new RepImage(ms, drWidth, drHeight);
                double dMargPosX = 12;
                double dMargPosY = 85;
                pagina.Add(margenPagina + dMargPosX, margenPagina + dMargPosY, image);
    ...
    private void fnTotales(XmlNamespaceManager nsmComprobante, XPathNavigator navPie, StaticContainer Pie, int nNumPag, int nTotPag, string sColor)
    {
        ...
        if (nNumPag == nTotPag)//Si es ultima pagina
        {
            ...
            fnAgregarMultilinea(Pie, "Forma de Pago:", fPropNegrita, tamCodigo + horPadding, altoRenglon * 2.6, 80, true);
            fnAgregarMultilinea(Pie, formaDePago, fPropChica, tamCodigo + horPadding, altoRenglon * 3.2, 80, true);
            ...
            if (version == "3.2") //Si es version 3.2 muestra el siguiente contenido
            {
                ...
                {
                    fnAgregarMultilinea(Pie, "Método de Pago:", fPropNegrita, tamCodigo + horPadding, altoRenglon * 4.2, 80, true);
                    fnAgregarMultilinea(Pie, metodoPago, fPropChica, tamCodigo + horPadding, altoRenglon * 4.8, 80, true);

                    //Número de cuenta
                    if (!string.IsNullOrEmpty(sNumCtaPago))
                    {
                        fnAgregarMultilinea(Pie, "Número de Cuenta:", fPropNegrita, tamCodigo + horPadding, altoRenglon * 5.6, 80, true);
                        fnAgregarMultilinea(Pie, sNumCtaPago, fPropChica, tamCodigo + horPadding, altoRenglon * 6.3, 80, true);
                    }
                }
                else
                {
                    ...
                    if (!string.IsNullOrEmpty(sNumCtaPago))
                    {
                        fnAgregarMultilinea(Pie, "Número de Cuenta:", fPropNegrita, tamCodigo + horPadding, altoRenglon * 4.2, 80, true);
                        fnAgregarMultilinea(Pie, sNumCtaPago, fPropChica, tamCodigo + horPadding, altoRenglon * 4.8, 80, true);
                    }
                }
            }
        }
        ...
        if (nNumPag == nTotPag)//Si es ultima pagina
        {
            ...
            renglon += 0.8;
            renglon += fnAgregarMultilinea(Pie, sello, fPropChica, horPadding, tamCodigo + altoRenglon * renglon, 100, false);
            renglon -= 0.5;
            Pie.Add(horPadding, tamCodigo + altoRenglon * renglon, new RepString(fPropNormal, "Sello digital del SAT:"));
            renglon += 0.8;
            renglon += fnAgregarMultilinea(Pie, timbre, fPropChica, horPadding, tamCodigo + altoRenglon * renglon, 100, false);
            renglon -= 0.5;
            ...
            renglon += fnAgregarMultilinea(Pie, TransformacionLogo.fnConstruirCadenaTimbrado(gxComprobante.CreateNavigator().SelectSingleNode("/cfdi:Comprobante/cfdi:Complemento/tfd:TimbreFiscalDigital", nsmComprobante).CreateNavigator(), "cadenaoriginal_TFD_1_0")
                , fPropChica, horPadding, tamCodigo + altoRenglon * renglon, 100, false);
        ...
        /////////////////////////////////-Adenda-/////////////////////////////////////
        string sNumOrden = string.Empty;
        string sCondPago = string.Empty;
        string sDiasCredito = string.Empty;
        string sDiaVencimiento = string.Empty;
        ...
        try
        {
            ...
            XmlNodeList lista =
            ((XmlElement)Adenda[0]).GetElementsByTagName("FacturaAcerosyEquiposMineros");
            ...
            foreach (XmlElement nodo in lista)
            {
                ...
                XmlNodeList xNumOrden =
                nodo.GetElementsByTagName("numOrden");

                XmlNodeList xCondPago =
                nodo.GetElementsByTagName("condPago");

                XmlNodeList xDiaVencimiento =
                nodo.GetElementsByTagName("diaVencimiento");


                sNumOrden = xNumOrden[a].InnerText;

                XmlElement xnCondPago = (XmlElement)xCondPago[a];

                sCondPago = xnCondPago.FirstChild.InnerText;

                if (sCondPago.ToUpper() == "CRÉDITO")
                {
                    sDiasCredito = xnCondPago["diasCredito"].FirstChild.InnerText;
                }

                sDiaVencimiento = xDiaVencimiento[a].InnerText;
            }
        }
        catch { nBanAdd = 1; }
        ...
        renglon = 1;
        double dHeight = renglon * (fPropChica.rSize * 1.2) + Pie.rHeight;
        altoPie = Pie.rHeightMM;

        renglon = 8.9;
        double altopnl = 57.5;
        //dibujamos el borde del pie   fnCrearPanelRedondeado(Pie, 0, tamCodigo, Pie.rWidth, Pie.rHeight - tamCodigo - altoRenglon, grosorPen, radioCurva, false, ColorT);
        fnCrearPanelRedondeado(Pie, 0, tamCodigo, Pie.rWidth, Pie.rHeight - tamCodigo - tamCodigo - altopnl, grosorPen, radioCurva, false, ColorT);
        //dibujamos el borde2 del pagare
        fnCrearPanelRedondeado(Pie, 0, tamCodigo + 102.8, Pie.rWidth, dHeight - tamCodigo - altoRenglon - 105, grosorPen, radioCurva, false, ColorT);
        double dposX = 0;
        //Leyenda
        Pie.Add(horPadding + dposX, tamCodigo + altoRenglon * renglon++, new RepString(fPropChica, "SI EL COBRO DE ÉSTA FACTURA REQUIERE DE INTERVENCIÓN JUDICIAL SERÁN COMPETENTES LOS TRIBUNALES DE LA CIUDAD DE HIDALGO DEL PARRAL, CHIH."));
        Pie.Add(horPadding + dposX, tamCodigo + altoRenglon * renglon, new RepString(fPropChica, "EN CASO DE MORA EN EL PAGO DE ÉSTE DOCUMENTO SE CARGARÁ UN INTERÉS MENSUAL A LA TASA DEL "));
        Pie.Add(horPadding + 373, tamCodigo + altoRenglon * renglon++, new RepString(fPropNegrita, "4%"));

        //Pagare
        renglon += 0.4;
        Pie.Add(horPadding + dposX, tamCodigo + altoRenglon * renglon, new RepString(fPropChica, "DEBO(EMOS) Y PAGARÉ(MOS) INCONDICIONALMENTE POR ÉSTE PAGARÉ A LA ORDEN DE"));// FELICIANO ARREOLA MAGAÑA EN LA CIUDAD DE HIDALGO DEL PARRAL CHIHUAHUA "));
        Pie.Add(horPadding + 312, tamCodigo + altoRenglon * renglon, new RepString(fPropNegrita, "FELICIANO ARREOLA MAGAÑA "));
        Pie.Add(horPadding + 445, tamCodigo + altoRenglon * renglon++, new RepString(fPropChica, "EN LA CIUDAD DE HIDALGO "));// HIDALGO DEL PARRAL CHIHUAHUA EL DIA "));// (DIA FECHA + DÍAS DE CRÉDITO) LA CANTIDAD "));
        Pie.Add(horPadding + dposX, tamCodigo + altoRenglon * renglon, new RepString(fPropChica, "DEL PARRAL CHIHUAHUA EL DÍA "));
        Pie.Add(horPadding + 115/*150*/, tamCodigo + altoRenglon * renglon++, new RepString(fPropNegrita, ((!String.IsNullOrEmpty(sDiasCredito)) ? String.Format("{0}", sDiaVencimiento) : sDiaVencimiento))); //+ {1} DÍAS DE CRÉDITO", sDiaVencimiento, sDiasCredito) : sDiaVencimiento)));
        Pie.Add(horPadding, tamCodigo + altoRenglon * renglon, new RepString(fPropChica, "LA CANTIDAD DE $ "));
        //fnAgregarMultilinea(Pie, "(CANTIDAD CON NUMERO Y LETRA DEL TOTAL DE LA FACTURA) ", fPropNegrita, horPadding + 320, tamCodigo + altoRenglon * renglon, 60, true);
        Pie.Add(horPadding + 70, tamCodigo + altoRenglon * renglon++, new RepString(fPropNegrita, total + " " + fnTextoImporte(TransformacionLogo.fnFormatoRedondeo(total), moneda)));
        Pie.Add(horPadding + dposX, tamCodigo + altoRenglon * renglon++, new RepString(fPropChica, "VALOR DE MERCANCÍA RECIBIDO A MI ENTERA SATISFACCIÓN. ÉSTE PAGARÉ ES MERCANTIL, Y ESTÁ REGIDO POR LA LEY GENERAL DE TÍTULOS Y OPERACIONES"));
        Pie.Add(horPadding + dposX, tamCodigo + altoRenglon * renglon++, new RepString(fPropChica, "DE CRÉDITO EN SU ARTÍCULO 173 PARTE FINAL Y ARTÍCULOS CORRELATIVOS POR NO SER UN PAGARÉ DOMICILIADO. "));
        //Pie.Add(horPadding + dposX, tamCodigo + altoRenglon * renglon++, new RepString(fPropChica, "POR NO SER UN PAGARE DOMICILIADO. "));
        renglon += 2;
        Pie.Add(horPadding + 450, tamCodigo + altoRenglon * renglon++, new RepString(fPropChica, "ACEPTO(AMOS)"));
        //Agregamos linea para firmar
        renglon -= 1.7;
        Pie.Add(posPanelTotales + horPadding, tamCodigo + altoRenglon * renglon, new RepLine(new PenProp(PDF, grosorPen, ColorT), Pie.rWidth - posPanelTotales - horPadding * 2, 0));
    ...
    private StaticContainer fnCrearEncabezado(System.Drawing.Color sColor)
    {
        ...
        fnDatosReceptor(nsmComprobante, navEncabezado, Encabezado, sColor);
    ...
    private void fnDatosEmisor(XmlNamespaceManager nsmComprobante, XPathNavigator navEncabezado, StaticContainer Encabezado, System.Drawing.Color sColor)
    {
        ...
        //Modificacion a la plantilla hecha por David Elizalde el 15 de Mayo del 2014, se agrego el Regimen Fiscal debajo del RFC del emisor
        ...
        Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignCenter, posRenglon - 9, RepObj.rAlignTop, new RepString(fPropTitulo, "ACEROS Y EQUIPOS MINEROS"));
        Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignCenter, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, "FELICIANO ARREOLA MAGAÑA"));
        fPropNormal.rSize = 5;
        //salto de renglon 
        dposRen += dcalRen;
        int nTamRenglon = 80;
        fnAgregarMultilineaCentrada(Encabezado, "C.Granjeles No.21 Col. Inf. Praderas Hgo. del Parral, Chihu.", fPropNormal, leftPadding + dPX, posRenglon + tamRenglon * dposRen, nTamRenglon);
        //salto de renglon 
        dposRen += dcalRen;
        fnAgregarMultilineaCentrada(Encabezado, "(Tel 01 473 73 3 35 57 Guanajuato,Gto.)", fPropNormal, leftPadding + dPX, posRenglon + tamRenglon * dposRen, nTamRenglon);
        //salto de renglon 
        dposRen += dcalRen;
        fnAgregarMultilineaCentrada(Encabezado, "R.F.C. AEMF631020848 CURP AEMF631020HMNRGL05 ", fPropNormal, leftPadding + dPX, posRenglon + tamRenglon * dposRen, nTamRenglon);
        //Salto de renglon
        dposRen += dcalRen;
        fnAgregarMultilineaCentrada(Encabezado, sRegimenFiscal, fPropNormal, leftPadding + dPX, posRenglon + tamRenglon * dposRen, nTamRenglon);
        ...
        double dvalor = 4;
        fnAgregarMultilineaCentrada(Encabezado, TipoDocumento, fPropNegrita, posX + fAnchoPanel / 6, fAltoPanel / dvalor, 15);
        ...
        {
            Encabezado.AddAligned(posX, RepObj.rAlignRight,
                fAltoPanel * 2 - dPosY, RepObj.rAlignTop,
    ...
    private void fnDatosReceptor(XmlNamespaceManager nsmComprobante, XPathNavigator navEncabezado, StaticContainer Encabezado, System.Drawing.Color sColor)
    {
        ...
        //Agregamos los paneles para datos de Receptor
        double fAltoPanel = Encabezado.rHeight / 6;
        double fAnchoPanel = Encabezado.rWidth / 5;
        double dpnlPosY = 84;
        double dpnlalto = 14.5;
        double dAnchpnl = 457.5;//475//340
        //fnCrearPanelRedondeado(Encabezado, 0, Encabezado.rHeight - dpnlPosY, fAnchoPanel + dAnchpnl, fAltoPanel + dpnlalto, grosorPen, radioCurva, false, sColor);
        dpnlPosY -= 43;
        fnCrearPanelRedondeado(Encabezado, 0, Encabezado.rHeight - dpnlPosY, fAnchoPanel + dAnchpnl, fAltoPanel, grosorPen, radioCurva, false, sColor);
        ...
        Encabezado.AddAligned(leftPadding, RepObj.rAlignLeft, posRazon + tamRenglon * Renglon, RepObj.rAlignTop, new RepString(fPropNegrita, "NOMBRE "));
        //Encabezado.AddAligned(leftPadding + 50, RepObj.rAlignLeft, posRazon + tamRenglon * Renglon, RepObj.rAlignTop, new RepString(fPropNormal, razonSocial));
        Renglon += 0.5;
        int lineaNom = fnAgregarMultilinea(Encabezado, razonSocial, fPropNormal, leftPadding + 50, posRazon + tamRenglon * Renglon, 50, false);

        Renglon -= 0.5;
        Encabezado.AddAligned(leftPadding + posX, RepObj.rAlignLeft, posRazon + tamRenglon * Renglon, RepObj.rAlignTop, new RepString(fPropNegrita, "R.F.C. "));
        Encabezado.AddAligned(leftPadding + posX + 80, RepObj.rAlignLeft, posRazon + tamRenglon * Renglon, RepObj.rAlignTop, new RepString(fPropNormal, rfc));
        ...
        //salto de linea
        Renglon += 1;
        Encabezado.AddAligned(leftPadding, RepObj.rAlignLeft, posRazon + tamRenglon * Renglon, RepObj.rAlignTop, new RepString(fPropNegrita, "DIRECCIÓN "));
        //salto de linea
        Renglon += 0.5;
        //Encabezado.AddAligned(leftPadding + 50, RepObj.rAlignLeft, posRazon + tamRenglon * Renglon, RepObj.rAlignTop, new RepString(fPropNormal, "DIRECC. PRIVADA GERMANIO #100 CIUDAD INDUSTRIAL 2 C.P 34229"));
        //Agregamos un renglon multilinea
        int linea = fnAgregarMultilinea(Encabezado, direccion + " " + codigoPostal, fPropNormal, leftPadding + 50, posRazon + tamRenglon * Renglon, 50, false);
        //if (linea > 1)
        Renglon -= 0.5;
        Encabezado.AddAligned(leftPadding + posX, RepObj.rAlignLeft, posRazon + tamRenglon * Renglon, RepObj.rAlignTop, new RepString(fPropNegrita, "ORDEN DE COMPRA "));
        Encabezado.AddAligned(leftPadding + posX + 80, RepObj.rAlignLeft, posRazon + tamRenglon * Renglon, RepObj.rAlignTop, new RepString(fPropNormal, sNumOrden));
        //salto de linea
        if (linea <= 1)
            Renglon -= 0.8;
        Renglon += 1.8;
        Encabezado.AddAligned(leftPadding, RepObj.rAlignLeft, posRazon + tamRenglon * Renglon, RepObj.rAlignTop, new RepString(fPropNegrita, "CUIDAD "));
        Encabezado.AddAligned(leftPadding + 50, RepObj.rAlignLeft, posRazon + tamRenglon * Renglon, RepObj.rAlignTop, new RepString(fPropNormal, ubicacion));

        Encabezado.AddAligned(leftPadding + posX, RepObj.rAlignLeft, posRazon + tamRenglon * Renglon, RepObj.rAlignTop, new RepString(fPropNegrita, "CONDICIONES "));

        if (sCondPago.ToUpper() == "CRÉDITO")
        {
            Encabezado.AddAligned(leftPadding + posX + 80, RepObj.rAlignLeft, posRazon + tamRenglon * Renglon, RepObj.rAlignTop, new RepString(fPropNormal, "CRÉDITO " + sDiasCredito + " DÍAS"));
        }
        else if (sCondPago.ToUpper() == "CONTADO")
        {
            Encabezado.AddAligned(leftPadding + posX + 80, RepObj.rAlignLeft, posRazon + tamRenglon * Renglon, RepObj.rAlignTop, new RepString(fPropNormal, "CONTADO"));
        }
        ...
        double dDistX = 140;
        posX = 50;
        Encabezado.AddAligned(leftPadding + posX, RepObj.rAlignLeft, posRazon + tamRenglon * Renglon, RepObj.rAlignTop, new RepString(fPropNegrita, "CONDUCTO "));
        posX += dDistX;
        Encabezado.AddAligned(leftPadding + posX, RepObj.rAlignLeft, posRazon + tamRenglon * Renglon, RepObj.rAlignTop, new RepString(fPropNegrita, "No.DE GUÍA "));
        posX += dDistX;
        Encabezado.AddAligned(leftPadding + posX, RepObj.rAlignLeft, posRazon + tamRenglon * Renglon, RepObj.rAlignTop, new RepString(fPropNegrita, "T.CAMBIO"));
        posX += dDistX;
        Encabezado.AddAligned(leftPadding + posX, RepObj.rAlignLeft, posRazon + tamRenglon * Renglon, RepObj.rAlignTop, new RepString(fPropNegrita, "REQUISICIÓN"));
    ...
    private void fnAgregarBordeRedondeado(StaticContainer poObjeto, double pfGrosor, double pfRadioCurva, System.Drawing.Color sColor)
    {
        ...
        double dLugarY = -27;
        double dTamLinea = -7.3;
        double dLugarX = dTamLinea + 3.3;
        ...
        double dPosX = 410;
        double dPosY = 15;
        poObjeto.Add(poObjeto.rWidth - dPosX, pfRadioCurva - dPosY, new RepLine(pen, 0, pfRadioCurva * dTam));
        dPosX = 280;
        poObjeto.Add(poObjeto.rWidth - dPosX, pfRadioCurva - dPosY, new RepLine(pen, 0, pfRadioCurva * dTam));
        dPosX = 155;
        poObjeto.Add(poObjeto.rWidth - dPosX, pfRadioCurva - dPosY, new RepLine(pen, 0, pfRadioCurva * dTam));
    ...
    /// </summary>
    /// <returns></returns>
    private RepImage fnMarcaAguaAcerosyEquiposMineros()
    {
        MemoryStream ms = new MemoryStream();
        try
        {
            string fichero = HttpContext.Current.Server.MapPath("~/LogosPlantillas/MarcaDeAguaAcerosyEquipoMineros.jpg");
    ...
    /// <returns></returns>
    private MemoryStream fnImagenClienteCarp(string sNombreLogo)
    {
        MemoryStream ms = new MemoryStream();
...
public class clsErrorLogAcerosyEquiposMineros
{
    //private static clsInicioSesionUsuario gUsuario;

    public clsErrorLogAcerosyEquiposMineros()
    {
    }
...
public class clsComunLogo
{
    public static string ObtenerParamentro(string sParametro)
    {
        ...
        using (SqlConnection con = new SqlConnection(PAXCrypto.CryptoAES.DesencriptaAES64(ConfigurationManager.ConnectionStrings["conControl"].ConnectionString)))
        {
            ...
            {
                con.Close();
                clsErrorLogAcerosyEquiposMineros.fnNuevaEntrada(ex, clsErrorLogAcerosyEquiposMineros.TipoErroresLog.Conexion);
            }
        }
        return sRetorno;
    }
