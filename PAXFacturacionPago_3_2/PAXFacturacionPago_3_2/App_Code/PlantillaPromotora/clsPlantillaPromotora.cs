...
/// </summary>
public class clsPlantillaPromotora
{
    #region atributos
    ...
    public clsPlantillaPromotora(XmlDocument pxComprobante)
	{
	...
	public void fnGenerarPDF(int id_contribuyente,int sid_rfc,string scolor)
	{
		...
		while (bSeguir)
		{
            ...
            //Se cargan las imagenes del encabezado
            ms =fnImagenClienteEncabezado("LogoGobierno.jpg");
            if (ms.Length > 0)
            {
                double dAlt = 41;
                double dAnc = 110;
            ...
            ms = fnImagenClienteEncabezado("LogoPromotora.jpg");
            if (ms.Length > 0)
            {
                double dAlt = 41;
                double dAnc = 150;
	...
	private StaticContainer fnCrearDetalle(List<DetalleLogo> detalles, System.Drawing.Color sColor)
	{
		...
		//definimos las posiciones
        double posColumna1 = areaDetalle.rWidth * 0.01;     //clave
        double posColumna2 = areaDetalle.rWidth * 0.140;    //unidad
        double posColumna3 = areaDetalle.rWidth * 0.33;     //Cantidad  
        double posColumna4 = areaDetalle.rWidth * 0.40;     //Descripcion 300
        double posColumna5 = areaDetalle.rWidth * 0.82;     //precio Unitario  430
        ...
        for (int i = 0; renglonActual <= maxRenglones; i++)
		{
            ...
            Col1 = fnAgregarMultilinea(areaDetalle, d.noIdentificacion, fPropNormal, posColumna1, posRenglon, 9, true);
            Col2 = fnAgregarMultilinea(areaDetalle, d.unidad, fPropNormal, posColumna2, posRenglon, 9, true);
			areaDetalle.AddAligned(posColumna3, RepObj.rAlignRight, posRenglon, RepObj.rAlignBottom, new RepString(fPropNormal, d.cantidad));
	...
	private void fnTotales(XmlNamespaceManager nsmComprobante, XPathNavigator navPie, StaticContainer Pie, int nNumPag, int nTotPag, string sColor)
	{
        ...
        double posPanelTotales = Pie.rWidth - 169;
        if (nNumPag == nTotPag)//Si es ultima pagina
        {
	...
	private void fnDibujarTitulosDetalle(StaticContainer Encabezado, System.Drawing.Color sColor)
	{
		//Dibujamos el área de los titulos
            ...
            double posColumna3 = Encabezado.rWidth * 0.28;     //Cantidad  160
            double posColumna4 = Encabezado.rWidth * 0.48;     //Descripcion 300
            double posColumna5 = Encabezado.rWidth * 0.72;     //precio Unitario  430
            ...
            Encabezado.AddAligned(posColumna3, RepObj.rAlignLeft, puntoMedio, RepObj.rAlignCenter, new RepString(fPropBlanca, "CANTIDAD"));
            Encabezado.AddAligned(posColumna4, RepObj.rAlignLeft, puntoMedio, RepObj.rAlignCenter, new RepString(fPropBlanca, "DESCRIPCION"));
            Encabezado.AddAligned(posColumna5, RepObj.rAlignLeft, puntoMedio, RepObj.rAlignCenter, new RepString(fPropBlanca, "PRECIO UNITARIO"));
    ...
    private void fnDatosEmisor(XmlNamespaceManager nsmComprobante, XPathNavigator navEncabezado, StaticContainer Encabezado, System.Drawing.Color sColor)
	{
        ...
        Encabezado.AddAligned(leftPadding , RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, "Don Quijote de la Mancha No. 1 Complejo Industrial Chihuahua"));
        Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignRight, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, "Delegacion Cd.Juárez Chih."));
        sep = 13;
        tamRenglon = fPropNormal.rSize + sep;
        //salto de renglon 
        dposRen += dcalRen;
        Encabezado.AddAligned(leftPadding, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, rfc));// "31109 Chihuahua,Chih., México"));
        Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignRight, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, "Boulevard Oscar Flores No.8960"));
        dcalRen = 0.5;
        //salto de renglon 
        dposRen += dcalRen;
        Encabezado.AddAligned(leftPadding, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, "31109 Chihuahua,Chih., México"));// "Tels. +52 (614) 442-3300 Fax +52 (614) 442-3397"));
        Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignRight, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, "+ 52(656) 633-00-76"));
        //salto de renglon 
        dposRen += dcalRen;
        Encabezado.AddAligned(leftPadding, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, "Tels. + 52 (614) 442-3300 Fax + 52 (614) 442-3397")); //"442-3360"));
        Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignRight, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, "Cd. Juarez, Chih."));
        //salto de renglon 
        dposRen += dcalRen;
        dPX = 51;
        Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, "442-3360"));// "442-3398"));
        dPX = 130;
        Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, "442-3398"));
        //salto de renglon 
        dposRen += dcalRen;
        ...
        //Paneles alineados a la Derecha
        double dAncho = 55;
        double pfPosX = 169.5;
        fnCrearPanelRedondeado(Encabezado, Encabezado.rWidth - pfPosX, 0, fAnchoPanel + dAncho, fAltoPanel, grosorPen, radioCurva, false, sColor);
        double dAlto = 110;
        fnCrearPanelRedondeado(Encabezado, Encabezado.rWidth - pfPosX,fAltoPanel, fAnchoPanel + dAncho, fAltoPanel + dAlto, grosorPen, radioCurva, false, sColor);

        //Panel de Receptor
        //double dGrosor = 36;
        double pfPosY = 109.8;
        double pfAncho = 288;//267.8;
        double dpfPosX = 572;
        fnCrearPanelRedondeado(Encabezado, Encabezado.rWidth - dpfPosX , fAltoPanel + Encabezado.rHeight - pfPosY, fAnchoPanel + pfAncho, fAltoPanel + 36, grosorPen, radioCurva, false, sColor);
    ...
    private void fnDatosReceptor(XmlNamespaceManager nsmComprobante, XPathNavigator navEncabezado, StaticContainer Encabezado, double Renglon)
	{
        ...
        i = fnAgregarMultilinea(Encabezado, "CLIENTE", fPropNormal, leftPadding, posRenglon + tamRenglon * Renglon++, 50, true);
        Renglon += i;
        if (i > 1)
            Renglon -= 1.5;
        else
            Renglon -= 1;
        i = fnAgregarMultilinea(Encabezado, razonSocial, fPropNormal, leftPadding, posRenglon + tamRenglon * Renglon++, 50, true);
        Renglon += i;
        if (i > 1)
            Renglon -= 1.5;
        else
            Renglon -= 1;
        i = fnAgregarMultilineaDir(Encabezado, direccion, fPropNormal, leftPadding, posRenglon + tamRenglon * Renglon, 40, true);
        Renglon += i;
        if (i > 1)
            Renglon -= 0.7;
        ...
        fPropNormal.rSize = 4;
        double pfPosX = 250;
        Encabezado.AddAligned(leftPadding + pfPosX, RepObj.rAlignRight, posRenglon + tamRenglon * Renglon++, RepObj.rAlignTop, new RepString(fPropNormal, "Lugar de expedición:"));
        Encabezado.AddAligned(leftPadding + pfPosX, RepObj.rAlignRight, posRenglon + tamRenglon * Renglon++, RepObj.rAlignTop, new RepString(fPropNormal, "No. Cliente"));
        Encabezado.AddAligned(leftPadding + pfPosX, RepObj.rAlignRight, posRenglon + tamRenglon * Renglon++, RepObj.rAlignTop, new RepString(fPropNormal, "Manzana."));
        Encabezado.AddAligned(leftPadding + pfPosX, RepObj.rAlignRight, posRenglon + tamRenglon * Renglon++, RepObj.rAlignTop, new RepString(fPropNormal, "Lote:"));
        Encabezado.AddAligned(leftPadding + pfPosX, RepObj.rAlignRight, posRenglon + tamRenglon * Renglon++, RepObj.rAlignTop, new RepString(fPropNormal, "Parque Ind."));
        pfPosX = 255;
        Renglon = 9;
        Encabezado.AddAligned(leftPadding + pfPosX, RepObj.rAlignLeft, posRenglon + tamRenglon * Renglon++, RepObj.rAlignTop, new RepString(fPropNormal, "Chihuahua, Chih"));
        Encabezado.AddAligned(leftPadding + pfPosX, RepObj.rAlignLeft, posRenglon + tamRenglon * Renglon++, RepObj.rAlignTop, new RepString(fPropNormal, "11521"));
        Encabezado.AddAligned(leftPadding + pfPosX, RepObj.rAlignLeft, posRenglon + tamRenglon * Renglon++, RepObj.rAlignTop, new RepString(fPropNormal, "15"));
        Encabezado.AddAligned(leftPadding + pfPosX, RepObj.rAlignLeft, posRenglon + tamRenglon * Renglon++, RepObj.rAlignTop, new RepString(fPropNormal, "247"));
        double dvalor = 3.8;
        fnAgregarMultilinea(Encabezado,"Complejo Industrial Chihuahua", fPropNormal, leftPadding + pfPosX, posRenglon + tamRenglon * Renglon + dvalor , 40, true);
        fPropNormal.rSize = 5;
    ...
    private double fnAgregarMultilineaDir(StaticContainer pContenedor, string psTexto, FontProp pFuente, double pdPosX, double pdPosY, int pnTamRenglon, bool bBuscarEspacio)
    {
        double nAlturaRenglon = pFuente.rSize * 1.2;
        double nNoRenglon = 0;
    ...
    private MemoryStream fnImagenClienteEncabezado(string sNombreLogo)
    {
        MemoryStream ms = new MemoryStream();
...
public class clsErrorLogPromotora
{
    //private static clsInicioSesionUsuario gUsuario;

    public clsErrorLogPromotora()
    {
    }
...
public class clsComunLogo
{
    public static string ObtenerParamentro(string sParametro)
    {
        ...
        using (SqlConnection con = new SqlConnection(PAXCrypto.CryptoAES.DesencriptaAES64(ConfigurationManager.ConnectionStrings["conControl"].ConnectionString)))
        {
            ...
            {
                con.Close();
                clsErrorLogPromotora.fnNuevaEntrada(ex, clsErrorLogPromotora.TipoErroresLog.Conexion);
            }
        }
        return sRetorno;
    }
