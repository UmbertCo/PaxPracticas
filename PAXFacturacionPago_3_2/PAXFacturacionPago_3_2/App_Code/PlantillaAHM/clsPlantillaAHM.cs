...
public class clsPlantillaAHM
{
    ...
    public clsPlantillaAHM(XmlDocument pxComprobante)
	{
	...
	public void fnGenerarPDF(int id_contribuyente,int sid_rfc,string scolor)
	{
        ...
        List<DetalleAHM> detallesNum = fnObtenerDetalles();
		...
		List<DetalleAHM> detalles = fnObtenerDetalles();
    ...
    /// <returns></returns>
    private StaticContainer fnContarPaginas(List<DetalleAHM> detalles)
    {
        ...
        DetalleAHM[] copiaDetalles = detalles.ToArray();
        DetalleAHM d;
        double posRenglon;
            ...
            if (d.aduana != null || !String.IsNullOrEmpty(d.RfcT))
            {
                ...
                if (!String.IsNullOrEmpty(d.RfcT))
                {
                    ...
                    if ((d.RetencionesT != null && d.RetencionesT.Count > 0) || (d.TrasladosT != null && d.TrasladosT.Count > 0)) //Impuestos tercero
                    {
                        ...
                        {
                            foreach (ComprobanteImpuestosRetencionTAHM retencion in d.RetencionesT)
                            {
                                switch (retencion.impuesto)
                                {
                                    case ComprobanteImpuestosRetencionImpuestoTAHM.IVA: sDetalleTerceros.Append("IVA Ret: "); break;
                                    case ComprobanteImpuestosRetencionImpuestoTAHM.ISR: sDetalleTerceros.Append("ISR: "); break;
                                }
                        ...
                        {
                            foreach (ComprobanteImpuestosTrasladoTAHM traslado in d.TrasladosT)
                            {
                                switch (traslado.impuesto)
                                {
                                    case ComprobanteImpuestosTrasladoImpuestoTAHM.IVA: sDetalleTerceros.Append("IVA Tras "); break;
                                    case ComprobanteImpuestosTrasladoImpuestoTAHM.IEPS: sDetalleTerceros.Append("IEPS "); break;
                                }
	...
	/// <returns></returns>
	private StaticContainer fnCrearDetalle(List<DetalleAHM> detalles, System.Drawing.Color sColor)
	{
		StaticContainer areaDetalle = new StaticContainer(RT.rPointFromMM(anchoPagina) - margenPagina * 2, RT.rPointFromMM(altoPagina - altoEncabezado - altoPie) - margenPagina * 2);
	...
	private List<DetalleAHM> fnObtenerDetalles()
	{
		...
		{
            detalles.Add(new DetalleAHM(navDetalles.Current, nsmComprobante));
            ...
            if (nodenavigator.HasChildren)//Si contiene nodo hijo
            {
                ...
                //    detalles.Add(new DetalleAHM(navDetallesAduana.Current, nsmComprobante));
                ...
                if (navDetallesAduana != null)
                    detalles.Add(new DetalleAHM(navDetallesAduana, nsmComprobante));
                ...
                if (navComplTerceros != null)
                {
                    ...
                    if (navTerceros != null)
                        detalles.Add(new DetalleAHM(navTerceros, nsmComprobanteComplTerceros));
                }
            }
		}
	...
	private void fnTotales(XmlNamespaceManager nsmComprobante, XPathNavigator navPie, StaticContainer Pie, int nNumPag, int nTotPag, string sColor)
	{
		...
		List<ImpuestoAHM> impuestos = new List<ImpuestoAHM>();
        ...
        if (nNumPag == nTotPag)//Si es ultima pagina
        {
            ...
            {
                impuestos.Add(new ImpuestoAHM(navImpuestos.Current, nsmComprobante));
            }
        ...
        if (nNumPag == nTotPag)//Si es ultima pagina
        {
            ...
            Pie.AddAligned(Pie.rWidth - horPadding, RepObj.rAlignRight, verPadding, RepObj.rAlignTop, new RepString(fPropNormal, TransformacionAHM.fnFormatoCurrency(subtotal)));
        }
        ...
        if (nNumPag == nTotPag)//Si es ultima pagina
        {
            foreach (ImpuestoAHM i in impuestos)
            {
                if (i.Nombre != "IEPS")
                {
                    Pie.AddAligned(horPadding + posPanelTotales, RepObj.rAlignLeft, verPadding + altoRenglon * renglon, RepObj.rAlignTop, new RepString(fPropNormal, i.Nombre + " " + TransformacionAHM.fnFormatoPorcentaje(i.Tasa)));
                    Pie.AddAligned(Pie.rWidth - horPadding, RepObj.rAlignRight, verPadding + altoRenglon * renglon, RepObj.rAlignTop, new RepString(fPropNormal, TransformacionAHM.fnFormatoCurrency(i.Importe)));
                    renglon++;
                }
            }
        ...
        Pie.Add(posPanelTotales + horPadding, tamCodigo - altoRenglon - verPadding, new RepLine(new PenProp(PDF, grosorPen, ColorT), Pie.rWidth - posPanelTotales - horPadding * 2, 0));
            ...
            Pie.AddAligned(Pie.rWidth - horPadding, RepObj.rAlignRight, tamCodigo - altoRenglon, RepObj.rAlignTop, new RepString(fPropNormal, TransformacionAHM.fnFormatoCurrency(total)));
        }
        ...
        if (nNumPag == nTotPag)//Si es ultima pagina
        {
            ...
            fnAgregarMultilinea(Pie, fnTextoImporte(TransformacionAHM.fnFormatoRedondeo(total), moneda), fPropChica, tamCodigo + horPadding, altoRenglon * 1.7, 80, true);
        ...
        if (nNumPag == nTotPag)//Si es ultima pagina
        {
            ...
            renglon = fnAgregarMultilinea(Pie, TransformacionAHM.fnConstruirCadenaTimbrado(gxComprobante.CreateNavigator().SelectSingleNode("/cfdi:Comprobante/cfdi:Complemento/tfd:TimbreFiscalDigital", nsmComprobante).CreateNavigator(), "cadenaoriginal_TFD_1_0")
        ...
        Pie.AddAligned(Pie.rWidth / 7, RepObj.rAlignCenter, Pie.rHeight - verPadding, RepObj.rAlignTop, new RepString(fPropNormal, clsComunAHM.ObtenerParamentro("urlHostComercial")));
	...
	private string fnTextoImporte(string psValor, string psMoneda)
	{
		...
		NumaletAHM parser = new NumaletAHM();
        parser.LetraCapital = true;

		switch (psMoneda)
		{
			case "MXN":
				parser.TipoMoneda = NumaletAHM.Moneda.Peso;
				break;
			case "USD":
				parser.TipoMoneda = NumaletAHM.Moneda.Dolar;
				break;
			case "XEU":
				parser.TipoMoneda = NumaletAHM.Moneda.Euro;
				break;
		}
    ...
    private RepImage fnMarcaAguaCorpus()
    {
        ...
        {
            clsErrorLogAHM.fnNuevaEntrada(ex, clsErrorLogAHM.TipoErroresLog.Referencia);
        }
...
public class ImpuestoAHM
{
	...
	public string TextoImpuesto
	{
		get
		{
			return Nombre + " " + TransformacionAHM.fnFormatoPorcentaje(Tasa) + " " + TransformacionAHM.fnFormatoCurrency(Importe);
		}
	}
	...
	public ImpuestoAHM(XPathNavigator navImpuesto, XmlNamespaceManager nsmComprobante)
	{
...
public class DetalleAHM
{
    ...
    private List<ComprobanteImpuestosRetencionTAHM> _retencionesT;
    public List<ComprobanteImpuestosRetencionTAHM> RetencionesT
    {
        get { return _retencionesT; }
        set { _retencionesT = value; }
    }

    private List<ComprobanteImpuestosTrasladoTAHM> _trasladosT;
    public List<ComprobanteImpuestosTrasladoTAHM> TrasladosT
    {
        get { return _trasladosT; }
        set { _trasladosT = value; }
    }

    private t_UbicacionFiscalTAHM _ubicacionFiscalT;
    public t_UbicacionFiscalTAHM UbicacionFiscalT
    {
        get { return _ubicacionFiscalT; }
        set { _ubicacionFiscalT = value; }
    }

    private t_InformacionAduaneraTAHM _informacionAduaneraT;
    public t_InformacionAduaneraTAHM InformacionAduaneraT
    {
	...
	/// <param name="nsmComprobante">Manejador de nombres de espacio</param>
	public DetalleAHM(XPathNavigator navDetalle, XmlNamespaceManager nsmComprobante)
	{
        try
        {
        ...
        try
        {
            List<ComprobanteImpuestosRetencionTAHM> impuestosRetencion = new List<ComprobanteImpuestosRetencionTAHM>();
            List<ComprobanteImpuestosTrasladoTAHM> impuestosTraslado = new List<ComprobanteImpuestosTrasladoTAHM>();
            ...
            {
                ComprobanteImpuestosRetencionTAHM impRet = new ComprobanteImpuestosRetencionTAHM();
                ...
                try
                {
                    ...
                    if (!String.IsNullOrEmpty(sImpuestoRet))
                    {
                        ...
                        {
                            case "IVA": impRet.impuesto = ComprobanteImpuestosRetencionImpuestoTAHM.IVA; break;
                            case "ISR": impRet.impuesto = ComprobanteImpuestosRetencionImpuestoTAHM.ISR; break;
                        }
                    }
            ...
            {
                ComprobanteImpuestosTrasladoTAHM impTras = new ComprobanteImpuestosTrasladoTAHM();
                ...
                try
                {
                    ...
                    if (!String.IsNullOrEmpty(sImpuestoTras))
                    {
                        ...
                        {
                            case "IVA": impTras.impuesto = ComprobanteImpuestosTrasladoImpuestoTAHM.IVA; break;
                            case "IEPS": impTras.impuesto = ComprobanteImpuestosTrasladoImpuestoTAHM.IEPS; break;
                        }
                    }
            ...
            try
            {
                t_UbicacionFiscalTAHM ubicacionTerceros = new t_UbicacionFiscalTAHM();
            ...
            try
            {
                t_InformacionAduaneraTAHM aduaneraTerceros = new t_InformacionAduaneraTAHM();
...
public partial class ComprobanteImpuestosRetencionTAHM
{
    private ComprobanteImpuestosRetencionImpuestoTAHM impuestoField;
    private decimal importeField;

    public ComprobanteImpuestosRetencionImpuestoTAHM impuesto
    {
...
public enum ComprobanteImpuestosRetencionImpuestoTAHM
{
    ISR, IVA,
}

public partial class ComprobanteImpuestosTrasladoTAHM
{
    private ComprobanteImpuestosTrasladoImpuestoTAHM impuestoField;
    private decimal tasaField;
    private decimal importeField;

    public ComprobanteImpuestosTrasladoImpuestoTAHM impuesto
    {
        get { return this.impuestoField; }
        set { this.impuestoField = value; }
    }
...
public enum ComprobanteImpuestosTrasladoImpuestoTAHM
{
    IVA, IEPS,
}

public partial class t_UbicacionFiscalTAHM
{
...
public partial class t_InformacionAduaneraTAHM
{
...
/// </summary>
public class TransformacionAHM
{
...
public class clsErrorLogAHM
{
    //private static clsInicioSesionUsuario gUsuario;

    public clsErrorLogAHM()
    {
    }
...
public class clsComunAHM
{
    public static string ObtenerParamentro(string sParametro)
    {
        ...
        using (SqlConnection con = new SqlConnection(PAXCrypto.CryptoAES.DesencriptaAES64(ConfigurationManager.ConnectionStrings["conControl"].ConnectionString)))
        {
            ...
            {
                con.Close();
                clsErrorLogAHM.fnNuevaEntrada(ex, clsErrorLogAHM.TipoErroresLog.Conexion);
            }
        }
        return sRetorno;
    }
...
public sealed class NumaletAHM
{
    ...
    public NumaletAHM(Boolean ConvertirDecimales, String MascaraSalidaDecimal, String SeparadorDecimalSalida, Boolean LetraCapital)
    {
    ...
    {
        return NumaletAHM.ToString(Numero, CultureInfo.CurrentCulture);
    }
