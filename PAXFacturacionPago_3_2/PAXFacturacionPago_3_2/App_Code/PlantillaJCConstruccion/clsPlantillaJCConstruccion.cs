...
/// </summary>
public class clsPlantillaJCConstruccion
{
    #region atributos
    ...
    public clsPlantillaJCConstruccion(XmlDocument pxComprobante)
	{
	...
	public void fnGenerarPDF(int id_contribuyente,int sid_rfc,string scolor)
	{
		...
		while (bSeguir)
		{
            ...
            ColorT = Color.Navy; // (System.Drawing.Color)colConvert.ConvertFromString(scolor);
            ...
            if (ms.Length > 0)
            {
                double rW = 158;
                double rH = 127;
                RepImage image = new RepImage(ms, rW, rH);
                double spY = 131;
                pagina.Add(margenPagina + 12, margenPagina + spY, image);
            }

            /*Agregado 17/09/2013 Marca de Agua JC Construccion*/
            double drX = 150;
            double drY = 515;
            pagina.Add(margenPagina + drX, margenPagina + drY, fnMarcaAguaJCConstruccion());
            /*Fin Agragado*/
	...
	private void fnTotales(XmlNamespaceManager nsmComprobante, XPathNavigator navPie, StaticContainer Pie, int nNumPag, int nTotPag, string sColor)
	{
        ...
        if (nNumPag == nTotPag)//Si es ultima pagina
        {
            if (descuento != string.Empty)
            {
                ...
                Pie.AddAligned(Pie.rWidth - horPadding, RepObj.rAlignRight, verPadding + altoRenglon * renglon, RepObj.rAlignTop, new RepString(fPropNormal, TransformacionLogo.fnFormatoCurrency(descuento)));
                renglon++;
            }
    ...
    private void fnDatosEmisor(XmlNamespaceManager nsmComprobante, XPathNavigator navEncabezado, StaticContainer Encabezado, System.Drawing.Color sColor, ref double Renglon)
    {
        ...
        double posRenglon = fPropTitulo.rSize + 4;
        ...
        double distanciaX = 260;
        //double Renglon = 0;
        double i = 0;

        i = fnAgregarMultilineaCentrada(Encabezado, "I.C JORGE ISIDORO CONCHA TORRES" /*razonSocial*/, fPropTitulo, leftPadding + distanciaX, posRenglon, 46);
        distanciaX = 177;
        Renglon += i;
        if (i > 1)
            Renglon -= 0.5;
        //if (exCalle != string.Empty)
        //{
            //i = fnAgregarMultilinea(Encabezado, "RFC: COTJ600404695"/*rfc*/, fPropNormal, leftPadding + distanciaX, posRenglon + tamRenglon * Renglon, 46, true);
            //Renglon += i;
            //if (i > 1)
            //    Renglon -= 0.5;
            //i = fnAgregarMultilinea(Encabezado, "EXPEDIDO EN", fPropNormal, leftPadding + distanciaX, posRenglon + tamRenglon * Renglon, 46, true);
            //Renglon += i;
            //if (i > 1)
            //    Renglon -= 0.5;
            //i = fnAgregarMultilinea(Encabezado, exDireccion, fPropNormal, leftPadding + distanciaX, posRenglon + tamRenglon * Renglon, 46, true);
            //Renglon += i;
            //if (i > 1)
            //    Renglon -= 0.5;
            //i = fnAgregarMultilinea(Encabezado, exUbicacion, fPropNormal, leftPadding + distanciaX, posRenglon + tamRenglon * Renglon, 46, true);
            //Renglon += i;
            //if (i > 1)
            //    Renglon -= 0.5;
            //i = fnAgregarMultilinea(Encabezado, exUbicacion2, fPropNormal, leftPadding + distanciaX, posRenglon + tamRenglon * Renglon, 46, true);
            //Renglon += i;
            //if (i > 1)
            //    Renglon -= 0.5;
        ...
        //else
        //{
            i = fnAgregarMultilinea(Encabezado, "RFC: COTJ600404695", fPropNormal, leftPadding + distanciaX, posRenglon + tamRenglon * Renglon, 46, true);
            Renglon += i;
            if (i > 1)
                Renglon -= 0.5;

            i = fnAgregarMultilinea(Encabezado, "San Bizancio # 4315" /*ubicacion*/, fPropNormal, leftPadding + distanciaX, posRenglon + tamRenglon * Renglon, 46, true);
            Renglon += i;
            if (i > 1)
                Renglon -= 0.5;
            i = fnAgregarMultilinea(Encabezado, "Fracc. Privanzza San Angel Cp. 31104" /*rfc*/, fPropNormal, leftPadding + distanciaX, posRenglon + tamRenglon * Renglon, 46, true);
            Renglon += i;
            if (i > 1)
                Renglon -= 0.5;
            i = fnAgregarMultilinea(Encabezado, "Chihuahua, Chihuhua", fPropNormal, leftPadding + distanciaX, posRenglon + tamRenglon * Renglon, 46, true);
            Renglon += i;
            if (i > 1)
                Renglon -= 0.5;
            i = fnAgregarMultilinea(Encabezado, "Email. Jorge460@prodigy.net.mx, Jorgeconcha@jcconstruccion.com, www.jcconstruccion.com", fPropNormal, leftPadding + distanciaX, posRenglon + tamRenglon * Renglon, 46, true);
            Renglon += i;
            if (i > 1)
                Renglon -= 0.5;
        //}
        ...
        double fAltoPanel = Encabezado.rHeight / 11.5;
        double fAnchoPanel = Encabezado.rWidth / 5;
        double posX = Encabezado.rWidth - fAnchoPanel;
        
        fnCrearPanelRedondeado(Encabezado, Encabezado.rWidth - 190, 0, fAnchoPanel + 75, fAltoPanel, grosorPen, radioCurva,  true, sColor);
       ...
       // Encabezado.AddAligned(posX + fAnchoPanel / 6, fAltoPanel / 2, 15, RepObj.rAlignLeft, new RepString(fPropNegrita, TipoDocumento));
        fnAgregarMultilineaCentrada(Encabezado, TipoDocumento, fPropBlanca, posX + fAnchoPanel / 6, fAltoPanel / 2, 15);
        ...
        //Agregamos panel para titulo de datos de clientes 
        fnCrearPanelRedondeado(Encabezado, 0, fAltoPanel * 2 + 95, Encabezado.rWidth, fAltoPanel, grosorPen, radioCurva, true, sColor);
        //Titulo de Clientes
        double dmitad = 2.2;
        //fnAgregarMultilinea(Encabezado, "Datos Clientes" + fechaAutorizacion, fPropBlanca, Encabezado.rWidth / dmitad, fAltoPanel * 2 + 108,46, true);
        Encabezado.AddAligned(Encabezado.rWidth / dmitad, RepObj.rAlignCenter, fAltoPanel * 2 + 105, RepObj.rAlignCenter, new RepString(fPropBlanca, "DATOS CLIENTES"));
    ...
    /// </summary>
    /// <returns></returns>
    private RepImage fnMarcaAguaJCConstruccion()
    {
        MemoryStream ms = new MemoryStream();
        try
        {
            string fichero = HttpContext.Current.Server.MapPath("~/LogosPlantillas/MarcaDeAguaJCConstruccion.jpg");
        ...
        double rWidth = 300;
        double rHeight = 260;
        return new RepImage(ms, rWidth, rHeight);
    }
...
public class clsErrorLogJCContruccion
{
    //private static clsInicioSesionUsuario gUsuario;

    public clsErrorLogJCContruccion()
    {
    }
...
public class clsComunLogo
{
    public static string ObtenerParamentro(string sParametro)
    {
        ...
        using (SqlConnection con = new SqlConnection(PAXCrypto.CryptoAES.DesencriptaAES64(ConfigurationManager.ConnectionStrings["conControl"].ConnectionString)))
        {
            ...
            {
                con.Close();
                clsErrorLogJCContruccion.fnNuevaEntrada(ex, clsErrorLogJCContruccion.TipoErroresLog.Conexion);
            }
        }
        return sRetorno;
    }
