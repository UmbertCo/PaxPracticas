...
/// </summary>
public class clsOperacionConsultaPdf
{
    #region atributos
    ...
    private bool bContinuaConceptoPagina = false;
    //Factura
    private string noConcepto = string.Empty;
    ...
    public clsOperacionConsultaPdf(XmlDocument pxComprobante)
    {
    ...
    private StaticContainer fnContarPaginas(List<Detalle> detalles)
    {
        ...
        //int maxRenglones = (int)((areaDetalle.rHeight / (fPropNormal.rSize + 2)));
        int maxRenglones = 34;
        ...
        for (int i = 0; renglonActual <= maxRenglones; i++)
        {
            ...
            if (d.aduana != null || !String.IsNullOrEmpty(d.RfcT))
            {
                ...
                if (!String.IsNullOrEmpty(d.RfcT))
                {
                    ...
                    if ((d.RetencionesT != null && d.RetencionesT.Count > 0) || (d.TrasladosT != null && d.TrasladosT.Count > 0)) //Impuestos tercero
                    {
                        ...
                        {
                            foreach (ComprobanteImpuestosRetencionT retencion in d.RetencionesT)
                            {
                                switch (retencion.impuesto)
                                {
                                    case ComprobanteImpuestosRetencionImpuestoT.IVA: sDetalleTerceros.Append("IVA Ret: "); break;
                                    case ComprobanteImpuestosRetencionImpuestoT.ISR: sDetalleTerceros.Append("ISR: "); break;
                                }
                        ...
                        {
                            foreach (ComprobanteImpuestosTrasladoT traslado in d.TrasladosT)
                            {
                                switch (traslado.impuesto)
                                {
                                    case ComprobanteImpuestosTrasladoImpuestoT.IVA: sDetalleTerceros.Append("IVA Tras "); break;
                                    case ComprobanteImpuestosTrasladoImpuestoT.IEPS: sDetalleTerceros.Append("IEPS "); break;
                                }
            ...
            else
            {
                ...
                if (renglones > maxRenglones - renglonActual)
                {
                    Col3 = fnContarMultilinea(areaDetalle, fnDescripcionTamanioColumna(d.descripcion, 42, renglonActual, maxRenglones), fPropNormal, posColumna3, posRenglon, 42, true);

                    detalles[detalles.IndexOf(d)].descripcion = fnSiguienteDescripcionTamanioColumna(d.descripcion, 42, renglonActual, maxRenglones - renglonActual + 1);
                    if (!detalles[detalles.IndexOf(d)].descripcion.Equals(string.Empty))
                        break;
                }
                else
                {
    ...
    private StaticContainer fnCrearDetalle(List<Detalle> detalles, System.Drawing.Color sColor)
    {
        ...
        for (int i = 0; renglonActual <= maxRenglones; i++)
        {
            ...
            if (d.aduana != null || !String.IsNullOrEmpty(d.RfcT))
            {
                ...
                bContinuaConceptoPagina = false;
            }
            else
            {
                if (bContinuaConceptoPagina)
                {
                    if (renglones > maxRenglones - renglonActual)
                    {
                        Col3 = fnAgregarMultilineaDetalle(areaDetalle, fnDescripcionTamanioColumna(d.descripcion, 42, renglonActual, maxRenglones), fPropNormal, posColumna3, posRenglon, 42, true);
                        detalles[detalles.IndexOf(d)].descripcion = fnSiguienteDescripcionTamanioColumna(d.descripcion, 42, renglonActual, maxRenglones);
                        if (!detalles[detalles.IndexOf(d)].descripcion.Equals(string.Empty))
                        {
                            bContinuaConceptoPagina = true;
                            break;
                        }
                    }
                    else
                    {
                        ...
                        noConcepto = string.Empty;
                        bContinuaConceptoPagina = false;
                    }
                }
                else
                {
                    noConcepto = d.noIdentificacion;
                    ...
                    if (renglones > maxRenglones - renglonActual)
                    {
                        int nRenglonActual = renglonActual;
                        Col3 = fnAgregarMultilineaDetalle(areaDetalle, fnDescripcionTamanioColumna(d.descripcion, 42, renglonActual, maxRenglones), fPropNormal, posColumna3, posRenglon, 42, true);
                        detalles[detalles.IndexOf(d)].descripcion = fnSiguienteDescripcionTamanioColumna(d.descripcion, 42, renglonActual, maxRenglones - nRenglonActual + 1);
                        if (!detalles[detalles.IndexOf(d)].descripcion.Equals(string.Empty))
                        {
                            bContinuaConceptoPagina = true;
                            break;
        ...
        while (navDetalles.MoveNext())
        {
            ...
            if (nodenavigator.HasChildren)//Si contiene nodo hijo
            {
                ...
                if (navDetallesAduana != null)
                    detalles.Add(new Detalle(navDetallesAduana, nsmComprobante));
                ...
                if (navComplTerceros != null)
                {
                    ...
                    if (navTerceros != null)
                        detalles.Add(new Detalle(navTerceros, nsmComprobanteComplTerceros));
                }
            }
        }
    ...
    private void fnTotales(XmlNamespaceManager nsmComprobante, XPathNavigator navPie, StaticContainer Pie, int nNumPag, int nTotPag, string sColor)
    {
        ...
        string subtotal, total, moneda, sello, timbre, formaDePago, metodoPago, Regimenfiscal, version, sNumCtaPago, descuento, motivoDescuento;
        subtotal = total = moneda = sello = timbre = formaDePago = Regimenfiscal = metodoPago = version = sNumCtaPago = descuento = motivoDescuento = string.Empty;
        ...
        if (descuento != string.Empty)
        {
            ...
            Pie.AddAligned(Pie.rWidth - horPadding, RepObj.rAlignRight, verPadding + altoRenglon * renglon, RepObj.rAlignTop, new RepString(fPropNormal, Transformacion.fnFormatoCurrency(descuento)));
            renglon++;
        }
        ...
        if (nNumPag == nTotPag)//Si es ultima pagina
        {
            ////Mostramos Motivo de descuento en caso de que exista abajo del panel para detalles
            //if (descuento != string.Empty)
            //{
            //    double dAltoPosY = 15;
            //    Pie.Add(horPadding, altoRenglon - dAltoPosY, new RepString(fPropNegrita, "Motivo De Descuento: "));
            //    Pie.Add(tamCodigo + horPadding, altoRenglon - dAltoPosY, new RepString(fPropNormal, motivoDescuento));
            //}
        ...
        if (nNumPag == nTotPag)//Si es ultima pagina
        {
            ...
            Pie.Add(horPadding, tamCodigo + altoRenglon * renglon++ - 7, new RepString(fPropNormal, "Sello digital del SAT:"));
            renglon += fnAgregarMultilinea(Pie, timbre, fPropChica, horPadding, tamCodigo + altoRenglon * renglon - 7, 130, false);

            //Agregamos la cadena original y alargamos la sección del pie según sea necesario
            Pie.Add(horPadding, tamCodigo + altoRenglon * renglon++ - 8, new RepString(fPropNormal, "Cadena original del complemento de certificación digital del SAT:"));
            renglon = fnAgregarMultilinea(Pie, "|" + Transformacion.fnConstruirCadenaTimbrado(gxComprobante.CreateNavigator().SelectSingleNode("/cfdi:Comprobante/cfdi:Complemento/tfd:TimbreFiscalDigital", nsmComprobante).CreateNavigator(), "cadenaoriginal_TFD_1_0") + "||"
                , fPropChica, horPadding, tamCodigo + altoRenglon * renglon - 8, 130, false);
    ...
    private void fnDatosEmisor(XmlNamespaceManager nsmComprobante, XPathNavigator navEncabezado, StaticContainer Encabezado, System.Drawing.Color sColor, ref double Renglon)
    {
        ...
        //double Renglon = 0;
        Renglon = fnAgregarMultilinea(Encabezado, razonSocial, fPropTitulo, leftPadding, posRenglon, 50, true);
        if (exCalle != string.Empty)
        {
            Renglon += fnAgregarMultilinea(Encabezado, rfc, fPropNormal, leftPadding, posRenglon + tamRenglon * Renglon, 65, true);
            Renglon += fnAgregarMultilinea(Encabezado, "EXPEDIDO EN :", fPropNegrita, leftPadding, posRenglon + tamRenglon * Renglon, 65, true);
            Renglon += fnAgregarMultilinea(Encabezado, exDireccion, fPropNormal, leftPadding, posRenglon + tamRenglon * Renglon, 65, true);
            Renglon += fnAgregarMultilinea(Encabezado, exUbicacion, fPropNormal, leftPadding, posRenglon + tamRenglon * Renglon, 65, true);
        }
        else
        {
            Renglon += fnAgregarMultilinea(Encabezado, direccion, fPropNormal, leftPadding, posRenglon + tamRenglon * Renglon, 65, true);
            Renglon += fnAgregarMultilinea(Encabezado, ubicacion, fPropNormal, leftPadding, posRenglon + tamRenglon * Renglon, 65, true);
            Renglon += fnAgregarMultilinea(Encabezado, rfc, fPropNormal, leftPadding, posRenglon + tamRenglon * Renglon, 65, true);
        }
        ...
        if (!string.IsNullOrEmpty(referencia))
        {
            ...
            Renglon += fnAgregarMultilinea(Encabezado, Refer, fPropNormal, leftPadding, posRenglon + tamRenglon * Renglon, 65, true);
            //Encabezado.Add(leftPadding, posRenglon + tamRenglon * 4, new RepString(fPropNormal, Refer));

            if (version == "3.2")
            {
                sRegFis = "Regimen Fiscal: " + Regimenfiscal;
                Renglon += fnAgregarMultilinea(Encabezado, sRegFis, fPropNormal, leftPadding, posRenglon + tamRenglon * Renglon, 65, true);
                // Encabezado.Add(leftPadding, posRenglon + tamRenglon * 5, new RepString(fPropNormal, sRegFis));
            }
        }
        else
        {
            if (version == "3.2")
            {
                sRegFis = "Regimen Fiscal: " + Regimenfiscal;
                Renglon += fnAgregarMultilinea(Encabezado, sRegFis, fPropNormal, leftPadding, posRenglon + tamRenglon * Renglon, 65, true);
                // Encabezado.Add(leftPadding, posRenglon + tamRenglon * 4, new RepString(fPropNormal, sRegFis));
            }
        }
    ...
    private void fnDatosReceptor(XmlNamespaceManager nsmComprobante, XPathNavigator navEncabezado, StaticContainer Encabezado, double Renglon)
    {
        ...
        //double Renglon = 0;
        Renglon += 0.5;
        Renglon += fnAgregarMultilinea(Encabezado, razonSocial, fPropTitulo, leftPadding, posRenglon + tamRenglon * Renglon++, 55, true);
        Renglon += fnAgregarMultilinea(Encabezado, direccion, fPropNormal, leftPadding, posRenglon + tamRenglon * Renglon, 75, true);
        Renglon += fnAgregarMultilinea(Encabezado, ubicacion, fPropNormal, leftPadding, posRenglon + tamRenglon * Renglon, 75, true);
        Renglon += fnAgregarMultilinea(Encabezado, rfc, fPropNormal, leftPadding, posRenglon + tamRenglon * Renglon, 75, true);
                ...
                while (nlongitudTexto > 0)
                {
                    ...
                    else //Si no existe saltos de linea en la cadena
                    {
                        ...
                        if (psTexto.Length > pnTamRenglon)
                        {
                            ...
                            if (nUltimoEspacio == -1 || nUltimoEspacio == 0)
                                nUltimoEspacio = pnTamRenglon;
                            renglones.Add(new RepString(pFuente, psTexto.Substring(0, nUltimoEspacio).TrimStart().TrimEnd()));
                        }
                        else
                        {
                            //Si no revasa lo permitido se escribe la linea
                        ...
                        //Se asigna la cadena restante a escribir
                        psTexto = psTexto.Substring(nUltimoEspacio).ToString().TrimStart().TrimEnd();
                    }
    ...
    /// <summary>
    /// Función que se encarga de agregar vueltas de carro cuando la descripción del concepto es muy grande.
    /// </summary>
    /// <param name="psTexto">Texto</param>
    /// <param name="pnTamRenglon">Tamaño del renglon</param>
    /// <returns></returns>
    private string fnVueltaCarro(string psTexto, int pnTamRenglon)
    {
        string sResultado = string.Empty;
        int nUltimoEspacio = 0;
        int nlongitudTexto = 0;

        nlongitudTexto = psTexto.Length;

        while (nlongitudTexto > 0)
        {
            if ((psTexto.Length > pnTamRenglon) && (psTexto.Contains('\n') && !(psTexto.Substring(0, pnTamRenglon).LastIndexOf('\n') <= 0)))
            {
                nUltimoEspacio = psTexto.Substring(0, pnTamRenglon).IndexOf('\n');
                sResultado += psTexto.Substring(0, nUltimoEspacio).TrimStart().TrimEnd() + '\n';
                psTexto = psTexto.Remove(0, 1).Substring(nUltimoEspacio).ToString();
            }
            else if (psTexto.Length > pnTamRenglon)
            {
                nUltimoEspacio = psTexto.Substring(0, pnTamRenglon).LastIndexOf(" ");
                sResultado += psTexto.Substring(0, nUltimoEspacio).TrimStart().TrimEnd() + '\n';
                psTexto = psTexto.Substring(nUltimoEspacio).ToString().TrimStart().TrimEnd() + " ";
    ...
    /// <summary>
    /// Función que nos ayuda a obtener la descripción del concepto del tamaño de la columna
    /// </summary>
    /// <param name="psTexto">Texto</param>
    /// <param name="pnTamRenglon">Tamaño del renglon</param>
    /// <param name="pnRengloActual">Renglon actual</param>
    /// <param name="pnMaximoRenglones">Maximo de renglones</param>
    /// <returns></returns>
    private string fnDescripcionTamanioColumna(string psTexto, int pnTamRenglon, int pnRengloActual, int pnMaximoRenglones)
    {
        string sResultado = string.Empty;
        int nlongitudTexto = 0;
        int nUltimoEspacio = 0;
        ...
        while (pnRengloActual <= pnMaximoRenglones && nlongitudTexto > 0)
        {
            if ((psTexto.Length > pnTamRenglon && pnRengloActual <= pnMaximoRenglones) && (psTexto.Contains('\n') && !(psTexto.Substring(0, pnTamRenglon).LastIndexOf('\n') <= 0)))
            {
                nUltimoEspacio = psTexto.Substring(0, pnTamRenglon).IndexOf('\n');
                sResultado += psTexto.Substring(0, nUltimoEspacio).TrimStart() + '\n';
                psTexto = psTexto.Remove(0, 1).Substring(nUltimoEspacio).ToString();
            }
            else if (psTexto.Length > pnTamRenglon && pnRengloActual <= pnMaximoRenglones)
            {
                nUltimoEspacio = psTexto.Substring(0, pnTamRenglon).LastIndexOf(" ");
                sResultado += psTexto.Substring(0, nUltimoEspacio).TrimStart().TrimEnd() + " ";
                psTexto = psTexto.Substring(nUltimoEspacio).ToString().TrimStart().TrimEnd() + " ";
            ...
            else
            {
                nUltimoEspacio = psTexto.Length;
                sResultado += psTexto.Substring(0, nUltimoEspacio).TrimStart();
                psTexto = psTexto.Substring(nUltimoEspacio).ToString().TrimStart();
            }
            pnRengloActual++;
            nlongitudTexto = psTexto.Length;
        }
    ...
    /// <summary>
    /// Función que obtiene el siguiente renglon del texto, recortando el actual.
    /// </summary>
    /// <param name="psTexto">Texto</param>
    /// <param name="pnTamRenglon">Tamaño del renglon</param>
    /// <param name="pnRengloActual">Renglon actual</param>
    /// <param name="pnMaximoRenglones">Maximo de renglones</param>
    /// <returns></returns>
    private string fnSiguienteDescripcionTamanioColumna(string psTexto, int pnTamRenglon, int pnRengloActual, int pnMaximoRenglones)
    {
        string sResultado = string.Empty;
        string[] Descripcion = null;

        Descripcion = fnVueltaCarro(psTexto, pnTamRenglon).Split('\n'); //.Replace("\r", " ").Split('\n');

        for (int i = 0; i < Descripcion.Length; i++)
        {
            if (i >= (pnMaximoRenglones))
            {
                //if (!Descripcion[i].Contains('\r'))
                //    sResultado += Descripcion[i] + System.Environment.NewLine;
                //else
                sResultado += Descripcion[i] + '\n';
            }
        }
        return sResultado;
    }
...
public class Detalle
{
    ...
    public string cantidad
    {
        ...
        set
        {
            ...
            sCantidad = Convert.ToDouble(value, languaje).ToString("N2", languaje);
        }
    }
    ...
    private List<ComprobanteImpuestosRetencionT> _retencionesT;
    public List<ComprobanteImpuestosRetencionT> RetencionesT
    {
        get { return _retencionesT; }
        set { _retencionesT = value; }
    }

    private List<ComprobanteImpuestosTrasladoT> _trasladosT;
    public List<ComprobanteImpuestosTrasladoT> TrasladosT
    {
        get { return _trasladosT; }
        set { _trasladosT = value; }
    }

    private t_UbicacionFiscalT _ubicacionFiscalT;
    public t_UbicacionFiscalT UbicacionFiscalT
    {
        get { return _ubicacionFiscalT; }
        set { _ubicacionFiscalT = value; }
    }

    private t_InformacionAduaneraT _informacionAduaneraT;
    public t_InformacionAduaneraT InformacionAduaneraT
    {
    ...
    public Detalle(XPathNavigator navDetalle, XmlNamespaceManager nsmComprobante)
    {
        ...
        try
        {
            List<ComprobanteImpuestosRetencionT> impuestosRetencion = new List<ComprobanteImpuestosRetencionT>();
            List<ComprobanteImpuestosTrasladoT> impuestosTraslado = new List<ComprobanteImpuestosTrasladoT>();
            ...
            {
                ComprobanteImpuestosRetencionT impRet = new ComprobanteImpuestosRetencionT();
                ...
                try
                {
                    ...
                    if (!String.IsNullOrEmpty(sImpuestoRet))
                    {
                        ...
                        {
                            case "IVA": impRet.impuesto = ComprobanteImpuestosRetencionImpuestoT.IVA; break;
                            case "ISR": impRet.impuesto = ComprobanteImpuestosRetencionImpuestoT.ISR; break;
                        }
                    }
            ...
            {
                ComprobanteImpuestosTrasladoT impTras = new ComprobanteImpuestosTrasladoT();
                ...
                try
                {
                    ...
                    if (!String.IsNullOrEmpty(sImpuestoTras))
                    {
                        ...
                        {
                            case "IVA": impTras.impuesto = ComprobanteImpuestosTrasladoImpuestoT.IVA; break;
                            case "IEPS": impTras.impuesto = ComprobanteImpuestosTrasladoImpuestoT.IEPS; break;
                        }
                    }
            ...
            try
            {
                t_UbicacionFiscalT ubicacionTerceros = new t_UbicacionFiscalT();
            ...
            try
            {
                t_InformacionAduaneraT aduaneraTerceros = new t_InformacionAduaneraT();
...
public class clsErrorLogPDF
{
    //private static clsInicioSesionUsuario gUsuario;

    public clsErrorLogPDF()
    {
    }
...
public class clsComunPDF
{
    public static string ObtenerParamentro(string sParametro)
    {
        ...
        using (SqlConnection con = new SqlConnection(PAXCrypto.CryptoAES.DesencriptaAES64(ConfigurationManager.ConnectionStrings["conControl"].ConnectionString)))
        {
            ...
            {
                con.Close();
                clsErrorLogPDF.fnNuevaEntrada(ex, clsErrorLogPDF.TipoErroresLog.Conexion);
            }
        }
        return sRetorno;
    }
