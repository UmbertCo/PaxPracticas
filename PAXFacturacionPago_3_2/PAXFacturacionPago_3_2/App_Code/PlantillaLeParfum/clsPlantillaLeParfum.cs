...
/// </summary>
public class clsPlantillaLeParfum
{
    #region atributos
    ...
    public clsPlantillaLeParfum(XmlDocument pxComprobante)
	{
    ...
    private int fnContarMultilinea(StaticContainer pContenedor, string psTexto, FontProp pFuente, double pdPosX, double pdPosY, int pnTamRenglon, bool bBuscarEspacio)
    {
        ...
        foreach (RepString r in fnDividirRenglonesDetalle(psTexto, pnTamRenglon, pFuente, bBuscarEspacio))// fnDividirRenglones 
        {
            nNoRenglon++;
        }
    ...
    private void fnTotales(XmlNamespaceManager nsmComprobante, XPathNavigator navPie, StaticContainer Pie, int nNumPag, int nTotPag, string sColor)
    {
        ...
        double dYPVac = 60;
        double dYAltPVac = 60;
        fnCrearPanelRedondeado(Pie, posPanelTotales - dXPTot, dYPVac, Pie.rWidth - posPanelTotales + dAnPTot, tamCodigo - dYAltPVac, grosorPen, radioCurva, true, ColorT);
    ...
    private void fnDatosEmisor(XmlNamespaceManager nsmComprobante, XPathNavigator navEncabezado, StaticContainer Encabezado, System.Drawing.Color sColor)
    {
        ...
        rfc = "RFC: " + "LPA090922C77"; //navEncabezado.SelectSingleNode("/cfdi:Comprobante/cfdi:Emisor/@rfc", nsmComprobante).Value;
        ...
        double dposRen = 2.9;
        double dcalRen = 0.8;
        Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignCenter, posRazon - 3, RepObj.rAlignTop, new RepString(fPropTitulo, "LE PARFUM ACCESORIOS Y PERFUMES S.A. DE C.V."));
        Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignCenter, posRenglon + tamRenglon * 0.5, RepObj.rAlignTop, new RepString(fPropNormal, "Niños Heroes No. 708-B Col. Centro, C.P. 31OOO"));
        Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignCenter, posRenglon + tamRenglon * 1.3, RepObj.rAlignTop, new RepString(fPropNormal, "Tels: (614) 415-80-39 y 414-42-00"));
        Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignCenter, posRenglon + tamRenglon * 2.1, RepObj.rAlignTop, new RepString(fPropNormal, "Chihuahua, Chihuahua"));
        Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignCenter, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, "email: bolsasmm_chihuahua@hotmail.com"));
        dposRen += dcalRen;
        Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignCenter, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, "RFC: LPA090922C77"));
        dposRen += dcalRen;
                    ...
                    if (sCad.Length > 1)
                    {
                        foreach (string s in sCad)
                        {
                            ...
                            if (s.Length > pnTamRenglon)
                            {
                                ...
                                if (nUltimoEspacio <= 0)
                                    nUltimoEspacio = s.Substring(0, pnTamRenglon).Length;
...
public class clsErrorLogLeParfum
{
    //private static clsInicioSesionUsuario gUsuario;

    public clsErrorLogLeParfum()
    {
    }
...
public class clsComunLogo
{
    public static string ObtenerParamentro(string sParametro)
    {
        ...
        using (SqlConnection con = new SqlConnection(PAXCrypto.CryptoAES.DesencriptaAES64(ConfigurationManager.ConnectionStrings["conControl"].ConnectionString)))
        {
            ...
            {
                con.Close();
                clsErrorLogLeParfum.fnNuevaEntrada(ex, clsErrorLogLeParfum.TipoErroresLog.Conexion);
            }
        }
        return sRetorno;
    }
