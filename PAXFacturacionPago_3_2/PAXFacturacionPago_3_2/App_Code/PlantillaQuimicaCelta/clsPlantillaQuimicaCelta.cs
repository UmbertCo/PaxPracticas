...
/// </summary>
public class clsPlantillaQuimicaCelta
{
    #region atributos
    ...
    private FontProp fPropP300C; //Pantone 300C
    private FontProp fPropP306C; //Pantone 306C
    ...
    public clsPlantillaQuimicaCelta(XmlDocument pxComprobante)
	{
	...
	public void fnGenerarPDF(int id_contribuyente,int sid_rfc,string scolor)
	{
        ...
        fPropTitulo = new FontProp(fuenteTitulo, tamFuenteTitulo, ColorTranslator.FromHtml("#005CB9"));
		fPropTitulo.bBold = true;

		//letra normal
		fuenteNormal = new FontDef(PDF, FontDef.StandardFont.Helvetica);
        fPropNormal = new FontProp(fuenteNormal, tamFuenteNormal, ColorTranslator.FromHtml("#005CB9"));
        fPropChica = new FontProp(fuenteNormal, tamFuenteChica, ColorTranslator.FromHtml("#005CB9"));
        ...
        fPropNegrita = new FontProp(fuenteNormal, tamFuenteNormal, ColorTranslator.FromHtml("#005CB9"));
        fPropNegrita.bBold = true;

        fPropP300C = new FontProp(fPropNegrita.fontDef, tamFuenteNormal, ColorTranslator.FromHtml("#005CB9"));
        fPropP306C = new FontProp(fPropNegrita.fontDef, tamFuenteNormal, ColorTranslator.FromHtml("#00b3e3"));
		...
		while (bSeguir)
		{
            ...
            ColorT = ColorTranslator.FromHtml("#005CB9");
            //ColorT = (System.Drawing.Color)colConvert.ConvertFromString(scolor);
            StaticContainer Encabezado = fnCrearEncabezado(ColorT);

            decimal pSubtotal = 0;
            pagina.Add(margenPagina, margenPagina + Encabezado.rHeight, fnCrearDetalle(detalles, ColorT,ref pSubtotal));
           
            StaticContainer Pie = fnCrearPie(pagina.iPageNo, nTotPag, scolor,pSubtotal);
            ...
            if (ms.Length > 0)
            {
                RepImage image = new RepImage(ms, 130, 70);
                pagina.Add(margenPagina + 12, margenPagina + 80, image);
            }
            ...
            if (d.aduana != null || !String.IsNullOrEmpty(d.RfcT))
            {
                ...
                if (!String.IsNullOrEmpty(d.RfcT))
                {
                    ...
                    if ((d.RetencionesT != null && d.RetencionesT.Count > 0) || (d.TrasladosT != null && d.TrasladosT.Count > 0)) //Impuestos tercero
                    {
                        ...
                        {
                            foreach (ComprobanteImpuestosRetencionTLogo retencion in d.RetencionesT)
                            {
                                switch (retencion.impuesto)
                                {
                                    case ComprobanteImpuestosRetencionImpuestoTLogo.IVA: sDetalleTerceros.Append("IVA Ret: "); break;
                                    case ComprobanteImpuestosRetencionImpuestoTLogo.ISR: sDetalleTerceros.Append("ISR: "); break;
                                }
                                sDetalleTerceros.Append(TransformacionLogo.fnFormatoCurrency(retencion.importe.ToString()) + " ");
                            }
                        }

                        if (d.TrasladosT != null && d.TrasladosT.Count > 0)
                        {
                            foreach (ComprobanteImpuestosTrasladoTLogo traslado in d.TrasladosT)
                            {
                                switch (traslado.impuesto)
                                {
                                    case ComprobanteImpuestosTrasladoImpuestoTLogo.IVA: sDetalleTerceros.Append("IVA Tras "); break;
                                    case ComprobanteImpuestosTrasladoImpuestoTLogo.IEPS: sDetalleTerceros.Append("IEPS "); break;
                                }
                                sDetalleTerceros.Append(TransformacionLogo.fnFormatoPorcentaje(traslado.tasa.ToString()) + ": ");
                                sDetalleTerceros.Append(TransformacionLogo.fnFormatoCurrency(traslado.importe.ToString()) + " ");
                            }
                        }
                    }
	...
	/// <returns></returns>
	private StaticContainer fnCrearDetalle(List<DetalleLogo> detalles, System.Drawing.Color sColor, ref decimal pSubtotal)
	{
		StaticContainer areaDetalle = new StaticContainer(RT.rPointFromMM(anchoPagina) - margenPagina * 2, RT.rPointFromMM(altoPagina - altoEncabezado - altoPie) - margenPagina * 2);
        areaDetalle.rHeight = areaDetalle.rHeight - 6;
		...
		//definimos las posiciones
		//double posColumna1 = areaDetalle.rWidth * 0.015;
		//double posColumna2 = areaDetalle.rWidth * 0.114;
        //double posColumna3 = areaDetalle.rWidth * 0.28; //0.2;
        ...
        double posColumna1 = areaDetalle.rWidth * 0.015;    //clave
        double posColumna2 = areaDetalle.rWidth * 0.1;      //unidad
        double posColumna3 = areaDetalle.rWidth * 0.28;     //descripcion
        double posColumna4 = areaDetalle.rWidth * 0.72;     //cantidad
        double posColumna5 = areaDetalle.rWidth * 0.88;     //precio
        double posColumna6 = areaDetalle.rWidth * 0.99;     //importe
        ...
        for (int i = 0; renglonActual <= maxRenglones; i++)
		{
            ...
            {
                //d.descripcion = "Esta es una Descripcion 20%|";
                //d.descripcion = "Esta es una Descripcion";

                Col1 = fnAgregarMultilinea(areaDetalle, d.cantidad, fPropNormal, posColumna1, posRenglon, 9, true);
                Col2 = fnAgregarMultilinea(areaDetalle, d.unidad, fPropNormal, posColumna2, posRenglon, 9, true);
                ...
                try
                {
                    ...
                    va = d.importe.Replace("$", "").Replace(",","");
                    output = d.descripcion.Split(new char[] { '|', '|' })[1];
                    int index = d.descripcion.IndexOf(output);
                    d.descripcion = d.descripcion.Remove(index, output.Length);
                    index = d.descripcion.IndexOf("||");
                    d.descripcion = d.descripcion.Remove(index, 2);

                    try//Evaluamos que sea solo número para realizar el calculo
                    {
                       System.Threading.Thread.CurrentThread.CurrentCulture = new System.Globalization.CultureInfo("es-MX");

                        decimal ddesc = decimal.Parse(output, System.Globalization.NumberStyles.AllowDecimalPoint, System.Globalization.CultureInfo.InvariantCulture);
                        decimal dPreUnitario = decimal.Parse(va, System.Globalization.NumberStyles.AllowDecimalPoint, System.Globalization.CultureInfo.InvariantCulture);

                        decimal dTotal = (dPreUnitario * ddesc) / 100;

                        dTotal = dPreUnitario - dTotal;

                        pSubtotal += dTotal;

                        string stotal = string.Format("{0:c2}", dTotal);

                        areaDetalle.AddAligned(posColumna4, RepObj.rAlignRight, posRenglon, RepObj.rAlignBottom, new RepString(fPropNormal, output));
                        ...
                        areaDetalle.AddAligned(posColumna6, RepObj.rAlignRight, posRenglon, RepObj.rAlignBottom, new RepString(fPropNormal,"$"+ Convert.ToString(dTotal)));
                    }
                    catch (Exception)
                    {
                        pSubtotal += decimal.Parse(va, System.Globalization.NumberStyles.AllowDecimalPoint, System.Globalization.CultureInfo.InvariantCulture);
                        areaDetalle.AddAligned(posColumna4, RepObj.rAlignRight, posRenglon, RepObj.rAlignBottom, new RepString(fPropNormal, output));
                        areaDetalle.AddAligned(posColumna5, RepObj.rAlignRight, posRenglon, RepObj.rAlignBottom, new RepString(fPropNormal, d.valorUnitario));
		...
		while (navDetalles.MoveNext())
		{
            ...
            if (nodenavigator.HasChildren)//Si contiene nodo hijo
            {
                ...
                if (navDetallesAduana != null)
                    detalles.Add(new DetalleLogo(navDetallesAduana, nsmComprobante));
                ...
                if (navComplTerceros != null)
                {
                    ...
                    if (navTerceros != null)
                        detalles.Add(new DetalleLogo(navTerceros, nsmComprobanteComplTerceros));
                }
            }
		}
	...
	/// </summary>
	/// <returns>StaticContainer con la información del pie de página</returns>
	private StaticContainer fnCrearPie(int nNumPag, int nTotPag, string sColor , decimal pSubtotal)
	{
		StaticContainer Pie = new StaticContainer(RT.rPointFromMM(anchoPagina) - margenPagina * 2, RT.rPointFromMM(altoPie));
        ...
        fnTotales(nsmComprobante, navEncabezado, Pie, nNumPag, nTotPag, sColor, pSubtotal);
	...
	/// <param name="navPie">Navegador con los datos del pie de página</param>
	/// <param name="Pie">Contenedor en el cual se desplegarán los datos del pie de página</param>
	private void fnTotales(XmlNamespaceManager nsmComprobante, XPathNavigator navPie, StaticContainer Pie, int nNumPag, int nTotPag, string sColor ,decimal pSubtotal)
	{
        System.Threading.Thread.CurrentThread.CurrentCulture = new System.Globalization.CultureInfo("es-MX");
        ...
        subtotal = Convert.ToString(pSubtotal); // navPie.SelectSingleNode("/cfdi:Comprobante/@subTotal", nsmComprobante).Value;
	...
	private void fnDibujarTitulosDetalle(StaticContainer Encabezado, System.Drawing.Color sColor)
	{
		//Dibujamos el área de los titulos
            ...
            //double altoBarra = fPropP300C.rSize * 2;

            fnCrearPanelRedondeado(Encabezado, 0, Encabezado.rHeight - altoBarra, Encabezado.rWidth, altoBarra, grosorPen, radioCurva, true, ColorTranslator.FromHtml("#00b3e3"));
            ...
            //double puntoMedio = Encabezado.rHeight - fPropP300C.rSize;

            //Definimos la posicion de los titulos
            double posColumna1 = Encabezado.rWidth * 0.015;    //clave
            ...
            double posColumna5 = Encabezado.rWidth * 0.77;     //precio
            double posColumna6 = Encabezado.rWidth * 0.92;     //importe

            Encabezado.AddAligned(posColumna1, RepObj.rAlignLeft, puntoMedio, RepObj.rAlignCenter, new RepString(fPropP300C, "CANT."));
            Encabezado.AddAligned(posColumna2, RepObj.rAlignLeft, puntoMedio, RepObj.rAlignCenter, new RepString(fPropP300C, "UNIDAD DE MEDIDA"));
            Encabezado.AddAligned(posColumna3, RepObj.rAlignLeft, puntoMedio, RepObj.rAlignCenter, new RepString(fPropP300C, "DESCRIPCIÓN"));
            Encabezado.AddAligned(posColumna4, RepObj.rAlignLeft, puntoMedio, RepObj.rAlignCenter, new RepString(fPropP300C, "% DESC."));
            Encabezado.AddAligned(posColumna5, RepObj.rAlignLeft, puntoMedio, RepObj.rAlignCenter, new RepString(fPropP300C, "PRECIO UNITARIO"));
            Encabezado.AddAligned(posColumna6, RepObj.rAlignLeft, puntoMedio, RepObj.rAlignCenter, new RepString(fPropP300C, "TOTAL"));
	}
    ...
    private void fnDatosEmisor(XmlNamespaceManager nsmComprobante, XPathNavigator navEncabezado, StaticContainer Encabezado, System.Drawing.Color sColor, ref double Renglon)
	{
        ...
        fPropNormal.rSize = 5;
        //fPropNormal = new FontProp(fuenteNormal, 4);
        fPropNegrita = new FontProp(fuenteNormal, 5, ColorTranslator.FromHtml("#005CB9"));
        fPropNegrita.bBold = true;
        ...
        if (exCalle != string.Empty)
        {
            ...
            if (i > 1)
                Renglon -= 0.5;
            i = fnAgregarMultilinea(Encabezado, exUbicacion + " " + exUbicacion2, fPropNormal, leftPadding + 155, posRenglon + tamRenglon * Renglon, 46, true);
            Renglon += i;
            //if (i > 1)
            //    Renglon -= 0.5;
        ...
        {
            i = fnAgregarMultilinea(Encabezado, direccion, fPropNormal, leftPadding + 155, posRenglon + tamRenglon * Renglon, 60, true);
            Renglon += i;
            if (i > 1)
                Renglon -= 0.5;

            i = fnAgregarMultilinea(Encabezado, ubicacion+ " " +ubicacion2, fPropNormal, leftPadding + 155, posRenglon + tamRenglon * Renglon, 55, true);
            Renglon += i;
            if (i > 1)
                Renglon -= 0.5;
            i = fnAgregarMultilinea(Encabezado, rfc, fPropNormal, leftPadding + 155, posRenglon + tamRenglon * Renglon, 46, true);
        ...
        if (!string.IsNullOrEmpty(referencia))
        {
            ...
            if (version == "3.2")
            {
                ...
                i = fnAgregarMultilinea(Encabezado, sRegFis + " " + Regimenfiscal, fPropNormal, leftPadding + 155, posRenglon + tamRenglon * Renglon, 60, true);
                Renglon += i;
                //if (i > 1)
                    //Renglon -= 0.5;
                ...
                if (i > 1)
                    Renglon -= 0.5;
                i = fnAgregarMultilinea(Encabezado, "Tels. 57876171  57879509  36120938  36120954", fPropNormal, leftPadding + 155, posRenglon + tamRenglon * Renglon, 46, true);
                Renglon += i;

                if (i > 1)
                    Renglon -= 0.5;
                i = fnAgregarMultilinea(Encabezado, "Email: ventas@biogal.mx", fPropNormal, leftPadding + 155, posRenglon + tamRenglon * Renglon, 46, true);
                Renglon += i;

                if (i > 1)
                    Renglon -= 0.5;
                i = fnAgregarMultilinea(Encabezado, "contacto@biogal.mx", fPropNormal, leftPadding + 155, posRenglon + tamRenglon * Renglon, 46, true);
                Renglon += i;

                if (i > 1)
                    Renglon -= 0.5;
                i = fnAgregarMultilinea(Encabezado, "www.biogal.mx", fPropNormal, leftPadding + 155, posRenglon + tamRenglon * Renglon, 46, true);
                Renglon += i;
            }
        }
        else
        ...
        fPropNormal = new FontProp(fuenteNormal, 4, ColorTranslator.FromHtml("#005CB9"));
        fPropNegrita = new FontProp(fuenteNormal, 5, ColorTranslator.FromHtml("#005CB9"));
        fPropNegrita.bBold = true;
        fPropRoja = new FontProp(fuenteNormal, 5, Color.Red);
    ...
    public void fnCrearPanelRedondeado(StaticContainer poObjeto, double pfPosX, double pfPosY, double pfAncho, double pfAlto, double pfGrosor, double pfRadioCurva, bool pbRellenar, System.Drawing.Color sColor)
    {
        ...
        //PenProp penAzul = new PenProp(PDF, pfGrosor, ColorTranslator.FromHtml("#00B5E6"));
        //Color ColorT = ColorTranslator.FromHtml("#0067C6");
        //BrushProp brush = new BrushProp(PDF, ColorT);
...
public class DetalleLogo
{
    ...
    private List<ComprobanteImpuestosRetencionTLogo> _retencionesT;
    public List<ComprobanteImpuestosRetencionTLogo> RetencionesT
    {
        get { return _retencionesT; }
        set { _retencionesT = value; }
    }

    private List<ComprobanteImpuestosTrasladoTLogo> _trasladosT;
    public List<ComprobanteImpuestosTrasladoTLogo> TrasladosT
    {
        get { return _trasladosT; }
        set { _trasladosT = value; }
    }

    private t_UbicacionFiscalTLogo _ubicacionFiscalT;
    public t_UbicacionFiscalTLogo UbicacionFiscalT
    {
        get { return _ubicacionFiscalT; }
        set { _ubicacionFiscalT = value; }
    }

    private t_InformacionAduaneraTLogo _informacionAduaneraT;
    public t_InformacionAduaneraTLogo InformacionAduaneraT
    {
	...
	public DetalleLogo(XPathNavigator navDetalle, XmlNamespaceManager nsmComprobante)
	{
        ...
        try
        {
            List<ComprobanteImpuestosRetencionTLogo> impuestosRetencion = new List<ComprobanteImpuestosRetencionTLogo>();
            List<ComprobanteImpuestosTrasladoTLogo> impuestosTraslado = new List<ComprobanteImpuestosTrasladoTLogo>();
            ...
            {
                ComprobanteImpuestosRetencionTLogo impRet = new ComprobanteImpuestosRetencionTLogo();
                ...
                try
                {
                    ...
                    if (!String.IsNullOrEmpty(sImpuestoRet))
                    {
                        ...
                        {
                            case "IVA": impRet.impuesto = ComprobanteImpuestosRetencionImpuestoTLogo.IVA; break;
                            case "ISR": impRet.impuesto = ComprobanteImpuestosRetencionImpuestoTLogo.ISR; break;
                        }
                    }
            ...
            {
                ComprobanteImpuestosTrasladoTLogo impTras = new ComprobanteImpuestosTrasladoTLogo();
                ...
                try
                {
                    ...
                    if (!String.IsNullOrEmpty(sImpuestoTras))
                    {
                        ...
                        {
                            case "IVA": impTras.impuesto = ComprobanteImpuestosTrasladoImpuestoTLogo.IVA; break;
                            case "IEPS": impTras.impuesto = ComprobanteImpuestosTrasladoImpuestoTLogo.IEPS; break;
                        }
                    }
            ...
            try
            {
                t_UbicacionFiscalTLogo ubicacionTerceros = new t_UbicacionFiscalTLogo();
            ...
            try
            {
                t_InformacionAduaneraTLogo aduaneraTerceros = new t_InformacionAduaneraTLogo();
...
public partial class ComprobanteImpuestosRetencionTLogo
{
    private ComprobanteImpuestosRetencionImpuestoTLogo impuestoField;
    private decimal importeField;

    public ComprobanteImpuestosRetencionImpuestoTLogo impuesto
    {
...
public enum ComprobanteImpuestosRetencionImpuestoTLogo
{
    ISR, IVA,
}

public partial class ComprobanteImpuestosTrasladoTLogo
{
    private ComprobanteImpuestosTrasladoImpuestoTLogo impuestoField;
    private decimal tasaField;
    private decimal importeField;

    public ComprobanteImpuestosTrasladoImpuestoTLogo impuesto
    {
        get { return this.impuestoField; }
        set { this.impuestoField = value; }
    }
...
public enum ComprobanteImpuestosTrasladoImpuestoTLogo
{
    IVA, IEPS,
}

public partial class t_UbicacionFiscalTLogo
{
...
public partial class t_InformacionAduaneraTLogo
{
...
public class clsErrorLogQuimicaCelta
{
    //private static clsInicioSesionUsuario gUsuario;

    public clsErrorLogQuimicaCelta()
    {
    }
...
public class clsComunLogo
{
    public static string ObtenerParamentro(string sParametro)
    {
        ...
        using (SqlConnection con = new SqlConnection(PAXCrypto.CryptoAES.DesencriptaAES64(ConfigurationManager.ConnectionStrings["conControl"].ConnectionString)))
        {
            ...
            {
                con.Close();
                clsErrorLogQuimicaCelta.fnNuevaEntrada(ex, clsErrorLogQuimicaCelta.TipoErroresLog.Conexion);
            }
        }
        return sRetorno;
    }
