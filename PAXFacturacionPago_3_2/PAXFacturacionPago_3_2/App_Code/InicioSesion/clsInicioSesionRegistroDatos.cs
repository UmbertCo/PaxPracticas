...
public class clsInicioSesionRegistroDatos
{
                                                ...
                                                int      id_version)
    {
        ...
        using (SqlConnection con = new SqlConnection(PAXCrypto.CryptoAES.DesencriptaAES64(cadenaCon)))
        {
            ...
            using (SqlTransaction tran = con.BeginTransaction())
            {
                try
                {
                    using (SqlCommand cmd = new SqlCommand("usp_InicioSesion_RegistraDatos", con))
                    {
    ...
    /// <returns></returns>
    public static DataTable fnRecuperaVersionesVigentes(string psVersion)
    {
        ...
        using (SqlConnection con = new SqlConnection(PAXCrypto.CryptoAES.DesencriptaAES64(ConfigurationManager.ConnectionStrings["conConfiguracion"].ConnectionString)))
        {
            try
            {
                using (SqlCommand cmd = new SqlCommand("usp_Con_Versiones_Vigentes_Sel", con))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    if (psVersion != "")
                        cmd.Parameters.Add(new SqlParameter("sVersion", psVersion));
                                            ...
                                            byte[] sCodigo_Postal)
    {
        ...
        using (SqlConnection con = new SqlConnection(PAXCrypto.CryptoAES.DesencriptaAES64(cadenaCon)))
        {
            using (SqlTransaction tran = con.BeginTransaction())
            {
                try
                {
                    using (SqlCommand cmd = new SqlCommand("usp_Con_Domicilios_Ex_Ins", con))
                    {
                        cmd.Transaction = tran;
                        cmd.CommandType = System.Data.CommandType.StoredProcedure;
    ...
    public static bool solicitudRegistroContribuyenteDistribuidor(string sRfc,
                                                ...
                                                byte[] sLocalidad,
                                                byte[] sCalle,
                                                byte[] sNumero_Exterior,
                                                byte[] sNumero_Interior,
                                                byte[] sColonia,
                                                string sReferencia,
                                                byte[] sCodigo_Postal,
                                                int nId_Usuario,
                                                ...
                                                byte[]   nPfx)
    {
        ...
        using (SqlConnection con = new SqlConnection(PAXCrypto.CryptoAES.DesencriptaAES64(cadenaCon)))
        {
            ...
            using (SqlTransaction tran = con.BeginTransaction())
            {
                try
                {
                    using (SqlCommand cmd = new SqlCommand("usp_InicioSesion_RegistraDatosDistribuidor_ins", con))
                    {
                        cmd.Transaction = tran;
                        cmd.CommandType = System.Data.CommandType.StoredProcedure;
                        ...
                        if (PAXCrypto.CryptoAES.DesencriptaAES(sPassword) != "" || 
                            PAXCrypto.CryptoAES.DesencriptaAES(sPassword) != String.Empty)
                        {
                            cmd.Parameters.AddWithValue("mCertificado", mCertificado);
                            cmd.Parameters.AddWithValue("mKey", mKey);
                            cmd.Parameters.AddWithValue("mKeyPem", mKeyPem);
                                                ...
                                                )
    {
        ...
        using (SqlConnection con = new SqlConnection(PAXCrypto.CryptoAES.DesencriptaAES64(cadenaCon)))
        {
            ...
            using (SqlTransaction tran = con.BeginTransaction())
            {
                try
                {
                    using (SqlCommand cmd = new SqlCommand("usp_InicioSesion_RegistraDatos_Cobro", con))
                    {
                        cmd.Transaction = tran;
                        cmd.CommandType = System.Data.CommandType.StoredProcedure;
