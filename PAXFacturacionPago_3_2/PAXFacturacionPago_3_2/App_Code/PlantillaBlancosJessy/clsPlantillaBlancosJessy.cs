...
/// </summary>
public class clsPlantillaBlancosJessy
{
    #region atributos

    private DataTable sqlInfoMatriz;
    ...
    public clsPlantillaBlancosJessy(XmlDocument pxComprobante, DataTable sdrInfo)
	{
	...
	public void fnGenerarPDF(int id_contribuyente,int sid_rfc,string scolor)
	{
        ...
        //color especificado por el cliente
        scolor = "#13298C";
        ...
        System.Drawing.ColorConverter colConvertTit = new ColorConverter();
        Color ColorTit = (System.Drawing.Color)colConvertTit.ConvertFromString(scolor);
		
        //Letra titulo
		fuenteTitulo = new FontDef(PDF, FontDef.StandardFont.TimesRoman);
        fPropTitulo = new FontProp(fuenteTitulo, tamFuenteTitulo, ColorTit);
		...
		while (bSeguir)
		{
            ...
            if (ms.Length > 0)
            {
                double dWidth = 85;
                double dHeight = 86;
                RepImage image = new RepImage(ms, dWidth, dHeight);
                int zX = 15;
                int zY = 92;
                pagina.Add(margenPagina + zX, margenPagina + zY, image);
            }

            //Se carga imagen logo pie
            ms = fnImagenClienteDir("PieblancosJessy.jpg");
            if (ms.Length > 0)
            {
                double dAlt = 40;
                double dAnc = 520;
    ...
    public void fnGenerarPDFSave(int id_contribuyente,int sid_rfc,string scolor,string ruta)
    {
        ...
        while (bSeguir)
        {
            ...
            if (ms.Length > 0)
            {
                int w = 53;
                int h = 55;
                RepImage image = new RepImage(ms, w, h);
                int zX = 40;
                int zY = 65;
                pagina.Add(margenPagina + zX, margenPagina + zY, image);
            }
	...
	private void fnTotales(XmlNamespaceManager nsmComprobante, XPathNavigator navPie, StaticContainer Pie, int nNumPag, int nTotPag, string sColor)
	{
        ...
        double dPosicionY =50;
        double dAltoPnl = 55;
        fnCrearPanelRedondeado(Pie, 0, tamCodigo + dPosicionY, Pie.rWidth, Pie.rHeight - tamCodigo - altoRenglon - dAltoPnl, grosorPen, radioCurva, false, ColorT);
	...
	private void fnDibujarTitulosDetalle(StaticContainer Encabezado, System.Drawing.Color sColor)
	{
		//Dibujamos el área de los titulos
            ...
            Encabezado.AddAligned(posColumna5, RepObj.rAlignLeft, puntoMedio, RepObj.rAlignCenter, new RepString(fPropBlanca, "P. UNITARIO"));
            Encabezado.AddAligned(posColumna6, RepObj.rAlignLeft, puntoMedio, RepObj.rAlignCenter, new RepString(fPropBlanca, "IMPORTE"));
	}
    ...
    private void fnDatosEmisor(XmlNamespaceManager nsmComprobante, XPathNavigator navEncabezado, StaticContainer Encabezado, System.Drawing.Color sColor)
	{
        ...
        //Datos del emisor
        try { razonSocial = navEncabezado.SelectSingleNode("/cfdi:Comprobante/cfdi:Emisor/@nombre", nsmComprobante).Value; }
        catch { } 
		rfc = "RFC: " + navEncabezado.SelectSingleNode("/cfdi:Comprobante/cfdi:Emisor/@rfc", nsmComprobante).Value;
        try { calle = navEncabezado.SelectSingleNode("/cfdi:Comprobante/cfdi:Emisor/cfdi:DomicilioFiscal/@calle", nsmComprobante).Value; }
        catch { } 
        ...
        try { version = navEncabezado.SelectSingleNode("/cfdi:Comprobante/@version", nsmComprobante).Value; }
        catch { } 
        ...
        try { municipio = navEncabezado.SelectSingleNode("/cfdi:Comprobante/cfdi:Emisor/cfdi:DomicilioFiscal/@municipio", nsmComprobante).Value; }
        catch { }
        try { pais = navEncabezado.SelectSingleNode("/cfdi:Comprobante/cfdi:Emisor/cfdi:DomicilioFiscal/@pais", nsmComprobante).Value; }
        catch { }
        try { estado = navEncabezado.SelectSingleNode("/cfdi:Comprobante/cfdi:Emisor/cfdi:DomicilioFiscal/@estado", nsmComprobante).Value; }
        catch { } 
        if (version == "3.0")
        ...
        try { codigoPostal = navEncabezado.SelectSingleNode("/cfdi:Comprobante/cfdi:Emisor/cfdi:DomicilioFiscal/@codigoPostal", nsmComprobante).Value; }
        catch { }
        try { noCertificadoEmisor = navEncabezado.SelectSingleNode("/cfdi:Comprobante/@noCertificado", nsmComprobante).Value; }
        catch { } 
        ...
        string sCodigoPostalSucursal, sPaisSucursal, sEstadoSucursal, sMunicipioSucursal, sReferenciaSucursal, sLocalidadSucursal, sColoniaSucursal, sNoInteriorSucursal, sNoExteriorSucursal, sCalleSucursal;
        sCodigoPostalSucursal = sPaisSucursal = sEstadoSucursal = sMunicipioSucursal = sReferenciaSucursal = sLocalidadSucursal = sColoniaSucursal = sNoInteriorSucursal = sNoExteriorSucursal = sCalleSucursal = string.Empty;

        try { sCodigoPostalSucursal = navEncabezado.SelectSingleNode("/cfdi:Comprobante/cfdi:Emisor/cfdi:ExpedidoEn/@codigoPostal", nsmComprobante).Value; }
        catch { }
        try { sPaisSucursal = navEncabezado.SelectSingleNode("/cfdi:Comprobante/cfdi:Emisor/cfdi:ExpedidoEn/@pais", nsmComprobante).Value; }
        catch { }
        try { sEstadoSucursal = navEncabezado.SelectSingleNode("/cfdi:Comprobante/cfdi:Emisor/cfdi:ExpedidoEn/@estado", nsmComprobante).Value; }
        catch { }
        try { sMunicipioSucursal = navEncabezado.SelectSingleNode("/cfdi:Comprobante/cfdi:Emisor/cfdi:ExpedidoEn/@municipio", nsmComprobante).Value; }
        catch { }
        try { sReferenciaSucursal = navEncabezado.SelectSingleNode("/cfdi:Comprobante/cfdi:Emisor/cfdi:ExpedidoEn/@referencia", nsmComprobante).Value; }
        catch { }
        try { sLocalidadSucursal = navEncabezado.SelectSingleNode("/cfdi:Comprobante/cfdi:Emisor/cfdi:ExpedidoEn/@localidad", nsmComprobante).Value; }
        catch { }
        try { sColoniaSucursal = navEncabezado.SelectSingleNode("/cfdi:Comprobante/cfdi:Emisor/cfdi:ExpedidoEn/@colonia", nsmComprobante).Value; }
        catch { }
        try { sNoInteriorSucursal = navEncabezado.SelectSingleNode("/cfdi:Comprobante/cfdi:Emisor/cfdi:ExpedidoEn/@noInterior", nsmComprobante).Value; }
        catch { }
        try { sNoExteriorSucursal = navEncabezado.SelectSingleNode("/cfdi:Comprobante/cfdi:Emisor/cfdi:ExpedidoEn/@noExterior", nsmComprobante).Value; }
        catch { }
        try { sCalleSucursal = navEncabezado.SelectSingleNode("/cfdi:Comprobante/cfdi:Emisor/cfdi:ExpedidoEn/@calle", nsmComprobante).Value; }
        catch { }

        string sDireccionSucursal = string.Empty;

        if (!string.IsNullOrEmpty(sNoExteriorSucursal))
            direccion += " No. " + sNoExteriorSucursal;
        if (!string.IsNullOrEmpty(sNoInteriorSucursal))
            direccion += " Int. " + sNoInteriorSucursal;

        try { calle = sqlInfoMatriz.Rows[0]["calle"].ToString(); }
        catch { calle ="";}
        try { noInterior = sqlInfoMatriz.Rows[0]["numero_interior"].ToString(); }
        catch { noInterior = "" ;}
        try { noExterior = sqlInfoMatriz.Rows[0]["numero_exterior"].ToString(); }
        catch { noExterior = ""; }
        try { colonia = sqlInfoMatriz.Rows[0]["colonia"].ToString(); }
        catch { colonia = ""; }
        try { Localidad = sqlInfoMatriz.Rows[0]["localidad"].ToString(); }
        catch { Localidad = ""; }
        ...
        System.Drawing.ColorConverter colConvert = new ColorConverter();
        System.Drawing.Color ColorLetra = new System.Drawing.Color();
        string scolor = "#13298C";
        ColorLetra = (System.Drawing.Color)colConvert.ConvertFromString(scolor);
        
        fPropNegrita = new FontProp(fuenteNormal, 5, Color.Black);
        fPropNegrita.bBold = true;
        fPropRoja = new FontProp(fuenteNormal, 5, Color.Red);
        fPropTitulo.rSize = 13;
        fPropNormal.rSize = 5;
        fPropNormal.color = ColorLetra;

        double dPX = 250;
        double dposRen = 0.8;
        double dcalRen = 0.8;

        Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignCenter, posRenglon - 9, RepObj.rAlignTop, new RepString(fPropTitulo,"Blancos Jessy"));
        ...
        dposRen += dcalRen;
        int nTamRenglon = 30;
        fnAgregarMultilineaCentrada(Encabezado, "REGIMEN: " + Regimenfiscal, fPropNormal, leftPadding + dPX, posRenglon + tamRenglon * dposRen,nTamRenglon);
        //Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignCenter, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal,"REGIMEN: " + Regimenfiscal));
        dposRen += dcalRen;
        dposRen += dcalRen;

        dPX =120;
        Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal,"MATRIZ"));
        dPX = 280;
        Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, "SUCURSAL"));
        dposRen += dcalRen;
        ...
        double Renglon = 0;
        double seps = 4;
        double pos = fPropNormal.rSize + sep;
        double tamRenglons = fPropNormal.rSize + seps;
        double i = 0;

        i = fnAgregarMultilinea(Encabezado, calle, fPropNormal, 131.5, 65 + tamRenglons * Renglon, 30, true); Renglon += i;
        i = fnAgregarMultilinea(Encabezado, colonia, fPropNormal, 131.5, 65 + tamRenglons * Renglon, 30, true); Renglon += i;
        i = fnAgregarMultilinea(Encabezado, estado, fPropNormal, 131.5, 65 + tamRenglons * Renglon, 30, true); Renglon += i;
        i = fnAgregarMultilinea(Encabezado, "TEL.(614) 426 55 45", fPropNormal, 131.5, 65 + tamRenglons * Renglon, 30, true); Renglon += i;

        Renglon = 0;
        i = 0;

        i = fnAgregarMultilinea(Encabezado, sCalleSucursal, fPropNormal, 292, 65 + tamRenglons * Renglon, 30, true); Renglon += i;
        i = fnAgregarMultilinea(Encabezado, sColoniaSucursal, fPropNormal, 292, 65 + tamRenglons * Renglon, 30, true); Renglon += i;
        i = fnAgregarMultilinea(Encabezado, sEstadoSucursal, fPropNormal, 292, 65 + tamRenglons * Renglon, 30, true); Renglon += i;
        i = fnAgregarMultilinea(Encabezado, "TEL.(625) 583 08 85", fPropNormal, 292, 65 + tamRenglons * Renglon, 30, true); Renglon += i;

        /* Esto es como originalmente estaba. Lo reemplaze con el multilinea en caso de que los datos sean muy grandes y no se empalmen con otros paneles
        dPX = 0;
        dPX = 120;
        Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, calle));//+ direccion.ToUpper()));// +" "+noExterior.ToUpper()));
        dPX = 280;

        dcalRen = 0.7;
        Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, sCalleSucursal));
        dposRen += dcalRen;
        dPX = 120;
        Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, colonia )); //colonia.ToUpper()));
        dPX = 280;
        Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, sColoniaSucursal));
        dposRen += dcalRen;
        dPX = 120;
        Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, Localidad));//ubicacion.ToUpper()));
        dPX = 280;
        Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, sLocalidadSucursal));
        dposRen += dcalRen;
        dPX = 120;
        Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, "TEL.(614) 426 55 45"));
        dPX = 280;
        Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, "TEL.(625) 583 08 85"));
        dposRen += dcalRen;
         */
        ...
        double zY = 12;
        double zAltoPnl = 100;
        fnCrearPanelRedondeado(Encabezado, Encabezado.rWidth - 190, zY, fAnchoPanel + 75, fAltoPanel + zAltoPnl, grosorPen, radioCurva, false, sColor);
        ...
        fnCrearPanelRedondeado(Encabezado, Encabezado.rWidth - 190, zY, fAnchoPanel + 75, fAltoPanel -18, grosorPen, radioCurva, true, sColor);

        //fnAgregarMultilineaCentrada(Encabezado, TipoDocumento, fPropBlanca, posX + fAnchoPanel / 2, fAltoPanel / 2, 15);
        fPropBlanca.rSize = 4;
        int jjj = 25;
        fnAgregarMultilineaCentrada(Encabezado, TipoDocumento, fPropBlanca, posX + fAnchoPanel / 6, fAltoPanel -jjj / 2, 15);
        ...
        posX = 475; //Encabezado.rWidth - leftPadding;
        double dY = 0;
        double dYt = 0.9;
        double dYPnl = 33;
        //Folio
        ...
        {
            Encabezado.AddAligned(posX, RepObj.rAlignCenter,
                    fAltoPanel * dYt + dY, RepObj.rAlignTop,
                new RepString(fPropRoja, folio));
        }
        dY = 10;
        fnCrearPanelRedondeado(Encabezado, Encabezado.rWidth - 190, dYPnl, fAnchoPanel + 75, fAltoPanel - 18, grosorPen, radioCurva, true, sColor);
        //Folio Fiscal
        if (!string.IsNullOrEmpty(sUUID))
        {
            Encabezado.AddAligned(posX, RepObj.rAlignCenter,
                fAltoPanel * dYt + dY, RepObj.rAlignTop,
                new RepString(fPropBlanca, "Folio Fiscal"));
            dY += 11;
            Encabezado.AddAligned(posX, RepObj.rAlignCenter,
                fAltoPanel * dYt + dY, RepObj.rAlignTop,
        ...
        if (!string.IsNullOrEmpty(noCertificadoEmisor))
        {
            Encabezado.AddAligned(posX, RepObj.rAlignCenter,
                fAltoPanel * dYt + dY, RepObj.rAlignTop,
                new RepString(fPropBlanca, "No. de Serie del Certificado del Emisor"));
            dY += 11;
            Encabezado.AddAligned(posX, RepObj.rAlignCenter,
                fAltoPanel * dYt + dY, RepObj.rAlignTop,
        ...
        if (!string.IsNullOrEmpty(noCertificadoSAT))
        {
            Encabezado.AddAligned(posX, RepObj.rAlignCenter,
                fAltoPanel * dYt + dY, RepObj.rAlignTop,
                new RepString(fPropBlanca, "No. de Serie del Certificado del SAT"));
            dY += 11;
            Encabezado.AddAligned(posX, RepObj.rAlignCenter,
                fAltoPanel * dYt + dY, RepObj.rAlignTop,
        ...
        if (fechaTimbrado != DateTime.MinValue)//(fechaComprobante != DateTime.MinValue)
        {
            Encabezado.AddAligned(posX, RepObj.rAlignCenter,
                fAltoPanel * dYt + dY, RepObj.rAlignTop,
                new RepString(fPropBlanca, "Fecha y Hora de Certificación"));
            dY += 11;
            Encabezado.AddAligned(posX, RepObj.rAlignCenter,
                fAltoPanel * dYt + dY, RepObj.rAlignTop,
        ...
        if (fechaComprobante != DateTime.MinValue)
        {
            Encabezado.AddAligned(posX, RepObj.rAlignCenter,
                fAltoPanel * dYt + dY, RepObj.rAlignTop,
                new RepString(fPropBlanca, "Lugar, Fecha y Hora de Emisión:"));
            dY += 12;
            Encabezado.AddAligned(posX, RepObj.rAlignCenter,
                fAltoPanel * dYt + dY, RepObj.rAlignTop,
    ...
    private void fnDatosReceptor(XmlNamespaceManager nsmComprobante, XPathNavigator navEncabezado, StaticContainer Encabezado, double Renglon)
	{
        ...
        //Datos del receptor 
        Encabezado.AddAligned(leftPadding - dPXt, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNegrita, "CLIENTE"));
        Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, razonSocial));
        //salto de renglon 
        dcalRen = 0.3;
        dposRen += dcalRen;
        Encabezado.AddAligned(leftPadding - dPXt, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNegrita, "R.F.C."));
        ...
        dposRen += dcalRen;
        Encabezado.AddAligned(leftPadding - dPXt, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNegrita, "DOMICILIO"));
        ...
        dposRen += dcalRen;
        Encabezado.AddAligned(leftPadding - dPXt, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNegrita, "COLONIA"));
        Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, colonia));
        dposRen += dcalRen;
        Encabezado.AddAligned(leftPadding - dPXt, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNegrita, "CIUDAD"));
        ...
        dPX = 20;
        Encabezado.AddAligned(leftPadding * dPX, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNegrita, "PAIS"));
        dPX = 23;
        Encabezado.AddAligned(leftPadding * dPX, RepObj.rAlignLeft, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, pais));
    ...
    private RepImage fnMarcaAguaCorpus()
    {
        ...
        {
            clsErrorLogBlancosJessy.fnNuevaEntrada(ex, clsErrorLogBlancosJessy.TipoErroresLog.Referencia);
        }
...
public class TransformacionLogo
{
    ...
    public static string fnConstruirCadenaTimbrado(IXPathNavigable xml, string psNombreArchivoXSLT)
    {
        ...
        try
        {
            ...
            trans.Load(XmlReader.Create(new StringReader(clsComunBlancosJessy.ObtenerParamentro(psNombreArchivoXSLT))));
        ...
        {
            clsErrorLogBlancosJessy.fnNuevaEntrada(ex, clsErrorLogBlancosJessy.TipoErroresLog.Datos);
        }
...
public class clsErrorLogBlancosJessy
{
    public clsErrorLogBlancosJessy()
    {
    }
...
public class clsComunBlancosJessy
{
    public static string ObtenerParamentro(string sParametro)
    {
        ...
        using (SqlConnection con = new SqlConnection(PAXCrypto.CryptoAES.DesencriptaAES64(ConfigurationManager.ConnectionStrings["conControl"].ConnectionString)))
        {
            ...
            {
                con.Close();
                clsErrorLogBlancosJessy.fnNuevaEntrada(ex, clsErrorLogBlancosJessy.TipoErroresLog.Conexion);
            }
        }
        return sRetorno;
    }
