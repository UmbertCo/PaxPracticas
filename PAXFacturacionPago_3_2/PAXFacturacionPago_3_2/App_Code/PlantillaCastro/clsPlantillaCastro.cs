...
/// </summary>
public class clsPlantillaCastro
{
    #region atributos
    ...
    public clsPlantillaCastro(XmlDocument pxComprobante)
	{
	...
	public void fnGenerarPDF(int id_contribuyente,int sid_rfc,string scolor)
	{
		...
		while (bSeguir)
		{
            ...
            if (ms.Length > 0)
            {
                ...
                int sX = 503;
                double sY = 702;
                pagina.Add(margenPagina + sX, margenPagina + sY, image);
            }

            //Se carga imagen logo pie
            ms = fnImagenClienteDir("BarraCastro.jpg");
            if (ms.Length > 0)
            {
                double dAlt = 40;
                double dAnc = 570;
	...
	{
        double menos = 70;
		StaticContainer areaDetalle = new StaticContainer(RT.rPointFromMM(anchoPagina) - margenPagina * 2 - menos , RT.rPointFromMM(altoPagina - altoEncabezado - altoPie) - margenPagina * 2);
	...
	{
        double dmenos = 42;
		StaticContainer Pie = new StaticContainer(RT.rPointFromMM(anchoPagina) - margenPagina * 2 - dmenos, RT.rPointFromMM(altoPie));
	...
	private void fnTotales(XmlNamespaceManager nsmComprobante, XPathNavigator navPie, StaticContainer Pie, int nNumPag, int nTotPag, string sColor)
	{
        ...
        double horPadding = Pie.rWidth * 0.01;//480
        double posPanelTotales = Pie.rWidth - 220;//310
        //if (nNumPag == nTotPag)//Si es ultima pagina
        //{
            ...
            Pie.AddAligned(posPanelTotales + 170, RepObj.rAlignRight, verPadding, RepObj.rAlignTop, new RepString(fPropNormal, TransformacionLogo.fnFormatoCurrency(subtotal)));
        //}
        ...
        //{
            foreach (ImpuestoLogo i in impuestos)
            {
                if (i.Nombre != "IEPS")
                {
                    ...
                    Pie.AddAligned(posPanelTotales + 170, RepObj.rAlignRight, verPadding + altoRenglon * renglon, RepObj.rAlignTop, new RepString(fPropNormal, TransformacionLogo.fnFormatoCurrency(i.Importe)));
                    renglon++;
                }
            }
            ...
            Pie.AddAligned(posPanelTotales + 170, RepObj.rAlignRight, tamCodigo - altoRenglon, RepObj.rAlignTop, new RepString(fPropNormal, TransformacionLogo.fnFormatoCurrency(total)));

            //Agregamos el separador para los sellos
        ...
        //{
            ...
            renglon += fnAgregarMultilinea(Pie, sello, fPropChica, horPadding, tamCodigo + altoRenglon * renglon, nTamRenglon, false);

            Pie.Add(horPadding, tamCodigo + altoRenglon * renglon++, new RepString(fPropNormal, "Sello digital del SAT:"));
            renglon += fnAgregarMultilinea(Pie, timbre, fPropChica, horPadding, tamCodigo + altoRenglon * renglon, nTamRenglon, false);
            ...
            renglon = fnAgregarMultilinea(Pie, TransformacionLogo.fnConstruirCadenaTimbrado(gxComprobante.CreateNavigator().SelectSingleNode("/cfdi:Comprobante/cfdi:Complemento/tfd:TimbreFiscalDigital", nsmComprobante).CreateNavigator(), "cadenaoriginal_TFD_1_0")
                , fPropChica, horPadding, tamCodigo + altoRenglon * renglon, nTamRenglon, false);
        ...
        Pie.AddAligned(Pie.rWidth / 6.5, RepObj.rAlignCenter, Pie.rHeight - verPadding, RepObj.rAlignTop, new RepString(fPropNormal, clsComunPDF.ObtenerParamentro("urlHostComercial")));
    ...
    private void fnDibujarTitulosDetalle(StaticContainer Encabezado, System.Drawing.Color sColor)
    {
        ...
        double posColumna1 = Encabezado.rWidth * 0.006;     //clave
        double posColumna2 = Encabezado.rWidth * 0.11;      //unidad
        ...
        Encabezado.AddAligned(posColumna4, RepObj.rAlignLeft, puntoMedio, RepObj.rAlignCenter, new RepString(fPropNegrita, "CANTIDAD"));
        Encabezado.AddAligned(posColumna5, RepObj.rAlignLeft, puntoMedio, RepObj.rAlignCenter, new RepString(fPropNegrita, "PRECIO"));
        Encabezado.AddAligned(posColumna6, RepObj.rAlignLeft, puntoMedio, RepObj.rAlignCenter, new RepString(fPropNegrita, "TOTAL"));
    }
    ...
    private void fnDatosEmisor(XmlNamespaceManager nsmComprobante, XPathNavigator navEncabezado, StaticContainer Encabezado, System.Drawing.Color sColor, ref double Renglon)
	{
        ...
        //Datos del emisor titulo

        Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignCenter, posRenglon - 9, RepObj.rAlignTop, new RepString(fPropTitulo, razonSocial));
        ...
        dposRen += dcalRen;
        Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignCenter, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, ubicacion2));//tamRenglon * 2.1,
        dposRen += dcalRen;
        ...
        posX = 3;//Encabezado.rWidth - leftPadding;
    ...
    private void fnDatosReceptor(XmlNamespaceManager nsmComprobante, XPathNavigator navEncabezado, StaticContainer Encabezado, double Renglon)
	{
        ...
        double leftPadding = 3;// Encabezado.rWidth * 0.02;
        double sep = 5;//10
        ...
        i = fnAgregarMultilinea(Encabezado, razonSocial, fPropNormal, leftPadding + sX, posRenglon + tamRenglon * Renglon++, 90, true);
        Renglon += i;
        if (i > 1)
            Renglon -= 1.5;
        else
        ...
        if (i > 1)
            ...
            Renglon -= 1;
        i = fnAgregarMultilinea(Encabezado, rfc, fPropNormal, leftPadding + sX, posRenglon + tamRenglon * Renglon++, 90, true);
        ...
        if (i > 1)
            ...
            Renglon -= 1;
        i = fnAgregarMultilinea(Encabezado, direccion, fPropNormal, leftPadding + sX, posRenglon + tamRenglon * Renglon, 90, true);
        Renglon += i;
        if (i > 1)
            Renglon -= 0.5;
        i = fnAgregarMultilinea(Encabezado, colonia, fPropNormal, leftPadding + sX, posRenglon + tamRenglon * Renglon, 90, true);
        Renglon += i;
        if (i > 1)
            Renglon -= 0.5;
        i = fnAgregarMultilinea(Encabezado, codigoPostal +" "+ ubicacion, fPropNormal, leftPadding + sX, posRenglon + tamRenglon * Renglon, 90, true);
...
public class clsErrorLogCastro
{
    //private static clsInicioSesionUsuario gUsuario;

    public clsErrorLogCastro()
    {
    }
...
public class clsComunLogo
{
    public static string ObtenerParamentro(string sParametro)
    {
        ...
        using (SqlConnection con = new SqlConnection(PAXCrypto.CryptoAES.DesencriptaAES64(ConfigurationManager.ConnectionStrings["conControl"].ConnectionString)))
        {
            ...
            {
                con.Close();
                clsErrorLogCastro.fnNuevaEntrada(ex, clsErrorLogCastro.TipoErroresLog.Conexion);
            }
        }
        return sRetorno;
    }
