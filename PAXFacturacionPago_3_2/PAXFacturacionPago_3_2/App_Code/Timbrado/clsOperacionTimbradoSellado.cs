...
public class clsOperacionTimbradoSellado: IDisposable
{
	...
	public clsOperacionTimbradoSellado(byte[] pbLlave, byte[] psPassword)
	{
		gbLlave = pbLlave;
	...
	public string fnGenerarSello(string psCadenaOriginal, AlgoritmoSellado pAlgoritmo, bool pbDesencriptar)
	{
		try
		{
			...
			if(pbDesencriptar)
				...
				key.LoadPkcs8Encrypted(gbLlave, System.Text.Encoding.Unicode.GetString(gsPassword));
	...
	public byte[] fnEncriptarLlave()
	{
        ...
        return PAXCrypto.CryptoAES.EncriptaAESB(gbLlave);
	}
	...
	private byte[] fnDesencriptarLlave(byte[] pbLlave)
	{
        ...
        return PAXCrypto.CryptoAES.DesencriptaAESB(pbLlave);
	}
    ...
    //    return Convert.ToBase64String((PAXCrypto.CryptoAES.EncriptaAESB(Encoding.UTF8.GetBytes(gsPassword))));
    //}

    public byte[] fnEncriptarPassword()
    {
        //return Convert.ToBase64String((Utilerias.Encriptacion.DES3.Encriptar(Encoding.UTF8.GetBytes(gsPassword))));
        return PAXCrypto.CryptoAES.EncriptaAESB(gsPassword);
    }
