...
/// <summary>
/// Summary description for clsPlantillaAlbertoGutierrez
/// </summary>
public class clsPlantillaAlbertoGutierrez
{
    ...
    public clsPlantillaAlbertoGutierrez(XmlDocument pxComprobante)
    {
    ...
    public void fnGenerarPDF(int id_contribuyente, int sid_rfc, string scolor)
    {
        ...
        while (bSeguir)
        {
            ...
            if (ms.Length > 0)
            {
                RepImage image = new RepImage(ms, 120, 41);
    ...
    private void fnTotales(XmlNamespaceManager nsmComprobante, XPathNavigator navPie, StaticContainer Pie)
    {
        ...
        metodoPago = navPie.SelectSingleNode("/cfdi:Comprobante/@metodoDePago", nsmComprobante).Value;
    ...
    {
        string razonSocial, rfc, calle, noExterior, noInterior, colonia, municipio, estado, pais, codigoPostal, serie, folio, fecha, noCertificadoEmisor, referencia, Localidad, fechaTimb;
        razonSocial = rfc = calle = noExterior = noInterior = colonia = municipio = estado = pais = codigoPostal = fecha = serie = folio = noCertificadoEmisor = referencia = Localidad  = fechaTimb = string.Empty;
...
public class clsErrorAlbertoGutierrez
{
    //private static clsInicioSesionUsuario gUsuario;

    public clsErrorAlbertoGutierrez()
    {
    }
...
public class clsComunLogo
{
    public static string ObtenerParamentro(string sParametro)
    {
        ...
        using (SqlConnection con = new SqlConnection(PAXCrypto.CryptoAES.DesencriptaAES64(ConfigurationManager.ConnectionStrings["conControl"].ConnectionString)))
        {
            ...
            {
                con.Close();
                clsErrorAlbertoGutierrez.fnNuevaEntrada(ex, clsErrorAlbertoGutierrez.TipoErroresLog.Conexion);
            }
        }
        return sRetorno;
    }
