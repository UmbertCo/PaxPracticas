...
/// </summary>
public class clsPlantillaTermoClimas
{
    #region atributos
    ...
    //tamaños en puntos
	private const double altoEncabezado = 55;
	private double altoPie = 110;
    ...
    public clsPlantillaTermoClimas(XmlDocument pxComprobante)
	{
	...
	public void fnGenerarPDF(int id_contribuyente,int sid_rfc,string scolor)
	{
		...
		while (bSeguir)
		{
            ...
            if (ms.Length > 0)
            {
                ...
                double dPX = 16;
                double dPY = 89;
                pagina.Add(margenPagina + dPX, margenPagina + dPY, image);
            }
	...
	private StaticContainer fnCrearDetalle(List<DetalleLogo> detalles, System.Drawing.Color sColor)
	{
        ...
        for (int i = 0; renglonActual <= maxRenglones; i++)
		{
            ...
            //Acomodamos la descripción en tantos renglones como necesite
            renglonActual += fnAgregarMultilineaDetalle(areaDetalle, d.descripcion, fPropNormal, posColumna3, posRenglon, 52, true);
	...
	private void fnTotales(XmlNamespaceManager nsmComprobante, XPathNavigator navPie, StaticContainer Pie, int nNumPag, int nTotPag, string sColor)
	{
        ...
        //{
            ...
            fnAgregarMultilinea(Pie, formaDePago, fPropChica, tamCodigo + horPadding, altoRenglon * 3.9, 80, true);
            ...
            if (version == "3.2") //Si es version 3.2 muestra el siguiente contenido
            {
                ...
                {
                    fnAgregarMultilinea(Pie, "Método de Pago:", fPropNegrita, tamCodigo + horPadding, altoRenglon * 4.6, 80, true);
                    fnAgregarMultilinea(Pie, metodoPago, fPropChica, tamCodigo + horPadding, altoRenglon * 5.3, 80, true);

                    //Número de cuenta
                    if (!string.IsNullOrEmpty(sNumCtaPago))
                    {
                        fnAgregarMultilinea(Pie, "Número de Cuenta:", fPropNegrita, tamCodigo + horPadding, altoRenglon * 6, 80, true);
                        fnAgregarMultilinea(Pie, sNumCtaPago, fPropChica, tamCodigo + horPadding, altoRenglon * 6.7, 80, true);
                    }
                }
                else
                {
                    ...
                    if (!string.IsNullOrEmpty(sNumCtaPago))
                    {
                        fnAgregarMultilinea(Pie, "Número de Cuenta:", fPropNegrita, tamCodigo + horPadding, altoRenglon * 4.6, 80, true);
                        fnAgregarMultilinea(Pie, sNumCtaPago, fPropChica, tamCodigo + horPadding, altoRenglon * 5.3, 80, true);
                    }
                }
            }
        //}
        ...
        //{
            int nTRen = 165;
            double dRenglon = 0.8;
            Pie.Add(horPadding, tamCodigo + altoRenglon * dRenglon++, new RepString(fPropNormal, "Sello digital del Emisor:"));
            dRenglon -= 0.5;
            dRenglon += fnAgregarMultilinea(Pie, sello, fPropChica, horPadding, tamCodigo + altoRenglon * dRenglon, nTRen, false);
            dRenglon -= 1;
            Pie.Add(horPadding, tamCodigo + altoRenglon * dRenglon++, new RepString(fPropNormal, "Sello digital del SAT:"));
            dRenglon -= 0.5;
            dRenglon += fnAgregarMultilinea(Pie, timbre, fPropChica, horPadding, tamCodigo + altoRenglon * dRenglon, nTRen, false);

            dRenglon -= 1;
            //Agregamos la cadena original y alargamos la sección del pie según sea necesario
            Pie.Add(horPadding, tamCodigo + altoRenglon * dRenglon++, new RepString(fPropNormal, "Cadena original del complemento de certificación digital del SAT:"));
            dRenglon -= 0.5;
            renglon = fnAgregarMultilinea(Pie, TransformacionLogo.fnConstruirCadenaTimbrado(gxComprobante.CreateNavigator().SelectSingleNode("/cfdi:Comprobante/cfdi:Complemento/tfd:TimbreFiscalDigital", nsmComprobante).CreateNavigator(), "cadenaoriginal_TFD_1_0")
                , fPropChica, horPadding, tamCodigo + altoRenglon * dRenglon, nTRen, false);

            if (nNumPag == nTotPag)//Solo se modifica una vez 
            {
                Pie.rHeight += renglon * (fPropChica.rSize * 1.2);
                altoPie = Pie.rHeightMM;
            }
        ...
        double dAltPSello = 152;
        fnCrearPanelRedondeado(Pie, 0, tamCodigo, Pie.rWidth, Pie.rHeight - tamCodigo - altoRenglon - dAltPSello, grosorPen, radioCurva, false, ColorT);
        
        //*****************************************************************************************************************************************************************
        //|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||PANEL PAGARE||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
        //*****************************************************************************************************************************************************************
        double dRenPag = 6.8;
        fPropNegrita.rSize = 10;
        Pie.Add(horPadding, tamCodigo + altoRenglon * dRenPag, new RepString(fPropNegrita, "PAGARÉ"));
        ...
        double dXPSI = 70;
        //Panel superior izq
        fnCrearPanelRedondeado(Pie, dXPSI, tamCodigo + dYPSI, Pie.rWidth - dAncPSI, Pie.rHeight - tamCodigo - altoRenglon - dAltPSI, grosorPen, radioCurva, false, ColorT);

        double dXExp = 155;
        fPropChica.rSize = 3.5;
        dRenPag -= 0.5;
        Pie.Add(horPadding + dXExp, tamCodigo + altoRenglon * dRenPag, new RepString(fPropChica, "EXPEDICIÓN"));
        dXExp += 65;
        Pie.Add(horPadding + dXExp, tamCodigo + altoRenglon * dRenPag, new RepString(fPropChica, "LUGAR"));

        dXExp += 70;
        Pie.Add(horPadding + dXExp, tamCodigo + altoRenglon * dRenPag, new RepString(fPropChica, "DIA"));

        dXExp += 30;
        Pie.Add(horPadding + dXExp, tamCodigo + altoRenglon * dRenPag, new RepString(fPropChica, "MES"));

        dXExp += 30;
        Pie.Add(horPadding + dXExp, tamCodigo + altoRenglon * dRenPag, new RepString(fPropChica, "AÑO"));

        //Se reinicia tamaño
        fPropChica.rSize = 4;
        double dAncPSM = 330;
        double dXPSM = 148;
        //Panel superior medio
        fnCrearPanelRedondeado(Pie, dXPSM, tamCodigo + dYPSI, Pie.rWidth - dAncPSM, Pie.rHeight - tamCodigo - altoRenglon - dAltPSI, grosorPen, radioCurva, false, ColorT);

        fPropNegrita.rSize = 8;
        double dXImp = 400;
        dRenPag = 6.8;
        Pie.Add(horPadding + dXImp, tamCodigo + altoRenglon * dRenPag, new RepString(fPropNegrita, "IMPORTE"));

        double dAncPSD = 402;
        double dXPSD = 397;
        //Panel superior derecho
        fnCrearPanelRedondeado(Pie, dXPSD, tamCodigo + dYPSI, Pie.rWidth - dAncPSD, Pie.rHeight - tamCodigo - altoRenglon - dAltPSI, grosorPen, radioCurva, false, ColorT);

        dRenPag += 1.5;
        fnAgregarMultilinea(Pie, "Debo (emos) y pagare (mos) incondicionalmente a la orden de TERMO CLIMAS DE TIJUANA, S.A. DE C.V. en el lugar y fecha citados, la cantidad de:", fPropNormal, horPadding, tamCodigo + altoRenglon * dRenPag, nTRen, false);

        fPropNegrita.rSize = 6;
        dXExp = 210;
        dRenPag = 11;
        Pie.Add(horPadding + dXExp, tamCodigo + altoRenglon * dRenPag, new RepString(fPropNormal, "I M P O R T E   C O N    L E T R A"));
        //Panel medio
        double dAltPM = 188;
        double dAncPM = 10;
        double dYPM = 108;
        double dXPM = 5.72;
        fnCrearPanelRedondeado(Pie, dXPM, tamCodigo + dYPM, Pie.rWidth - dAncPM, Pie.rHeight - tamCodigo - altoRenglon - dAltPM, grosorPen, radioCurva, false, ColorT);

        dRenPag = 12.3;
        nTRen = 175;
        fnAgregarMultilinea(Pie, "Valor recibido a mi (nuestra) entera sastifacción. Este PAGARE forma parte de una serie numerada del 1 al _________ y todos están sujetos a la condición " +
        "de que, al no pagarse cualquiera de ellos a su vencimiento, serán exigibles todos los que le sigan en número, además de los ya vencidos, desde la fecha " +
        "de vencimiento hasta su liquidación, causará intereses moratorios al tipo de _____% mensual, pagadero en esta ciudad justamente con el principal", fPropNormal, horPadding, tamCodigo + altoRenglon * dRenPag, nTRen, false);
        ...
        double dYPII = 170;
        double dXPII = 5.72;
        fnCrearPanelRedondeado(Pie, dXPII, tamCodigo + dYPII, Pie.rWidth - dAncPII, Pie.rHeight - tamCodigo - altoRenglon - dAltPII, grosorPen, radioCurva, false, ColorT);

        dXExp = 264;
        dRenPag = 14.3;
        Pie.Add(horPadding + dXExp, tamCodigo + altoRenglon * dRenPag, new RepString(fPropChica, "C O N T R A T O"));
        //Panel inferior medio - arriba
        double dAltPIMA = 200;
        double dAncPIMA = 388;
        double dXPIMA = 204;
        double dYPIMA = 170;
        fnCrearPanelRedondeado(Pie, dXPIMA, tamCodigo + dYPIMA, Pie.rWidth - dAncPIMA, Pie.rHeight - tamCodigo - altoRenglon - dAltPIMA, grosorPen, radioCurva, false, ColorT);

        dXExp = 218;
        dRenPag = 16.4;
        Pie.Add(horPadding + dXExp, tamCodigo + altoRenglon * dRenPag, new RepString(fPropChica, "V E N C I M I E N T O                        DIA/MES/AÑO"));

        //Panel inferior medio - abajo
        double dYPIMB = 194;
        fnCrearPanelRedondeado(Pie, dXPIMA, tamCodigo + dYPIMB, Pie.rWidth - dAncPIMA, Pie.rHeight - tamCodigo - altoRenglon - dAltPIMA, grosorPen, radioCurva, false, ColorT);

        dXExp = 450;
        dRenPag = 14.3;
        Pie.Add(horPadding + dXExp, tamCodigo + altoRenglon * dRenPag, new RepString(fPropChica, "ACEPTO (AMOS)"));

        dXExp = 460;
        dRenPag = 17;
        Pie.Add(horPadding + dXExp, tamCodigo + altoRenglon * dRenPag, new RepString(fPropChica, "FIRMA(S)"));

        //Borde antes de firma
        double dYBF = 212;
        double dXAncBF = 17;
        Pie.Add(posPanelTotales + horPadding + 10, tamCodigo - altoRenglon + dYBF, new RepLine(new PenProp(PDF, grosorPen, ColorT), Pie.rWidth - posPanelTotales - horPadding * 2 - dXAncBF, 0));
        ...
        double dYPID = 170;
        double dXPID = 396;
        fnCrearPanelRedondeado(Pie, dXPID, tamCodigo + dYPID, Pie.rWidth - dAncPID, Pie.rHeight - tamCodigo - altoRenglon - dAltPID, grosorPen, radioCurva, false, ColorT);
	...
	private void fnDatosEmisor(XmlNamespaceManager nsmComprobante, XPathNavigator navEncabezado, StaticContainer Encabezado, System.Drawing.Color sColor)
	{
        ...
        rfc = "RFC: " + "TCT930917JG8"; //navEncabezado.SelectSingleNode("/cfdi:Comprobante/cfdi:Emisor/@rfc", nsmComprobante).Value;
        ...
        try { Regimenfiscal = "Régimen general de ley personas morales"; } //navEncabezado.SelectSingleNode("/cfdi:Comprobante/cfdi:Emisor/cfdi:RegimenFiscal/@Regimen", nsmComprobante).Value; }
        catch { }
        ...
        codigoPostal = "22116"; //navEncabezado.SelectSingleNode("/cfdi:Comprobante/cfdi:Emisor/cfdi:DomicilioFiscal/@codigoPostal", nsmComprobante).Value;
        ...
        if (!string.IsNullOrEmpty(colonia))
            coloniaemisor += "Colonia " + colonia; //direccion += " Colonia " + colonia;
        ...
        double dPX = 250;
        //Encabezado.Add(leftPadding + dPX, posRazon, new RepString(fPropTitulo, "TERMO CLIMAS DE TIJUANA S.A DE C.V.")); //razonSocial));
        //Encabezado.Add(leftPadding + dPX, posRenglon + tamRenglon * 0.5, new RepString(fPropNormal, "E-mail: teclimas@telnor.net"));
        //Encabezado.Add(leftPadding + dPX, posRenglon + tamRenglon * 1.3, new RepString(fPropNormal, direccion));
        //Encabezado.Add(leftPadding + dPX, posRenglon + tamRenglon * 2.1, new RepString(fPropNormal, coloniaemisor));
        //Encabezado.Add(leftPadding + dPX, posRenglon + tamRenglon * 2.9, new RepString(fPropNormal, ubicacion));
        //Encabezado.Add(leftPadding + dPX, posRenglon + tamRenglon * 3.7, new RepString(fPropNormal, ubicacion2));
        //Encabezado.Add(leftPadding + dPX, posRenglon + tamRenglon * 4.5, new RepString(fPropNormal, rfc));
        fPropTitulo.rSize = 8;
        double dposRen = 2.9;
        double dcalRen = 0.8;
        Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignCenter, posRazon - 3, RepObj.rAlignTop, new RepString(fPropTitulo, "TERMO CLIMAS DE TIJUANA S.A DE C.V."));
        Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignCenter, posRenglon + tamRenglon * 0.5, RepObj.rAlignTop, new RepString(fPropNegrita, "VENTA DE EQUIPOS, SERVICIOS E INSTALACIONES DE SISTEMAS"));
        Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignCenter, posRenglon + tamRenglon * 1.3, RepObj.rAlignTop, new RepString(fPropNegrita, "DE AIRE ACONDICIONADO, CALEFACCIÓN Y CUARTOS LIMPIOS"));
        Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignCenter, posRenglon + tamRenglon * 2.1, RepObj.rAlignTop, new RepString(fPropNegrita, "E-mail: teclimas@prodigy.net.mx"));
        Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignCenter, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, "CALLE \"C\" Y AV. 33 No. 166, Colonia FRACC. RUBIO")); //direccion + ", " + coloniaemisor));
        dposRen += dcalRen;
        //if (!string.IsNullOrEmpty(ubicacion))
        //{
        Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignCenter, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, "LA MESA, TIJUANA, Baja California. México C.P. " + codigoPostal)); //ubicacion + ubicacion2));
            //dposRen += dcalRen;
        ...
        //}
        dposRen += dcalRen;
        Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignCenter, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, "TELS: (664) 689 0561, (664) 689 2941" + ", " + rfc));
        ...
        if (!string.IsNullOrEmpty(referencia))
        {
            ...
            //Encabezado.Add(leftPadding + dPX, posRenglon + tamRenglon * 5.3, new RepString(fPropNormal, Refer));
            ...
            if (version == "3.2")
            {
                ...
                //Encabezado.Add(leftPadding + dPX, posRenglon + tamRenglon * 6.1, new RepString(fPropNormal, sRegFis));
                //Encabezado.Add(leftPadding + dPX, posRenglon + tamRenglon * 6.9, new RepString(fPropNormal, Regimenfiscal));
                dposRen += dcalRen;
                //Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignCenter, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, sRegFis));
                //dposRen += dcalRen;
                //Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignCenter, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, Regimenfiscal));
                Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignCenter, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, sRegFis + " " + Regimenfiscal));
            }
        }
        else
        {
            ...
            {
                sRegFis = "Regimen Fiscal:";
                //Encabezado.Add(leftPadding + dPX, posRenglon + tamRenglon * 5.3, new RepString(fPropNormal, sRegFis));
                //Encabezado.Add(leftPadding + dPX, posRenglon + tamRenglon * 6.1, new RepString(fPropNormal, Regimenfiscal));
                Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignCenter, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, sRegFis + " " + Regimenfiscal));
                //dposRen += dcalRen;
                //Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignCenter, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, Regimenfiscal));
            }
        ...
        fnCrearPanelRedondeado(Encabezado, Encabezado.rWidth - dposXpnl, 0, fAnchoPanel + dAncPnl, fAltoPanel, grosorPen, radioCurva, false, sColor);
        fnCrearPanelRedondeado(Encabezado, Encabezado.rWidth - dposXpnl, fAltoPanel, fAnchoPanel + dAncPnl, fAltoPanel, grosorPen, radioCurva, false, sColor);
	...
	private void fnDatosReceptor(XmlNamespaceManager nsmComprobante, XPathNavigator navEncabezado, StaticContainer Encabezado)
	{
        ...
        Encabezado.Add(leftPadding, posRazon + tamRenglon + 7, new RepString(fPropTitulo, razonSocial)); //posRazon + 30
        Encabezado.Add(leftPadding, posRenglon + tamRenglon * 2.0, new RepString(fPropNormal, direccion)); //* 2
        Encabezado.Add(leftPadding, posRenglon + tamRenglon * 2.8, new RepString(fPropNormal, ubicacion)); //*2.5
        Encabezado.Add(leftPadding, posRenglon + tamRenglon * 3.6, new RepString(fPropNormal, rfc)); //*3
        ...
        double fAltoPanel = 50; //Encabezado.rHeight / 6;
        double fAnchoPanel = 200; //Encabezado.rWidth / 5;
        double dAncPnl = 180;
        double dposYpnl = 93;
        System.Drawing.ColorConverter colConvert = new ColorConverter();
        System.Drawing.Color sColor = new System.Drawing.Color();
        sColor = (System.Drawing.Color)colConvert.ConvertFromString("Black");

        fnCrearPanelRedondeado(Encabezado, 0, dposYpnl, fAnchoPanel + dAncPnl, fAltoPanel, grosorPen, radioCurva, false, sColor);
	}
    ...
    private void fnAgregarBordeRedondeado(StaticContainer poObjeto, double pfGrosor, double pfRadioCurva, System.Drawing.Color sColor)
    {
        ...
        //Borde medio en pie de pagina en la parte de expedición
        double dWBD1 = 300;
        double dYBD1 = 431;
        double dYB2 = 248;
        poObjeto.Add(poObjeto.rWidth - dWBD1, pfRadioCurva + dYBD1, new RepLine(pen, 0, pfRadioCurva * 2 - poObjeto.rHeight + dYB2));

        //Borde derecho en pie de pagina en la parte de importe
        double dWBSD = 95;
        double dYBSD = 431;
        double dYBSD2 = 248;
        poObjeto.Add(poObjeto.rWidth - dWBSD, pfRadioCurva + dYBSD, new RepLine(pen, 0, pfRadioCurva * 2 - poObjeto.rHeight + dYBSD2));
    }
...
public class clsErrorLogTermoClimas
{
    //private static clsInicioSesionUsuario gUsuario;

    public clsErrorLogTermoClimas()
    {
    }
...
public class clsComunLogo
{
    public static string ObtenerParamentro(string sParametro)
    {
        ...
        using (SqlConnection con = new SqlConnection(PAXCrypto.CryptoAES.DesencriptaAES64(ConfigurationManager.ConnectionStrings["conControl"].ConnectionString)))
        {
            ...
            {
                con.Close();
                clsErrorLogTermoClimas.fnNuevaEntrada(ex, clsErrorLogTermoClimas.TipoErroresLog.Conexion);
            }
        }
        return sRetorno;
    }
