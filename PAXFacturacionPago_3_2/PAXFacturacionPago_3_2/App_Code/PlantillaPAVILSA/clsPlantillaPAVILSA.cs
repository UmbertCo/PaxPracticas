...
/// <summary>
/// Summary description for clsPlantillaPAVILSA
/// </summary>
public class clsPlantillaPAVILSA
{
    #region atributos
    ...
    public clsPlantillaPAVILSA(XmlDocument pxComprobante)
	{
        try
        {
    ...
    public void fnGenerarPDF(int id_contribuyente, int sid_rfc, string scolor)
    {
        ...
        List<DetallePAVILSA> detallesNum = fnObtenerDetalles();
        ...
        List<DetallePAVILSA> detalles = fnObtenerDetalles();
    ...
    /// <returns></returns>
    private StaticContainer fnContarPaginas(List<DetallePAVILSA> detalles)
    {
        ...
        DetallePAVILSA[] copiaDetalles = detalles.ToArray();
        DetallePAVILSA d;
        double posRenglon;
    ...
    /// <returns></returns>
    private StaticContainer fnCrearDetalle(List<DetallePAVILSA> detalles, System.Drawing.Color sColor)
    {
        StaticContainer areaDetalle = new StaticContainer(RT.rPointFromMM(anchoPagina) - margenPagina * 2, RT.rPointFromMM(altoPagina - altoEncabezado - altoPie) - margenPagina * 2);
    ...
    private List<DetallePAVILSA> fnObtenerDetalles()
    {
        XmlNamespaceManager nsmComprobante = new XmlNamespaceManager(gxComprobante.NameTable);
        nsmComprobante.AddNamespace("cfdi", "http://www.sat.gob.mx/cfd/3");

        List<DetallePAVILSA> detalles = new List<DetallePAVILSA>();
        ...
        {
            detalles.Add(new DetallePAVILSA(navDetalles.Current, nsmComprobante));
        }
    ...
    private void fnTotales(XmlNamespaceManager nsmComprobante, XPathNavigator navPie, StaticContainer Pie, int nNumPag, int nTotPag, string sColor)
    {
        ...
        double subtotal_extra = 0;

        List<ImpuestoPAVILSA> impuestos = new List<ImpuestoPAVILSA>();
        ...
        subtotal_extra = Convert.ToDouble(subtotal);
        ...
        if (nNumPag == nTotPag)//Si es ultima pagina
        {
            ...
            {
                impuestos.Add(new ImpuestoPAVILSA(navImpuestos.Current, nsmComprobante));
            }
        ...
        double verPadding = Pie.rHeight * 0.01;
        ...
        if (nNumPag == nTotPag)//Si es ultima pagina
        {
            ...
            Pie.AddAligned(Pie.rWidth - horPadding, RepObj.rAlignRight, verPadding, RepObj.rAlignTop, new RepString(fPropNormal, TransformacionPAVILSA.fnFormatoCurrency(subtotal)));
        }
        ...
        if (nNumPag == nTotPag)//Si es ultima pagina
        {
            foreach (ImpuestoPAVILSA i in impuestos)
            {
                if (i.Nombre != "IEPS")
                {
                    Pie.AddAligned(horPadding + posPanelTotales, RepObj.rAlignLeft, verPadding + altoRenglon * renglon, RepObj.rAlignTop, new RepString(fPropNormal, i.Nombre + " " + TransformacionPAVILSA.fnFormatoPorcentaje(i.Tasa)));
                    Pie.AddAligned(Pie.rWidth - horPadding, RepObj.rAlignRight, verPadding + altoRenglon * renglon, RepObj.rAlignTop, new RepString(fPropNormal, TransformacionPAVILSA.fnFormatoCurrency(i.Importe)));
                    subtotal_extra += Convert.ToDouble(i.Importe);
                    renglon++;
                }
            }
            ...
            try
            {
                if (!impuestosComp.Count.Equals(0))
                { 
                    //Agregamos el separador para la suma entre los Impuestos y la Suma, para separar los Impuestos locales en otra operación
                    Pie.Add(posPanelTotales + horPadding, verPadding + altoRenglon * renglon, new RepLine(new PenProp(PDF, grosorPen, ColorT), Pie.rWidth - posPanelTotales - horPadding * 2, 0));
                    renglon++;
                    Pie.AddAligned(horPadding + posPanelTotales, RepObj.rAlignLeft, verPadding + altoRenglon * renglon, RepObj.rAlignTop, new RepString(fPropNormal, "SUBTOTAL"));
                    Pie.AddAligned(Pie.rWidth - horPadding, RepObj.rAlignRight, verPadding + altoRenglon * renglon, RepObj.rAlignTop, new RepString(fPropNormal, TransformacionPAVILSA.fnFormatoCurrency((subtotal_extra).ToString())));
                    renglon++;
                }
                ...
                {
                    int col = 0;
                    col = fnAgregarMultilinea(Pie, i.Nombre + " " + TransformacionPAVILSA.fnFormatoPorcentaje(i.Tasa), fPropNormal, horPadding + posPanelTotales, (verPadding * 4) + altoRenglon * renglon, 35, true);
                    Pie.AddAligned(Pie.rWidth - horPadding, RepObj.rAlignRight, verPadding + altoRenglon * renglon, RepObj.rAlignTop, new RepString(fPropNormal, TransformacionPAVILSA.fnFormatoCurrency(i.Importe)));
                    renglon += fnObtenerRenglonMayor(col, 0, 0);
                }
            }
            catch { }
            ...
            Pie.AddAligned(Pie.rWidth - horPadding, RepObj.rAlignRight, tamCodigo - altoRenglon, RepObj.rAlignTop, new RepString(fPropNormal, TransformacionPAVILSA.fnFormatoCurrency(total)));
        }
        ...
        if (nNumPag == nTotPag)//Si es ultima pagina
        {
            ...
            fnAgregarMultilinea(Pie, fnTextoImporte(TransformacionPAVILSA.fnFormatoRedondeo(total), moneda), fPropChica, tamCodigo + horPadding, altoRenglon * 1.7, 80, true);

            fnAgregarMultilinea(Pie, "Forma de Pago:", fPropNegrita, tamCodigo + horPadding, altoRenglon * 3.2, 80, true);
            fnAgregarMultilinea(Pie, formaDePago, fPropChica, tamCodigo + horPadding, altoRenglon * 4, 50, true);
        ...
        if (nNumPag == nTotPag)//Si es ultima pagina
        {
            ...
            renglon = fnAgregarMultilinea(Pie, TransformacionPAVILSA.fnConstruirCadenaTimbrado(gxComprobante.CreateNavigator().SelectSingleNode("/cfdi:Comprobante/cfdi:Complemento/tfd:TimbreFiscalDigital", nsmComprobante).CreateNavigator(), "cadenaoriginal_TFD_1_0")
        ...
        Pie.AddAligned(Pie.rWidth / 7, RepObj.rAlignCenter, Pie.rHeight - verPadding, RepObj.rAlignTop, new RepString(fPropNormal, clsComunPAVILSA.ObtenerParamentro("urlHostComercial")));
    ...
    private StaticContainer fnCrearEncabezado(System.Drawing.Color sColor)
    {
        ...
        //Panel redondeado para datos de adenda
        double dYPnl = 451;
        double dHPnl = 68;
        fnCrearPanelRedondeado(Encabezado, 0, dYPnl, Encabezado.rWidth, Encabezado.rHeight - dHPnl, grosorPen, radioCurva, false, sColor);
    ...
    {
        string razonSocial, rfc, calle, noExterior, noInterior, colonia, municipio, estado, pais, codigoPostal, serie, folio, Localidad, sMoneda, snumCompania, snumProveedor, snumOrdenCompra, sTotal, sSubTotal;
        razonSocial = rfc = calle = noExterior = noInterior = colonia = municipio = estado = pais = codigoPostal = serie = folio = Localidad = sMoneda = snumCompania = snumProveedor = snumOrdenCompra = sTotal = sSubTotal = string.Empty;
        ...
        try
        {
            ...
            XmlNodeList lista =
            ((XmlElement)Adenda[0]).GetElementsByTagName("FacturaPAVILSA");
            ...
            foreach (XmlElement nodo in lista)
            {
                ...
                XmlNodeList xTotal =
                nodo.GetElementsByTagName("Total");

                XmlNodeList xSubTotal =
                nodo.GetElementsByTagName("SubTotal");

                XmlNodeList xMoneda =
                nodo.GetElementsByTagName("Moneda");

                XmlNodeList xnumCompania =
                nodo.GetElementsByTagName("numCompania");

                XmlNodeList xnumProveedor =
                nodo.GetElementsByTagName("numProveedor");

                XmlNodeList xnumOrdenCompra =
                nodo.GetElementsByTagName("numOrdenCompra");

                sTotal = xTotal[a].InnerText;
                sSubTotal = xSubTotal[a].InnerText;
                sMoneda = xMoneda[a].InnerText;
                snumCompania = xnumCompania[a].InnerText;
                snumProveedor = xnumProveedor[a].InnerText;
                snumOrdenCompra = xnumOrdenCompra[a++].InnerText;
            }
        }
        catch { nBanAdd = 1; }
        //////////////////////////////////////////////////////////////////////////////
        ...
        //***********************Agrega datos de adenda**************************
        //Si existe Addenda
        if (nBanAdd == 0)
        {
            double dSep = 0.8;
            Renglon = 40.5;
            fPropNegrita.rSize = 4.5;
            fPropChica.rSize = 4;
            Encabezado.Add(leftPadding, posRenglon + tamRenglon * Renglon, new RepString(fPropNegrita, "Total"));
            Renglon += 0.5;
            Encabezado.Add(leftPadding, posRenglon + tamRenglon * Renglon, new RepString(fPropChica, TransformacionPAVILSA.fnFormatoCurrency(sTotal)));
            Renglon += dSep;
            Encabezado.Add(leftPadding, posRenglon + tamRenglon * Renglon, new RepString(fPropNegrita, "SubTotal"));
            Renglon += 0.5;
            Encabezado.Add(leftPadding, posRenglon + tamRenglon * Renglon, new RepString(fPropChica, TransformacionPAVILSA.fnFormatoCurrency(sSubTotal)));
            Renglon += dSep;
            Encabezado.Add(leftPadding, posRenglon + tamRenglon * Renglon, new RepString(fPropNegrita, "Moneda"));
            Renglon += 0.5;
            Encabezado.Add(leftPadding, posRenglon + tamRenglon * Renglon, new RepString(fPropChica, sMoneda));
            Renglon += dSep;
            Encabezado.Add(leftPadding, posRenglon + tamRenglon * Renglon, new RepString(fPropNegrita, "No. Compañia"));
            Renglon += 0.5;
            Encabezado.Add(leftPadding, posRenglon + tamRenglon * Renglon, new RepString(fPropChica, snumCompania));
            Renglon += dSep;
            Encabezado.Add(leftPadding, posRenglon + tamRenglon * Renglon, new RepString(fPropNegrita, "No. Proveedor"));
            Renglon += 0.5;
            Encabezado.Add(leftPadding, posRenglon + tamRenglon * Renglon, new RepString(fPropChica, snumProveedor));
            Renglon += dSep;
            Encabezado.Add(leftPadding, posRenglon + tamRenglon * Renglon, new RepString(fPropNegrita, "No. Orden Compra"));
            Renglon += 0.5;
            i = fnAgregarMultilinea(Encabezado, snumOrdenCompra, fPropChica, leftPadding, posRenglon + tamRenglon * Renglon, 185, true);
    ...
    private string fnTextoImporte(string psValor, string psMoneda)
    {
        ...
        NumaletPAVILSA parser = new NumaletPAVILSA();
        parser.LetraCapital = true;

        switch (psMoneda)
        {
            case "MXN":
                parser.TipoMoneda = NumaletPAVILSA.Moneda.Peso;
                break;
            case "USD":
                parser.TipoMoneda = NumaletPAVILSA.Moneda.Dolar;
                break;
            case "XEU":
                parser.TipoMoneda = NumaletPAVILSA.Moneda.Euro;
                break;
        }
    ...
    private RepImage fnMarcaAguaCorpus()
    {
        ...
        {
            clsErrorLogPAVILSA.fnNuevaEntrada(ex, clsErrorLogPAVILSA.TipoErroresLog.Referencia);
        }
...
public class ImpuestoPAVILSA
{
    ...
    public string TextoImpuesto
    {
        get
        {
            return Nombre + " " + TransformacionPAVILSA.fnFormatoPorcentaje(Tasa) + " " + TransformacionPAVILSA.fnFormatoCurrency(Importe);
        }
    }
    ...
    public ImpuestoPAVILSA(XPathNavigator navImpuesto, XmlNamespaceManager nsmComprobante)
    {
...
public class DetallePAVILSA
{
    ...
    /// <param name="nsmComprobante">Manejador de nombres de espacio</param>
    public DetallePAVILSA(XPathNavigator navDetalle, XmlNamespaceManager nsmComprobante)
    {
...
/// </summary>
public class TransformacionPAVILSA
{
    ...
    public static string fnConstruirCadenaTimbrado(IXPathNavigable xml, string psNombreArchivoXSLT)
    {
        ...
        try
        {
            ...
            trans.Load(XmlReader.Create(new StringReader(clsComunPAVILSA.ObtenerParamentro(psNombreArchivoXSLT))));
        ...
        {
            clsErrorLogPAVILSA.fnNuevaEntrada(ex, clsErrorLogPAVILSA.TipoErroresLog.Datos);
        }
...
public class clsErrorLogPAVILSA
{
    //private static clsInicioSesionUsuario gUsuario;

    public clsErrorLogPAVILSA()
    {
    }
...
public class clsComunPAVILSA
{
    public static string ObtenerParamentro(string sParametro)
    {
        ...
        using (SqlConnection con = new SqlConnection(PAXCrypto.CryptoAES.DesencriptaAES64(ConfigurationManager.ConnectionStrings["conControl"].ConnectionString)))
        {
            ...
            {
                con.Close();
                clsErrorLogPAVILSA.fnNuevaEntrada(ex, clsErrorLogPAVILSA.TipoErroresLog.Conexion);
            }
        }
        return sRetorno;
    }
...
public sealed class NumaletPAVILSA
{
    ...
    public NumaletPAVILSA(Boolean ConvertirDecimales, String MascaraSalidaDecimal, String SeparadorDecimalSalida, Boolean LetraCapital)
    {
    ...
    {
        return NumaletPAVILSA.ToString(Numero, CultureInfo.CurrentCulture);
    }
