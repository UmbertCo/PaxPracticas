...
/// <summary>
/// Summary description for clsPlantillaConstGUET
/// </summary>
public class clsPlantillaConstGUET
{
    #region atributos
    ...
    //tamaños en puntos
    private const double altoEncabezado = 70;
    ...
    public clsPlantillaConstGUET(XmlDocument pxComprobante)
	{
    ...
    public void fnGenerarPDF(int id_contribuyente, int sid_rfc, string scolor)
    {
        ...
        fPropTitulo = new FontProp(fuenteTitulo, tamFuenteTitulo, Color.Green);
    ...
    private List<RepString> fnDividirRenglonesDetalle(string psTexto, int pnTamRenglon, FontProp pFuente, bool bBuscarEspacio, StaticContainer pareaDetalle, double pdPosY)
    {
        ...
        if (nlongitudTexto > pnTamRenglon)
        {
            //for (r = 0; r <= 1; r++)
            //{
                if (bBuscarEspacio)
                {
                    //if (r == 0) //Si es primer renglon
                    //{
                        //Se verifica si contiene salto de linea.
                        //int nSL = psTexto.Substring(nCursorCadena, pnTamRenglon).IndexOf("\n");
                        //if (nSL > 0)
                        //{
                        //    //Si se parte la cadena en el salto de linea y se agrega renglon.
                        //    nUltimoEspacio = psTexto.Substring(nCursorCadena, pnTamRenglon).IndexOf("\n");
                        ...
                        //else
                        //{
                        //    //En caso de no contener en el primer renglon salto de linea se verifica si contiene espacio y se agrega el renglon.
                    ...
                    //{
                        ...
                        string sText = psTexto.Substring(nCursorCadena).TrimStart();
                        string[] sCad;
                        string[] sSig;
                        char[] cCad = { '\n' };
                        char[] cSig = { '$' };
                        sCad = sText.Split(cCad);
                        //De contener uno o mas salto de lineas se parte la cadena y se agrega los renglones correspondientes
                        if (sCad.Length > 1)
                        {
                            string sRenglon;
                            ...
                            {
                                sRenglon = s.ToString();
                                sSig = sRenglon.Split(cSig); //Se parte la cadena al encontrar un $
                                if (sSig.Length > 1)
                                {
                                    //Si contiene $ se agrega en la parte derecha de la descripción correspondiente RepObj.rAlignRight
                                    pareaDetalle.AddAligned(nLeft - 60, RepObj.rAlignLeft, pdPosY + nAlturaRenglon * dPos, RepObj.rAlignTop, new RepString(pFuente, "$ "));
                                    pareaDetalle.AddAligned(nLeft, RepObj.rAlignRight, pdPosY + nAlturaRenglon * dPos, RepObj.rAlignTop, new RepString(pFuente, sSig[1].ToString().Trim()));
                                    renglones.Add(new RepString(pFuente, sSig[0].ToString()));
                                    if (r == 1) //Si es el segundo renglon se empieza a incrementar de uno "esto para posicionar los importes en concepto"
                                        dPas = 1;
                                ...
                                {
                                    //Si la parte de cadena no contiene $ se agrega tal como esta
                                    renglones.Add(new RepString(pFuente, sRenglon));
                                    if (r == 1) //Si es el segundo renglon se empieza a incrementar de uno "esto para posicionar los importes en concepto"
                                        dPas = 1;
                        ...
                        {
                            //Si la cadena restante no contiene ningun salto de linea se parte en espacios segun la longitud configurada
                            while (nlongitudTexto > pnTamRenglon)
                            {
                                nUltimoEspacio = psTexto.Substring(nCursorCadena, pnTamRenglon).LastIndexOf(" ");
    ...
    private void fnTotales(XmlNamespaceManager nsmComprobante, XPathNavigator navPie, StaticContainer Pie, int nNumPag, int nTotPag, string sColor)
    {
        ...
        //{
            ...
            fnAgregarMultilinea(Pie, fnTextoImporte(TransformacionLogo.fnFormatoRedondeo(total), moneda), fPropChica, tamCodigo + horPadding, altoRenglon * 2, 80, true);
    ...
    private void fnDatosEmisor(XmlNamespaceManager nsmComprobante, XPathNavigator navEncabezado, StaticContainer Encabezado, System.Drawing.Color sColor)
    {
        ...
        double sep = 5; //10
        double posRazon = fPropTitulo.rSize + 62;
        ...
        fnCrearPanelRedondeado(Encabezado, Encabezado.rWidth - dAP, dAlt, fAnchoPanel + 30, fAltoPanel - 10, grosorPen, radioCurva, false, sColor);
        ...
        fnAgregarMultilineaCentrada(Encabezado, TipoDocumento, fPropNegrita, dXFol, (fAltoPanel + dAlt - 57 / 2) + 4, 15);
        ...
        if (!string.IsNullOrEmpty(sUUID))
        {
            Encabezado.AddAligned(posX, RepObj.rAlignRight,
                (fAltoPanel + fAltoPanel + dAltoFol / 4.5) + 8, RepObj.rAlignTop,
                new RepString(fPropNegrita, "Folio Fiscal:"));
            Encabezado.AddAligned(posX, RepObj.rAlignRight,
                (fAltoPanel + fAltoPanel + dAltoFol / 4.5) + 16, RepObj.rAlignTop,
        ...
        {
            Encabezado.AddAligned(posX, RepObj.rAlignRight,
                fAltoPanel * 2 + 61, RepObj.rAlignTop,
                new RepString(fPropNegrita, "No. de Serie del Certificado del SAT:"));
            Encabezado.AddAligned(posX, RepObj.rAlignRight,
                fAltoPanel * 2 + 69, RepObj.rAlignTop,
                new RepString(fPropNormal, noCertificadoSAT));
            //Encabezado.AddAligned(posX, RepObj.rAlignRight,
            //    fAltoPanel * 2 + 28, RepObj.rAlignTop,
            //    new RepString(fPropNegrita, "No de Serie del Certificado del SAT:"));
        ...
        {
            Encabezado.AddAligned(posX, RepObj.rAlignRight,
                fAltoPanel * 2 + 77, RepObj.rAlignTop,
                new RepString(fPropNegrita, "Fecha y hora de certificación:"));
            Encabezado.AddAligned(posX, RepObj.rAlignRight,
                fAltoPanel * 2 + 85, RepObj.rAlignTop,
    ...
    private void fnDatosReceptor(XmlNamespaceManager nsmComprobante, XPathNavigator navEncabezado, StaticContainer Encabezado)
    {
        ...
        double leftPadding = Encabezado.rWidth * 0.02;
        double sep = 5;//10
        double posRazon = Encabezado.rHeight / 2 + fPropTitulo.rSize + 10;
        double tamRenglon = fPropNormal.rSize + sep;
        double posRenglon = posRazon + sep;
        fPropNormal.rSize = 5;
        ...
        Encabezado.Add(leftPadding, posRenglon + tamRenglon * 2.2, new RepString(fPropNormal, ubicacion)); //*2.5
        Encabezado.Add(leftPadding, posRenglon + tamRenglon * 2.9, new RepString(fPropNormal, rfc)); //*3
...
public class clsErrorLogConstGUET
{
    //private static clsInicioSesionUsuario gUsuario;

    public clsErrorLogConstGUET()
    {
    }
...
public class clsComunLogo
{
    public static string ObtenerParamentro(string sParametro)
    {
        ...
        using (SqlConnection con = new SqlConnection(PAXCrypto.CryptoAES.DesencriptaAES64(ConfigurationManager.ConnectionStrings["conControl"].ConnectionString)))
        {
            ...
            {
                con.Close();
                clsErrorLogConstGUET.fnNuevaEntrada(ex, clsErrorLogConstGUET.TipoErroresLog.Conexion);
            }
        }
        return sRetorno;
    }
