...
public class clsEnvioCorreoDocs
{
        ...
        string psNombreRutaZip, string psNombreRutaXML, string sNombreDoc, string sCC)
    {
        ...
        try
        {
            ...
            if (addenda.Rows.Count > 0)
            {
                ...
                {
                    XmlAttribute atXmlns = pxComprobantePDF.CreateAttribute("xmlns:ahmsa", "http://www.w3.org/2000/xmlns/");
                    atXmlns.Value = "http://www.ahmsa.com/xsd/AddendaAHM1";
                    childElement.Attributes.SetNamedItem(atXmlns);

                    XmlAttribute atXsi = pxComprobantePDF.CreateAttribute("xsi", "schemaLocation", "http://www.w3.org/2001/XMLSchema-instance");
                ...
                {
                    XmlElement xeAutoZone = pxComprobantePDF.CreateElement("ADDENDA20");
    ...
    /// <param name="scolor"></param>
    /// <returns></returns>
    public bool fnPdfEnvioCorreoSinZIP(string psPlantilla, string psIdCfd, string psTipoDocumento,
    string sRutaPDF, int pnid_contribuyente, int pnid_rfc, string pscolor, string pstrMailTo, string pstrSubject, string pstrMensaje,
        string psNombreRutaZip, string psNombreRutaXML, string sNombreDoc, string sCC)
    {
        bool bEnvio = false;
        ...
        try
        {
            ...
            //Enviar archvivos
            bEnvio = fnEnviarCorreoDocSinZIP(pstrMailTo, pstrSubject, pstrMensaje, path, sRutaPDF, sNombreDoc, sCC);
