...
public class clsPlantillaCoinsa
{
    ...
    public void fnGenerarPDF(int id_contribuyente, int sid_rfc, string scolor)
    {
        ...
        while (bSeguir)
        {
            ...
            //Revisar porque se pierde este valor
            //TODO: Revisar con cliente
            MemoryStream ms = new MemoryStream();
            ms = fnImagenCliente(id_contribuyente, 107);
    ...
    private void fnTotales(XmlNamespaceManager nsmComprobante, XPathNavigator navPie, StaticContainer Pie, int nNumPag, int nTotPag, string sColor)
    {
        ...
        dYDet = 2.2;
        fnAgregarMultilinea(Pie, fnTextoImporte(TransformacionLogo.fnFormatoRedondeo(total), moneda), fPropChica, tamCodigo + horPadding - dXDet, altoRenglon * dYDet, 80, true);

        //Metodo de pago 
        if (!string.IsNullOrEmpty(metodoPago))
        {
            fnAgregarMultilinea(Pie, "Método de Pago:", fPropNegrita, tamCodigo + horPadding - dXDet, altoRenglon * 3.9, 80, true);
            fnAgregarMultilinea(Pie, metodoPago, fPropChica, tamCodigo + horPadding - dXDet, altoRenglon * 4.6, 80, true);

            //Número de cuenta
            if (!string.IsNullOrEmpty(sNumCtaPago))
            {
                fnAgregarMultilinea(Pie, "Número de Cuenta:", fPropNegrita, tamCodigo + horPadding - dXDet, altoRenglon * 5.5, 80, true);
                fnAgregarMultilinea(Pie, sNumCtaPago, fPropChica, tamCodigo + horPadding - dXDet, altoRenglon * 6.3, 80, true);
            }
        }
        else
        {
            ...
            if (!string.IsNullOrEmpty(sNumCtaPago))
            {
                fnAgregarMultilinea(Pie, "Número de Cuenta:", fPropNegrita, tamCodigo + horPadding - dXDet, altoRenglon * 3.9, 80, true);
                fnAgregarMultilinea(Pie, sNumCtaPago, fPropChica, tamCodigo + horPadding - dXDet, altoRenglon * 4.6, 80, true);
            }
        }
...
public class clsErrorLogCoinsa
{
    //private static clsInicioSesionUsuario gUsuario;

    public clsErrorLogCoinsa()
    {
    }
...
public class clsComunLogo
{
    public static string ObtenerParamentro(string sParametro)
    {
        ...
        using (SqlConnection con = new SqlConnection(PAXCrypto.CryptoAES.DesencriptaAES64(ConfigurationManager.ConnectionStrings["conControl"].ConnectionString)))
        {
            ...
            {
                con.Close();
                clsErrorLogCoinsa.fnNuevaEntrada(ex, clsErrorLogCoinsa.TipoErroresLog.Conexion);
            }
        }
        return sRetorno;
    }
