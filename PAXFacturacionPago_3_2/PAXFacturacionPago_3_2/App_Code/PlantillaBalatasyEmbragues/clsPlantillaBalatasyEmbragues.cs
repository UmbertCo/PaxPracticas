...
/// <summary>
/// Summary description for clsPlantillaBalatasyEmbragues
/// </summary>
public class clsPlantillaBalatasyEmbragues
{
    #region atributos
    ...
    public clsPlantillaBalatasyEmbragues(XmlDocument pxComprobante)
    {
    ...
    public void fnGenerarPDF(int id_contribuyente, int sid_rfc, string scolor)
    {
        ...
        while (bSeguir)
        {
            ...
            if (ms.Length > 0)
            {
                double largo  = 185;//185
                double tamaño = 85;//85
                RepImage image = new RepImage(ms, largo, tamaño);
                double x = 12;
                double y = 90;//90
                pagina.Add(margenPagina + x, margenPagina + y, image);
            }
    ...
    private StaticContainer fnCrearDetalle(List<DetalleLogo> detalles, System.Drawing.Color sColor)
    {
        ...
        double posColumna1 = areaDetalle.rWidth * 0.060;     //CANTIDAD
        double posColumna2 = areaDetalle.rWidth / 9.5;      //CODIGO 
        double posColumna3 = areaDetalle.rWidth / 4.8;      //U.M 
        double posColumna4 = areaDetalle.rWidth * 0.28;      //DESCRIPCION
        double posColumna5 = areaDetalle.rWidth * 0.82;     //PRECIO  420 0.82
    ...
    private void    fnTotales(XmlNamespaceManager nsmComprobante, XPathNavigator navPie, StaticContainer Pie, int nNumPag, int nTotPag, string sColor)
    {
        ...
        if (nNumPag == nTotPag)//Si es ultima pagina
        {
            ...
            Pie.AddAligned(Pie.rWidth - horPadding, RepObj.rAlignRight, verPadding, RepObj.rAlignTop, new RepString(fPropNormal, TransformacionBalatasEmbragues.fnFormatoCurrency(subtotal)));
        }
        double altoRenglon = fPropNormal.rSize + verPadding;
        double renglon = 1;//int
        ...
        if (nNumPag == nTotPag)//Si es ultima pagina
        {
            foreach (ImpuestoLogo i in impuestos)
            {
                if (i.Nombre != "IEPS")
                {
                    Pie.AddAligned(horPadding + posPanelTotales, RepObj.rAlignLeft, verPadding + altoRenglon * renglon, RepObj.rAlignTop, new RepString(fPropNormal, i.Nombre + " " + TransformacionBalatasEmbragues.fnFormatoPorcentaje(i.Tasa)));
                    Pie.AddAligned(Pie.rWidth - horPadding, RepObj.rAlignRight, verPadding + altoRenglon * renglon, RepObj.rAlignTop, new RepString(fPropNormal, TransformacionBalatasEmbragues.fnFormatoCurrency(i.Importe)));
                    renglon++;
                }
            }
            ...
            Pie.AddAligned(Pie.rWidth - horPadding, RepObj.rAlignRight, tamCodigo - altoRenglon, RepObj.rAlignTop, new RepString(fPropNormal, TransformacionBalatasEmbragues.fnFormatoCurrency(total)));
        }
        ...
        if (nNumPag == nTotPag)//Si es ultima pagina
        {
            ...
            fnAgregarMultilinea(Pie, fnTextoImporte(TransformacionBalatasEmbragues.fnFormatoRedondeo(total), moneda), fPropChica, tamCodigo + horPadding, altoRenglon * 1.7, 80, true);
            ...
            if (version == "3.2") //Si es version 3.2 muestra el siguiente contenido
            {
                ...
                fnAgregarMultilinea(Pie, "EFECTOS FISCALES AL PAGO", fPropNormal, tamCodigo + horPadding, altoRenglon * 7, 80, true);
            }
        }
        ...
        if (nNumPag == nTotPag)//Si es ultima pagina
        {
            ...
            renglon = fnAgregarMultilinea(Pie, TransformacionBalatasEmbragues.fnConstruirCadenaTimbrado(gxComprobante.CreateNavigator().SelectSingleNode("/cfdi:Comprobante/cfdi:Complemento/tfd:TimbreFiscalDigital", nsmComprobante).CreateNavigator(), "cadenaoriginal_TFD_1_0")
        ...
        fnCrearPanelRedondeado(Pie, 0, tamCodigo, Pie.rWidth, dHeight - tamCodigo - altoRenglon - 85, grosorPen, radioCurva, false, ColorT);
        ...
        renglon = 10;
        //agrega el pagare
        Pie.Add(horPadding, tamCodigo + altoRenglon * renglon++, new RepString(fPropChica, "DEBO Y PAGARE INCONDICIONALMENTE A LA ORDEN DE BALATAS Y EMBRAGUES S.A DE C.V. EL IMPORTE INDICADO, RECONOCIENDO ESTA FACTURA"));
        Pie.Add(horPadding, tamCodigo + altoRenglon * renglon++, new RepString(fPropChica, "COMO TITULO DE CREDITO, VALOR RECIBIDO A MI ENTERA SATISFACCION, DE ACUERDO AL ART.173 DE LA LEY GENERAL DE TITULOS Y OPERACIONES"));
        Pie.Add(horPadding, tamCodigo + altoRenglon * renglon++, new RepString(fPropChica, "DE CREDITO LA TENENCIA DE ESTA FACTURA NO ES COMPROBANTE DE PAGO DE LA MISMA"));
        Pie.Add(horPadding, tamCodigo + altoRenglon * renglon++, new RepString(fPropChica, "EL PAGO SE ACREDITARA CON LA FICHA DE DEPOSITO O TRANSFERENCIA BANCARIA EN "));
        Pie.Add(horPadding, tamCodigo + altoRenglon * renglon++, new RepString(fPropChica, "LA INSTITUCION CORRESPONDIENTE. SI ESTA FACTURA NO ES PAGADA A SU VENCIMIENTO"));
        Pie.Add(horPadding, tamCodigo + altoRenglon * renglon++, new RepString(fPropChica, "CAUSARA UN INTERES MORATORIO DEL 3% MENSUAL "));
        Pie.Add(horPadding + 450, tamCodigo + altoRenglon * renglon++ - 5, new RepString(fPropChica, "ACEPTO(AMOS)"));
        //Agregamos linea para firmar
        Pie.Add(posPanelTotales + horPadding, tamCodigo - altoRenglon + 185, new RepLine(new PenProp(PDF, grosorPen, ColorT), Pie.rWidth - posPanelTotales - horPadding * 2, 0));

        //Agrega www.paxfacturacion.com
        fPropNormal = new FontProp(fuenteNormal, 4);
        Pie.AddAligned(Pie.rWidth / 7, RepObj.rAlignCenter, Pie.rHeight - verPadding, RepObj.rAlignTop, new RepString(fPropNormal, clsComunBalatasEmbragues.ObtenerParamentro("urlHostComercial")));
    ...
    private void fnDibujarTitulosDetalle(StaticContainer Encabezado, System.Drawing.Color sColor)
    {
        ...
        double posColumna3 = Encabezado.rWidth / 4.8;      //U.M 
        double posColumna4 = Encabezado.rWidth / 2.4;      //DESCRIPCION
        double posColumna5 = Encabezado.rWidth * 0.75;     //PRECIO  420
        double posColumna6 = Encabezado.rWidth * 0.90;     //IMPORTE

        Encabezado.AddAligned(posColumna1, RepObj.rAlignLeft, puntoMedio, RepObj.rAlignCenter, new RepString(fPropBlanca, "CANT"));
        Encabezado.AddAligned(posColumna2, RepObj.rAlignLeft, puntoMedio, RepObj.rAlignCenter, new RepString(fPropBlanca, "CODIGO"));
        Encabezado.AddAligned(posColumna3, RepObj.rAlignLeft, puntoMedio, RepObj.rAlignCenter, new RepString(fPropBlanca, "U.M"));
        Encabezado.AddAligned(posColumna4, RepObj.rAlignLeft, puntoMedio, RepObj.rAlignCenter, new RepString(fPropBlanca, "DESCRIPCION"));
    ...
    private void fnDatosEmisor(XmlNamespaceManager nsmComprobante, XPathNavigator navEncabezado, StaticContainer Encabezado, System.Drawing.Color sColor)
    {
        ...
        if (exCalle != string.Empty)
        {
            ...
            exNoExterior = navEncabezado.SelectSingleNode("/cfdi:Comprobante/cfdi:Emisor/cfdi:ExpedidoEn/@noExterior", nsmComprobante).Value;
            ...
            exColonia = navEncabezado.SelectSingleNode("/cfdi:Comprobante/cfdi:Emisor/cfdi:ExpedidoEn/@colonia", nsmComprobante).Value;

            exMunicipio = navEncabezado.SelectSingleNode("/cfdi:Comprobante/cfdi:Emisor/cfdi:ExpedidoEn/@municipio", nsmComprobante).Value;

            exEstado = navEncabezado.SelectSingleNode("/cfdi:Comprobante/cfdi:Emisor/cfdi:ExpedidoEn/@estado", nsmComprobante).Value;

            exPais = navEncabezado.SelectSingleNode("/cfdi:Comprobante/cfdi:Emisor/cfdi:ExpedidoEn/@pais", nsmComprobante).Value;

            exCodigoPostal = navEncabezado.SelectSingleNode("/cfdi:Comprobante/cfdi:Emisor/cfdi:ExpedidoEn/@codigoPostal", nsmComprobante).Value;
        ...
        if (!string.IsNullOrEmpty(exLocalidad))
        {
        ...
        Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignRight, posRenglon - 3, RepObj.rAlignTop, new RepString(fPropNormal, "BALATAS Y EMBRAGUES, S.A DE C.V."));
        fPropNormal.rSize = 5;
        Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignRight, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, "Av.Teofilo Borunda Ortiz 2502"));
        //salto de renglon 
        dposRen += dcalRen;
        Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignRight, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, "Col. Centro CP 31000"));
        //salto de renglon 
        dposRen += dcalRen;
        Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignRight, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, "Chihuahua,Chihuahua,Mexico"));
        //salto de renglon 
        dposRen += dcalRen;
        Encabezado.AddAligned(leftPadding + dPX, RepObj.rAlignRight, posRenglon + tamRenglon * dposRen, RepObj.rAlignTop, new RepString(fPropNormal, rfc));
        //salto de renglon 
        dposRen += dcalRen;
        dposRen += dcalRen;
        fnAgregarMultilineaDerecha(Encabezado, Regimenfiscal, fPropNormal, leftPadding + dPX, posRenglon + tamRenglon * dposRen, 45);
        dposRen += dcalRen;
        ...
        ////double Renglon = 0;
        //double i = 0;
        //i = fnAgregarMultilinea(Encabezado,"BALATAS Y EMBRAGUES, S.A DE C.V. " ,fPropTitulo, leftPadding + 155, posRenglon, 40, true);//razonSocial
        //Renglon += i;
        //if (i > 1)
        //    Renglon -= 0.5;
        //if (exCalle != string.Empty)
    ...
    private void fnDatosReceptor(XmlNamespaceManager nsmComprobante, XPathNavigator navEncabezado, StaticContainer Encabezado, double Renglon)
    {
        ...
        Renglon += 0.5;
        double i = 0;
        i = fnAgregarMultilinea(Encabezado, razonSocial, fPropNormal, leftPadding, posRenglon + tamRenglon * Renglon++, 75, true);
        Renglon += i;
        if (i > 1)
            Renglon -= 1.5;
        else
    ...
    /// <summary>
    /// Inserta en el documento una cadena multilinea con el formato especificado aliniado a la derecha
    /// </summary>
    /// <param name="pContenedor">Objeto StaticContainer al cual se le añadirá la cadena de texto</param>
    /// <param name="psTexto">Cadena de texto</param>
    /// <param name="pFuente">Fuente a utilizar</param>
    ...
    /// <param name="pnTamRenglon">Ancho en número de caracteres que tendrá cada renglon</param>
    /// <returns>Retorna un entero especificando el número de renglones final</returns>
    private int fnAgregarMultilineaDerecha(StaticContainer pContenedor, string psTexto, FontProp pFuente, double pdPosX, double pdPosY, int pnTamRenglon)
    {
        double nAlturaRenglon = pFuente.rSize * 1.2;
        int nNoRenglon = 0;
        ...
        foreach (RepString r in fnDividirRenglones(psTexto, pnTamRenglon, pFuente, true))
        {
            pContenedor.AddAligned(pdPosX, RepObj.rAlignRight, pdPosY + nAlturaRenglon * nNoRenglon, RepObj.rAlignRight, r);
            nNoRenglon++;
        }
    ...
    private void fnAgregarBordeRedondeado(StaticContainer poObjeto, double pfGrosor, double pfRadioCurva, System.Drawing.Color sColor)
    {
        ...
        double dvalor = 50;
        pfRadioCurva = 0;         
        poObjeto.Add(0 + dvalor, pfRadioCurva, new RepLine(pen, 0, pfRadioCurva * 2 - poObjeto.rHeight));
        ////Linea Codigo
        dvalor = 105;
        poObjeto.Add(0 + dvalor, pfRadioCurva, new RepLine(pen, 0, pfRadioCurva * 2 - poObjeto.rHeight));
        ////Linea U.M
    ...
    private RepImage fnMarcaAguaCorpus()
    {
        ...
        {
            clsErrorBalatasEmbragues.fnNuevaEntrada(ex, clsErrorBalatasEmbragues.TipoErroresLog.Referencia);
        }
...
public class ImpuestoLogo
{
    ...
    public string TextoImpuesto
    {
        get
        {
            return Nombre + " " + TransformacionBalatasEmbragues.fnFormatoPorcentaje(Tasa) + " " + TransformacionBalatasEmbragues.fnFormatoCurrency(Importe);
        }
    }
...
}//fin de Clase clsPlantillaBalatasyEmbregues
...
/// </summary>
public class TransformacionBalatasEmbragues
{
    ...
    public static string fnConstruirCadenaTimbrado(IXPathNavigable xml, string psNombreArchivoXSLT)
    {
        ...
        try
        {
            ...
            trans.Load(XmlReader.Create(new StringReader(clsComunBalatasEmbragues.ObtenerParamentro(psNombreArchivoXSLT))));
        ...
        {
            clsErrorBalatasEmbragues.fnNuevaEntrada(ex, clsErrorBalatasEmbragues.TipoErroresLog.Datos);
        }
...
public class clsErrorBalatasEmbragues
{
    //private static clsInicioSesionUsuario gUsuario;

    public clsErrorBalatasEmbragues()
    {
    }
...
public class clsComunBalatasEmbragues
{
    public static string ObtenerParamentro(string sParametro)
    {
        ...
        using (SqlConnection con = new SqlConnection(PAXCrypto.CryptoAES.DesencriptaAES64(ConfigurationManager.ConnectionStrings["conControl"].ConnectionString)))
        {
            ...
            {
                con.Close();
                clsErrorBalatasEmbragues.fnNuevaEntrada(ex, clsErrorBalatasEmbragues.TipoErroresLog.Conexion);
            }
        }
        return sRetorno;
    }
