...
public class clsOperacionConsultaPdf
{
    ...
    private StaticContainer fnContarPaginas(List<Detalle> detalles)
    {
        ...
        for (int i = 0; renglonActual <= maxRenglones; i++)
        {
            ...
            Col3 = fnAgregarMultilineaDetalle(areaDetalle, d.descripcion, fPropNormal, posColumna3, posRenglon, 52, true);
	...
	private void fnDatosEmisor(XmlNamespaceManager nsmComprobante, XPathNavigator navEncabezado, StaticContainer Encabezado, System.Drawing.Color sColor, ref double Renglon)
	{
        string razonSocial, rfc, calle, noExterior, noInterior, colonia, municipio, estado, pais, codigoPostal, serie, folio, fecha, fechaTimb,
            noCertificadoEmisor, referencia, Localidad, Regimenfiscal, version, estadolug;
        razonSocial = rfc = calle = noExterior = noInterior = colonia = municipio = estado = pais = codigoPostal = fecha = serie = folio =
            noCertificadoEmisor = referencia = Localidad = Regimenfiscal = version = fechaTimb = estadolug = string.Empty;
