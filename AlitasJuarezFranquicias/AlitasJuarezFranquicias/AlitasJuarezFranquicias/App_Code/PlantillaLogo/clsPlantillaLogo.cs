...
public class clsPlantillaLogo
{
    ...
    /// </summary>
	public void fnGenerarPDF(int id_contribuyente,int sid_rfc,string scolor, byte[] UrlImag)
	{
		...
		while (bSeguir)
		{
            ...
            if (ms.Length > 0)
            {
                double dWidth = 150;
                double Height = 80;
                RepImage image = new RepImage(ms, dWidth, Height);
                double spY = 90;
    ...
    private StaticContainer fnContarPaginas(List<DetalleLogo> detalles)
    {
        ...
        for (int i = 0; renglonActual <= maxRenglones; i++)
        {
            ...
            Col3 = fnContarMultilinea(areaDetalle, d.descripcion, fPropNormal, posColumna3, posRenglon, 52, true);
    ...
    public void fnGenerarPDFSave(int id_contribuyente, int sid_rfc, string scolor, string ruta, byte[] UrlImag)
    {
        Formatter formato = new PdfFormatter();
        PDF = new Report(formato);
	...
	private List<DetalleLogo> fnObtenerDetalles()
	{
		...
		{
            detalles.Add(new DetalleLogo(navDetalles.Current, nsmComprobante, navComprobante));
		}
    ...
    private void fnDatosEmisor(XmlNamespaceManager nsmComprobante, XPathNavigator navEncabezado, StaticContainer Encabezado, System.Drawing.Color sColor, ref double Renglon)
    {
        ...
        double leftPadding = Encabezado.rWidth * 0.03;
        double sep = 5; //10
        ...
        i = fnAgregarMultilinea(Encabezado, razonSocial, fPropTitulo, leftPadding + 255, posRenglon, 70, true);
        Renglon += i;
        if (i > 1)
            Renglon -= 0.5;
        i = fnAgregarMultilinea(Encabezado, direccion, fPropNormal, leftPadding + 255, posRenglon + tamRenglon * Renglon,70, true);
        Renglon += i;
        if (i > 1)
            Renglon -= 0.5;
        i = fnAgregarMultilinea(Encabezado, ubicacion, fPropNormal, leftPadding + 255, posRenglon + tamRenglon * Renglon, 70, true);
        Renglon += i;
        if (i > 1)
            Renglon -= 0.5;
        i = fnAgregarMultilinea(Encabezado, ubicacion2, fPropNormal, leftPadding + 255, posRenglon + tamRenglon * Renglon, 70, true);
        Renglon += i;
        if (i > 1)
            Renglon -= 0.5;
        i = fnAgregarMultilinea(Encabezado, rfc, fPropNormal, leftPadding + 255, posRenglon + tamRenglon * Renglon,70, true);
        Renglon += i;
        if (i > 1)
            Renglon -= 0.5;
        ...
        if (!string.IsNullOrEmpty(referencia))
        {
            ...
            i = fnAgregarMultilinea(Encabezado, Refer, fPropNormal, leftPadding + 255, posRenglon + tamRenglon * Renglon, 70, true);
            Renglon += i;
            if (i > 1)
                Renglon -= 0.5;
        }
        ...
        if (exCalle != string.Empty)
        {
            ...
            //if (i > 1)
            //    Renglon -= 0.5;
            i = fnAgregarMultilinea(Encabezado, "EXPEDIDO EN", fPropNormal, leftPadding + 255, posRenglon + tamRenglon * Renglon, 70, true);
            Renglon += i;
            if (i > 1)
                Renglon -= 0.5;
            i = fnAgregarMultilinea(Encabezado, exDireccion , fPropNormal, leftPadding + 255, posRenglon + tamRenglon * Renglon, 70, true);
            Renglon += i;
            if (i > 1)
                Renglon -= 0.5;
            i = fnAgregarMultilinea(Encabezado, exUbicacion + " - " + exUbicacion2, fPropNormal, leftPadding + 255, posRenglon + tamRenglon * Renglon, 70, true);
            Renglon += i;
            if (i > 1)
                Renglon -= 0.5;
            i = fnAgregarMultilinea(Encabezado, "Regimen Fiscal : " + Regimenfiscal, fPropNormal, leftPadding + 255, posRenglon + tamRenglon * Renglon, 70, true);
            Renglon += i;
            if (i > 1)
                Renglon -= 0.5;
        }
        ...
        //i = fnAgregarMultilinea(Encabezado, sRegFis + Regimenfiscal, fPropNormal, leftPadding + 280, posRenglon + tamRenglon * Renglon, 46, true);
        //Renglon += i;
        ////if (i > 1)
        ////    Renglon -= 0.5;
        ////i = fnAgregarMultilinea(Encabezado, Regimenfiscal, fPropNormal, leftPadding + 280, posRenglon + tamRenglon * Renglon, 46, true);
        ...
        fnCrearPanelRedondeado(Encabezado, Encabezado.rWidth - 170, 0, fAnchoPanel + 55, fAltoPanel, grosorPen, radioCurva, false, sColor);
        fnCrearPanelRedondeado(Encabezado, Encabezado.rWidth - 170, fAltoPanel, fAnchoPanel + 55, fAltoPanel, grosorPen, radioCurva, false, sColor);

        //fnAgregarMultilineaCentrada(Encabezado, TipoDocumento, fPropBlanca, posX + fAnchoPanel / 2, fAltoPanel / 2, 15);

        fnAgregarMultilineaCentrada(Encabezado, TipoDocumento, fPropNegrita, posX + fAnchoPanel / 4, fAltoPanel / 2, 15);
    ...
    private void fnDatosReceptor(XmlNamespaceManager nsmComprobante, XPathNavigator navEncabezado, StaticContainer Encabezado, double Renglon)
    {
        ...
        //Encabezado.Add(leftPadding, posRenglon + t0amRenglon * 1.5, new RepString(fPropNormal, direccion)); //* 2
        ...
        Renglon += 0.5;
        double i = 0;
        i = fnAgregarMultilinea(Encabezado, razonSocial, fPropTitulo, leftPadding, posRenglon + tamRenglon * Renglon++, 90, true);
    ...
    /// </summary>
    /// <returns></returns>
    private MemoryStream fnImagenCliente(byte[] UrlImg)
    {

        MemoryStream ms = new MemoryStream();
        try
        {
            //byte[] sImagen = HttpContext.Current.Server.MapPath(UrlImg);
            //System.Drawing.Image img = new Bitmap(sImagen);
            //byte[] byteImage = imageToByteArray(img);
            ms = new MemoryStream(UrlImg);
        }
        catch (Exception ex)
        {
...
public class DetalleLogo
{
	...
	/// <param name="nsmComprobante">Manejador de nombres de espacio</param>
    public DetalleLogo(XPathNavigator navDetalle, XmlNamespaceManager nsmComprobante, XPathNavigator navPie)
	{
        ...
        descripcion = navDetalle.SelectSingleNode("@descripcion", nsmComprobante).Value + " " + DateTime.Parse(navPie.SelectSingleNode("/cfdi:Comprobante/@fecha", nsmComprobante).Value, System.Globalization.CultureInfo.InvariantCulture, System.Globalization.DateTimeStyles.None).ToString("dd/MM/yyyy hh:mm tt"); 
...
public class clsComunLogo
{
    ...
    public static string ObtenerParamentro(string sParametro)
    {
        ...
        try
        {
            Utilerias.SQL.InterfazSQL conexion = fnCrearConexion("conConfiguracion");
