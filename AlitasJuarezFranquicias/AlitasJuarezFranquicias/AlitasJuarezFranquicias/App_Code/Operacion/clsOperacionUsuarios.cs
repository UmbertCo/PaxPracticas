...
public class clsOperacionUsuarios
{
    ...
    public int fnRegistroContribuyente(string sNombre, string sUsuario, string correo, string sPassword,string sUsuarioAlta, string sSistemaOrigen, string sTelefono)
    {
        int Resultado = 0;
        try
        {
    ...
    public DataTable fnObtenerIdsEstructuraUsuario(int nidUsuario)
    {
        DataTable dtIdsEstructuras = new DataTable() ;
        try
        {
            giSql = clsComun.fnCrearConexion(conCuenta);
            giSql.AgregarParametro("nIdUsuario",nidUsuario);
            giSql.Query("usp_con_Obtener_Ids_Estructura_Usuario_sel",true,ref dtIdsEstructuras);
            return dtIdsEstructuras;
        }
        catch (Exception ex)
        {
            clsErrorLog.fnNuevaEntrada(ex, clsErrorLog.TipoErroresLog.Conexion);
    ...
    /// </summary>   
    /// <returns>Regresa la lista de los usuarios</returns>
    public void fnEliminarEstructuraUsuarioSucursal(int nidUsuario,int nidEstructura)
    {
        try
        {
            giSql = clsComun.fnCrearConexion(conCuenta);
            ...
            giSql.AgregarParametro("@nId_Usuario", nidUsuario);
            giSql.AgregarParametro("@nId_Estructura", nidEstructura);
            giSql.NoQuery("usp_Ctp_UsuarioEstructura_Sucursal_del", true);
    ...
    /// </summary>   
    /// <returns></returns>
    public void fnActualizaUsuarioInfo(int nidUsuario, string sNombre, int nIdContribuyente, string sClaveUsuario, string sEmail, string sTelefono,string sPassword)
    {
        try
        {
            giSql = clsComun.fnCrearConexion(conCuenta);
    ...
    /// </summary>
    /// <param name="sPagina">Nombre de la página a la que se quiere tener acceso</param>
    /// <returns></returns>
    public bool fnObtenerPermisoPagina(string sPagina)
    {
        try
        {
            ...
            giSql = clsComun.fnCrearConexion(conCuenta);
            dtAuxiliar = new DataTable();
            giSql.AgregarParametro("@sPagina", sPagina);
            giSql.AgregarParametro("@nIdUsuario", usuario.id_usuario);
            int nId = Convert.ToInt32(giSql.TraerEscalar("usp_ses_Permiso_Pagina_Usuario_sel", true));
            if (nId > 0)
            {
                return true;
            }
