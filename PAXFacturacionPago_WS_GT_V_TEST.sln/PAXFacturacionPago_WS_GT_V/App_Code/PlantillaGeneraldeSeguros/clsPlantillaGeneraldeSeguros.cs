...
/// </summary>
public class clsPlantillaGeneraldeSeguros
{
    #region atributos
    ...
    //tamaños en puntos
	private const double altoEncabezado = 100;//60
    ...
    public clsPlantillaGeneraldeSeguros(XmlDocument pxComprobante)
	{
        try
        {
    ...
    /// </summary>
    public void fnGenerarPDF(int id_contribuyente, int sid_rfc, string scolor, ref byte[] imgCodigo)
	{
		...
		while (bSeguir)
		{
            ...
            pagina.Add(margenPagina + 5, RT.rPointFromMM(altoPagina) - margenPagina - Pie.rHeight + tamCodigo - 5, GenerarCodigoBidimensional(ref imgCodigo));
            ...
            //}
            MemoryStream ms = new MemoryStream();
            ms = fnImagenCliente("GeneraldeSeguros.jpg"); //id_contribuyente, sid_rfc);
    ...
    private StaticContainer fnContarPaginas(List<DetalleLogo> detalles)
    {
        ...
        double dvariable = 0.01;
        //Definimos la posicion de los titulos
        double posColumna1 = areaDetalle.rWidth * dvariable;      //Concepto  0.01
        double posColumna2 = areaDetalle.rWidth * dvariable;      //Debe (Cuenta Corriente) 0.43
        double posColumna3 = areaDetalle.rWidth * dvariable;      //Haber(Cuenta Corriente) 0.15
        double posColumna4 = areaDetalle.rWidth * dvariable;     //Debe (Cuenta Reserva)   0.515
        double posColumna5 = areaDetalle.rWidth * dvariable;     //Haber(Cuenta Reserva)   0.285
    ...
    public void fnGenerarPDFSave(int id_contribuyente, int sid_rfc, string scolor, string ruta, ref byte[] imgCodigo)
    {
        Formatter formato = new PdfFormatter();
        PDF = new Report(formato);
    ...
    public void fnGenerarCodigoBidimensional(ref byte[] imgCodigo)
    {        
        GenerarCodigoBidimensional(ref imgCodigo);
    }
	...
	/// <returns></returns>
	private StaticContainer fnCrearDetalle(List<DetalleLogo> detalles, System.Drawing.Color sColor, DataSet ds)
	{
		StaticContainer areaDetalle = new StaticContainer(RT.rPointFromMM(anchoPagina) - margenPagina * 2, RT.rPointFromMM(altoPagina - altoEncabezado - altoPie) - margenPagina * 2);
        ...
        //double posColumna4 = areaDetalle.rWidth * dvariable;      //Debe (Cuenta Reserva)   0.515
        //double posColumna5 = areaDetalle.rWidth * dvariable;      //Haber(Cuenta Reserva)   0.285
        double posColumna1 = areaDetalle.rWidth * 0.01;      //Concepto
        double posColumna2 = areaDetalle.rWidth * 0.43;      //Debe (Cuenta Corriente)
        double posColumna3 = areaDetalle.rWidth * 0.15;      //Haber(Cuenta Corriente)
        double posColumna4 = areaDetalle.rWidth * 0.515;     //Debe (Cuenta Reserva)
        double posColumna5 = areaDetalle.rWidth * 0.285;     //Haber(Cuenta Reserva)
        ...
        foreach (DataRow dr in ds.Tables["Concepto"].Rows)
        {
            ...
            {
                //Si es en la misma hoja se posiciona debajo del anterior concepto
                posRenglon = altoRenglon + (nAlturaRenglon * (renglonActual + a));
                a += 1;
            }

            fnAgregarMultilinea(areaDetalle, dr["concepto"].ToString(), fPropNormal, posColumna1, posRenglon, 40, true); //No Contrato

            //fnAgregarMultilinea(areaDetalle, Transformacion.fnFormatoRedondeo(dr["CCdebe"].ToString()), fPropNormal, posColumna1 + posColumna2, posRenglon, 13, true); //
            //fnAgregarMultilinea(areaDetalle, Transformacion.fnFormatoRedondeo(dr["CChaber"].ToString()), fPropNormal, posColumna2 + posColumna3, posRenglon, 13, true); //
            //fnAgregarMultilinea(areaDetalle, Transformacion.fnFormatoRedondeo(dr["CRdebe"].ToString()), fPropNormal, posColumna3 + posColumna4, posRenglon, 9, true); //
            //fnAgregarMultilinea(areaDetalle, Transformacion.fnFormatoRedondeo(dr["CRhaber"].ToString()), fPropNormal, posColumna4 + posColumna5, posRenglon, 13, true); //

            areaDetalle.AddAligned(posColumna1 + posColumna2 + dXDet, RepObj.rAlignRight, posRenglon, RepObj.rAlignBottom, new RepString(fPropNormal, Transformacion.fnFormatoRedondeo(dr["CCdebe"].ToString())));
            dXDet -= 15;
            areaDetalle.AddAligned(posColumna2 + posColumna3 + dXDet, RepObj.rAlignRight, posRenglon, RepObj.rAlignBottom, new RepString(fPropNormal, Transformacion.fnFormatoRedondeo(dr["CChaber"].ToString())));
            dXDet += 15;
            areaDetalle.AddAligned(posColumna3 + posColumna4 + dXDet, RepObj.rAlignRight, posRenglon, RepObj.rAlignBottom, new RepString(fPropNormal, Transformacion.fnFormatoRedondeo(dr["CRdebe"].ToString())));
            dXDet -= 15;
            areaDetalle.AddAligned(posColumna4 + posColumna5 + dXDet, RepObj.rAlignRight, posRenglon, RepObj.rAlignBottom, new RepString(fPropNormal, Transformacion.fnFormatoRedondeo(dr["CRhaber"].ToString())));
    ...
    private void fnTotales(XmlNamespaceManager nsmComprobante, XPathNavigator navPie, StaticContainer Pie, int nNumPag, int nTotPag, string sColor, DataSet ds)
    {
        ...
        if (nNumPag == nTotPag)//Si es ultima pagina
        {
            ...
            string sCRhaberSaldo, sCRdebeSaldo, sCChaberSaldo, sCCdebeSaldo;
            sCRhaberSaldo = sCRdebeSaldo = sCChaberSaldo = sCCdebeSaldo = string.Empty;
            foreach (DataRow dr in ds.Tables["TOTALES"].Rows)
            {
                sCRhaberSaldo = dr["CRhaberSaldo"].ToString();
                sCRdebeSaldo = dr["CRdebeSaldo"].ToString();
                sCChaberSaldo = dr["CChaberSaldo"].ToString();
                sCCdebeSaldo = dr["CCdebeSaldo"].ToString();
            }
            //SALDO ACTUAL
            double dYSal = 110;
            Pie.AddAligned(horPadding, RepObj.rAlignLeft, tamCodigo - dYSal, RepObj.rAlignTop, new RepString(fPropNegrita, "SALDO ACTUAL"));
            //CCdebeSaldo
            double dXCCd = 290.96;
            Pie.AddAligned(horPadding + dXCCd, RepObj.rAlignRight, tamCodigo - dYSal, RepObj.rAlignTop, new RepString(fPropNormal, TransformacionLogo.fnFormatoRedondeo(sCCdebeSaldo)));
            //CChaberSaldo
            double dXCCh = 356.04;
            Pie.AddAligned(horPadding + dXCCh, RepObj.rAlignRight, tamCodigo - dYSal, RepObj.rAlignTop, new RepString(fPropNormal, TransformacionLogo.fnFormatoRedondeo(sCChaberSaldo)));
            //CRdebeSaldo
            double dXCRd = 419.66;
            Pie.AddAligned(horPadding + dXCRd, RepObj.rAlignRight, tamCodigo - dYSal, RepObj.rAlignTop, new RepString(fPropNormal, TransformacionLogo.fnFormatoRedondeo(sCRdebeSaldo)));
            //CRhaberSaldo
            double dXCRh = 481.8799997;
            Pie.AddAligned(horPadding + dXCRh, RepObj.rAlignRight, tamCodigo - dYSal, RepObj.rAlignTop, new RepString(fPropNormal, TransformacionLogo.fnFormatoRedondeo(sCRhaberSaldo)));
        ...
        Pie.Add(tamCodigo + horPadding, altoRenglon, new RepString(fPropNegrita, "IMPORTE CON LETRA:"));
        fnAgregarMultilinea(Pie, fnTextoImporte(TransformacionLogo.fnFormatoRedondeo(total), moneda).ToUpper(), fPropChica, tamCodigo + horPadding, altoRenglon * 1.7, 80, true);
	...
	private StaticContainer fnCrearEncabezado(System.Drawing.Color sColor, ref DataSet ds)
	{
        ...
        fnCrearPanelRedondeado(Encabezado, 0, 0, Encabezado.rWidth, Encabezado.rHeight - fPropBlanca.rSize * 0 - 30, grosorPen, radioCurva, false, sColor);
        ...
        fnDatosReceptor(nsmComprobante, navEncabezado, Encabezado, Renglon, ref ds);
	...
	private void fnDibujarTitulosDetalle(StaticContainer Encabezado, System.Drawing.Color sColor)
	{
		//Dibujamos el área de los titulos
            double dvalor = 5;
            double altoBarra = fPropBlanca.rSize * dvalor;
            fnCrearPanelRedondeado(Encabezado, 0, Encabezado.rHeight - altoBarra, Encabezado.rWidth, altoBarra,grosorPen, radioCurva,false, sColor);
            double dpfAncho = 330;
            fnCrearPanelRedondeado(Encabezado, 0, Encabezado.rHeight - altoBarra, Encabezado.rWidth - dpfAncho , altoBarra, grosorPen, radioCurva, false, sColor);
            double dpfPosX = 242;
            dpfAncho = 445;
            fnCrearPanelRedondeado(Encabezado, dpfPosX , Encabezado.rHeight - altoBarra, Encabezado.rWidth - dpfAncho, altoBarra, grosorPen, radioCurva, false, sColor);
            dpfPosX = 369.5;
            fnCrearPanelRedondeado(Encabezado, dpfPosX, Encabezado.rHeight - altoBarra, Encabezado.rWidth - dpfAncho, altoBarra, grosorPen, radioCurva, false, sColor);
            dpfAncho += 52;
            fnCrearPanelRedondeado(Encabezado, dpfAncho, Encabezado.rHeight - altoBarra, Encabezado.rWidth - dpfAncho, altoBarra, grosorPen, radioCurva, false, sColor);
            ...
            double puntoMedio = Encabezado.rHeight - fPropNegrita.rSize;

            double dvariable = 0.25;
            //Definimos la posicion de los titulos
            double posColumna1 = Encabezado.rWidth * 0.01;      //Concepto
            double posColumna2 = Encabezado.rWidth * 0.43;      //Debe (Cuenta Corriente)
            double posColumna3 = Encabezado.rWidth * 0.15;      //Haber(Cuenta Corriente)
            double posColumna4 = Encabezado.rWidth * 0.515;     //Debe (Cuenta Reserva)
            double posColumna5 = Encabezado.rWidth * 0.285;     //Haber(Cuenta Reserva)
            //double posColumna6 = Encabezado.rWidth * 0.92;     //importe
            
            Encabezado.AddAligned(posColumna1, RepObj.rAlignLeft, puntoMedio -10, RepObj.rAlignCenter, new RepString(fPropNegrita, "CONCEPTO"));
            Encabezado.AddAligned(posColumna1 + posColumna2, RepObj.rAlignLeft, puntoMedio, RepObj.rAlignCenter, new RepString(fPropNegrita, "DEBE"));//CUENTA CORRIENTE
            Encabezado.AddAligned(posColumna2 + posColumna3, RepObj.rAlignLeft, puntoMedio, RepObj.rAlignCenter, new RepString(fPropNegrita, "HABER"));
            Encabezado.AddAligned(posColumna3 + posColumna4, RepObj.rAlignLeft, puntoMedio, RepObj.rAlignCenter, new RepString(fPropNegrita, "DEBE"));//CUENTA RESERVA
            Encabezado.AddAligned(posColumna4 + posColumna5, RepObj.rAlignLeft, puntoMedio, RepObj.rAlignCenter, new RepString(fPropNegrita, "HABER"));
            dpfPosX = 50;
            double dpfPosY = 15;
            Encabezado.AddAligned(posColumna1 + posColumna2 + dpfPosX, RepObj.rAlignCenter, puntoMedio - dpfPosY, RepObj.rAlignCenter, new RepString(fPropNegrita, "CUENTA CORRIENTE"));//CUENTA CORRIENTE
            Encabezado.AddAligned(posColumna3 + posColumna4 + dpfPosX, RepObj.rAlignCenter, puntoMedio - dpfPosY, RepObj.rAlignCenter, new RepString(fPropNegrita, "CUENTA RESERVA"));//CUENTA RESERVA
    ...
    private void fnDatosEmisor(XmlNamespaceManager nsmComprobante, XPathNavigator navEncabezado, StaticContainer Encabezado, System.Drawing.Color sColor, ref double Renglon)
	{
        string razonSocial, rfc, calle, noExterior, noInterior, colonia, municipio, estado, pais, codigoPostal, serie, folio, fecha, noCertificadoEmisor,
            ...
            exColonia, exMunicipio, exEstado, exPais, exCodigoPostal, exLocalidad, formaDePago;
        razonSocial = rfc = calle = noExterior = noInterior = colonia = municipio = estado = pais = codigoPostal = fecha = serie = folio = noCertificadoEmisor = 
            ...
            exCalle = exNoInterior = exNoExterior = exColonia = exMunicipio = exEstado = exPais = exCodigoPostal = exLocalidad = formaDePago = string.Empty;
        ...
        try { formaDePago = navEncabezado.SelectSingleNode("/cfdi:Comprobante/@formaDePago", nsmComprobante).Value; }
        catch { }
        ...
        //formaDePago = navEncabezado.SelectSingleNode("/cfdi:Comprobante/@formaDePago", nsmComprobante).Value;
        ...
        double fAltoPanel = Encabezado.rHeight / 18;
        double fAnchoPanel = Encabezado.rWidth / 5;
        double posX = Encabezado.rWidth - fAnchoPanel;
        double dPosXpnl = 280;
        double dAnchopnl = 166;
        //Agregamos los paneles visuales superior derecho
        fnCrearPanelRedondeado(Encabezado, Encabezado.rWidth - dPosXpnl,0, fAnchoPanel + dAnchopnl, fAltoPanel, grosorPen, radioCurva, false, sColor);
        fnCrearPanelRedondeado(Encabezado, Encabezado.rWidth - dPosXpnl, fAltoPanel, fAnchoPanel + dAnchopnl, fAltoPanel, grosorPen, radioCurva, false, sColor);
        fnCrearPanelRedondeado(Encabezado, Encabezado.rWidth - dPosXpnl, fAltoPanel + fAltoPanel, fAnchoPanel + dAnchopnl, fAltoPanel, grosorPen, radioCurva, false, sColor);
        fnCrearPanelRedondeado(Encabezado, Encabezado.rWidth - dPosXpnl, fAltoPanel + fAltoPanel + fAltoPanel, fAnchoPanel + dAnchopnl, fAltoPanel, grosorPen, radioCurva, false, sColor);
        fnCrearPanelRedondeado(Encabezado, Encabezado.rWidth - dPosXpnl, fAltoPanel + fAltoPanel + fAltoPanel + fAltoPanel, fAnchoPanel + dAnchopnl, fAltoPanel, grosorPen, radioCurva, false, sColor);
        fnCrearPanelRedondeado(Encabezado, Encabezado.rWidth - dPosXpnl, fAltoPanel + fAltoPanel + fAltoPanel + fAltoPanel + fAltoPanel, fAnchoPanel + dAnchopnl, fAltoPanel, grosorPen, radioCurva, false, sColor);

        //double dvalor = -130;
        dPosXpnl += -5;
        fnAgregarMultilinea(Encabezado, "FORMA DE PAGO ", fPropNegrita, Encabezado.rWidth - dPosXpnl, fAltoPanel / 1.5, 15, true);
        dPosXpnl += -125;
        //"PAGO EN UNA SOLA EXHIBICION "
        fnAgregarMultilinea(Encabezado, formaDePago.ToUpper(), fPropNormal, Encabezado.rWidth - dPosXpnl, fAltoPanel / 1.5, 40, true);

        #region Cellos, tipo ducumento, folio
        ...
        if (!string.IsNullOrEmpty(sUUID))
        {
            ...
            Encabezado.AddAligned(posX - dPosX, RepObj.rAlignLeft,
                fAltoPanel + fAltoPanel / dPosY, RepObj.rAlignTop,
        ...
        {
            Encabezado.AddAligned(posX - dPosX, RepObj.rAlignLeft,
                fAltoPanel * 2 + 4.8, RepObj.rAlignTop,
                new RepString(fPropNegrita, "NO. DE CERTIFICACION DIGITAL "));
            dPosX = -130;
            Encabezado.AddAligned(posX - dPosX, RepObj.rAlignLeft,
                fAltoPanel * 2 + 4.8, RepObj.rAlignTop,
        ...
        {
            Encabezado.AddAligned(posX-dPosX, RepObj.rAlignLeft,
                fAltoPanel * 2 + 21, RepObj.rAlignTop,
                new RepString(fPropNegrita, "FECHA/HORA CERTIFICACION "));
            dPosX = -130;
            Encabezado.AddAligned(posX - dPosX, RepObj.rAlignLeft,
                fAltoPanel * 2 + 21, RepObj.rAlignTop,
        ...
        {
            Encabezado.AddAligned(posX - dPosX, RepObj.rAlignLeft,
                fAltoPanel * 2 + 37.5, RepObj.rAlignTop,
                new RepString(fPropNegrita, "NO. SERIE CERTIFICADO SAT "));
            dPosX = -130;
            Encabezado.AddAligned(posX - dPosX, RepObj.rAlignLeft,
                fAltoPanel * 2 + 37.5, RepObj.rAlignTop,
                new RepString(fPropNormal, noCertificadoSAT));
        ...
        {
            Encabezado.AddAligned(posX - dPosX, RepObj.rAlignLeft,
                fAltoPanel * 2 + 53, RepObj.rAlignTop,
                new RepString(fPropNegrita, "FECHA DE EMISION "));
            dPosX = -130;
            Encabezado.AddAligned(posX - dPosX, RepObj.rAlignLeft,
                fAltoPanel * 2 + 53, RepObj.rAlignTop,
    ...
    private void fnDatosReceptor(XmlNamespaceManager nsmComprobante, XPathNavigator navEncabezado, StaticContainer Encabezado, double Renglon, ref DataSet ds)
	{
        string razonSocial, rfc, calle, noExterior, noInterior, colonia, municipio, estado, pais, codigoPostal, serie, folio, Localidad, exCalle, exNoInterior, exNoExterior,
            exColonia, exMunicipio, exEstado, exPais, exCodigoPostal, exLocalidad; 
        razonSocial = rfc = calle = noExterior = noInterior = colonia = municipio = estado = pais = codigoPostal = serie = folio = Localidad = exCalle = exNoInterior = exNoExterior =
            exColonia = exMunicipio = exEstado = exPais = exCodigoPostal = exLocalidad = string.Empty;
        ...
        //**********************************************************************************************************************
        //**************************************Adenda**************************************************************************
        string sNoContrato, sRamo, sNomCto;
        sNoContrato = sRamo = sNomCto = string.Empty;
        try
        {
            ...
            table.Columns.Add("CRhaber", typeof(double));
            table.Columns.Add("CRdebe", typeof(double));
            table.Columns.Add("CChaber", typeof(double));
            table.Columns.Add("CCdebe", typeof(double));
            ...
            columnTot.ColumnName = "id_Totales";
            columnTot.AutoIncrement = true;
            columnTot.AutoIncrementSeed = 1;
            columnTot.AutoIncrementStep = 1;

            dtTotales.Columns.Add(columnTot);
            keysTot[0] = columnTot;

            dtTotales.PrimaryKey = keysTot;

            dtTotales.Columns.Add("CRhaberSaldo", typeof(double));
            dtTotales.Columns.Add("CRdebeSaldo", typeof(double));
            dtTotales.Columns.Add("CChaberSaldo", typeof(double));
            dtTotales.Columns.Add("CCdebeSaldo", typeof(double));

            //leemos el nodo adenda
            XmlNodeList adenda = gxComprobante.GetElementsByTagName("cfdi:Addenda");
            XmlNodeList listaDatos = ((XmlElement)adenda[0]).GetElementsByTagName("Datos");

            //Se recorre nodos incidencia
            foreach (XmlElement nodo in listaDatos)
            {
                sNoContrato = nodo.GetAttribute("contratoNo");
                sRamo = nodo.GetAttribute("ramo");
                sNomCto = nodo.GetAttribute("nombreContrato");

                DataRow dr = dtTotales.NewRow();
                dr["CRhaberSaldo"] = nodo.GetAttribute("CRhaberSaldo");
                dr["CRdebeSaldo"] = nodo.GetAttribute("CRdebeSaldo");
                dr["CChaberSaldo"] = nodo.GetAttribute("CChaberSaldo");
                dr["CCdebeSaldo"] = nodo.GetAttribute("CCdebeSaldo");
                dtTotales.Rows.Add(dr);
            }

            //Llena conceptos
            XmlNodeList CtoUno = gxComprobante.GetElementsByTagName("Conceptos");
            ...
            foreach (XmlElement nodo in listaCpto1)
            {
                ...
                dr["concepto"] = nodo.GetAttribute("concepto");
                dr["CRhaber"] = nodo.GetAttribute("CRhaber");
                dr["CRdebe"] = nodo.GetAttribute("CRdebe");
                dr["CChaber"] = nodo.GetAttribute("CChaber");
                dr["CCdebe"] = nodo.GetAttribute("CCdebe");
                table.Rows.Add(dr);
            }
            ds.Tables.Add(table);
        ...
        if (!string.IsNullOrEmpty(exColonia))
            exDireccion += " Colonia " + exColonia;
        if (!string.IsNullOrEmpty(exCodigoPostal))
            exDireccion += " C.P. " + exCodigoPostal;
        ...
        double leftPadding = Encabezado.rWidth * 0.013;
        double sep = 47;//5;//10
        ...
        //lugar en X = 7 ;
        Renglon = 0.37;
        Encabezado.Add(leftPadding, posRazon + tamRenglon, new RepString(fPropNegrita, "LUGAR DE EXPEDICIÓN "));
        //AV. PATRIOTISMO NO. 266   COL. SAN PEDRO DE LOS PINOS C.P 
        fnAgregarMultilinea(Encabezado, exDireccion, fPropNormal, leftPadding, posRenglon + tamRenglon * Renglon, 75, true);
        //salto de linea
        Renglon += 0.19;
        fnAgregarMultilinea(Encabezado, "DELEG. BENITO JUAREZ ", fPropNormal, leftPadding, posRenglon + tamRenglon * Renglon, 75, true);
        //salto de linea
        Renglon += 0.19;
        //MEXICO , D.F. 
        fnAgregarMultilinea(Encabezado, exUbicacion2, fPropNormal, leftPadding, posRenglon + tamRenglon * Renglon, 75, true);

        //salto de linea
        double dRenglon = 0.28;//0.38
        double dPosTituX = 90;
        double dPosDesX = 60;
        Renglon += 0.38;
        fnAgregarMultilinea(Encabezado, "RECEPTOR ", fPropNegrita, leftPadding, posRenglon + tamRenglon * Renglon, 75, true);
        //salto de linea
        Renglon += dRenglon;
        fnAgregarMultilinea(Encabezado, "REASEGURADOR ", fPropNegrita, leftPadding, posRenglon + tamRenglon * Renglon, 75, true);
        //Mueve posion en X
        dPosTituX += dPosTituX;
        //MUENCHENER RUCKVERSICHERUNGS GESELLSCHAFT
        fnAgregarMultilinea(Encabezado, razonSocial, fPropNormal, leftPadding + dPosTituX, posRenglon + tamRenglon * Renglon, 75, true);
        //salto de linea
        dRenglon = 0.18;
        Renglon += dRenglon;
        fnAgregarMultilinea(Encabezado, "DIRECCIÓN ", fPropNegrita, leftPadding, posRenglon + tamRenglon * Renglon, 75, true);
        //Mueve posion en X
        dPosDesX += dPosDesX;
        //JOSE MA. VELASCO NO. 30 COL. SAN JOSE INSURGENTES
        fnAgregarMultilinea(Encabezado, direccion, fPropNormal, leftPadding + dPosDesX, posRenglon + tamRenglon * Renglon, 75, true);
        //salto de linea
        Renglon += dRenglon;
        //DELEG. BENITO JUAREZ , MEXICO, D.F.
        fnAgregarMultilinea(Encabezado, ubicacion, fPropNormal, leftPadding + dPosDesX, posRenglon + tamRenglon * Renglon, 75, true);
        //salto de linea
        Renglon += dRenglon;
        fnAgregarMultilinea(Encabezado, "R.F.C. ", fPropNegrita, leftPadding, posRenglon + tamRenglon * Renglon, 75, true);
        //xxx999999xxx
        fnAgregarMultilinea(Encabezado, rfc, fPropNormal, leftPadding + dPosDesX, posRenglon + tamRenglon * Renglon, 75, true);
        //salto de linea
        dRenglon = 0.6;
        Renglon += dRenglon;
        fnAgregarMultilinea(Encabezado, "CONTRATO No. ", fPropNegrita, leftPadding, posRenglon + tamRenglon * Renglon, 75, true);
        //2320
        fnAgregarMultilinea(Encabezado, sNoContrato, fPropNormal, leftPadding + dPosDesX, posRenglon + tamRenglon * Renglon, 75, true);

        fnAgregarMultilinea(Encabezado, "RAMO: ", fPropNegrita, leftPadding + dPosTituX, posRenglon + tamRenglon * Renglon, 75, true);
        //0050 INCENDIO
        fnAgregarMultilinea(Encabezado, sRamo, fPropNormal, leftPadding + dPosTituX + 80, posRenglon + tamRenglon * Renglon, 75, true);
        //salto de linea
        dRenglon = 0.18;
        Renglon += dRenglon;
        fnAgregarMultilinea(Encabezado, "NOMBRE DEL CONTRATO: ", fPropNegrita, leftPadding, posRenglon + tamRenglon * Renglon, 75, true);
        //3937 1ER EXCEDENTE -11 INCENDIO REASEGURO CEDIDO
        fnAgregarMultilinea(Encabezado, sNomCto, fPropNormal, leftPadding + dPosDesX, posRenglon + tamRenglon * Renglon, 75, true);
    ...
    private void fnAgregarBordeRedondeado(StaticContainer poObjeto, double pfGrosor, double pfRadioCurva, System.Drawing.Color sColor)
    {
        ...
        //linea divisora paneles superiores derechos
        poObjeto.Add(poObjeto.rWidth - 158, pfRadioCurva - 192.5, new RepLine(pen, 0, pfRadioCurva * 23.8 ));// - poObjeto.rHeight


        //Lineas Inferiores
        double posY = -175;
        double dSaltoRenglon = 10;
        //Linea datos lugar de expedicion
        poObjeto.Add(pfRadioCurva - 4.5, posY , new RepLine(pen, poObjeto.rWidth - lArc + 7.8, 0));
        //Salto de renglon
        dSaltoRenglon += dSaltoRenglon;
        dSaltoRenglon += dSaltoRenglon;
        poObjeto.Add(pfRadioCurva - 4.5, posY + dSaltoRenglon, new RepLine(pen, poObjeto.rWidth - lArc + 7.8, 0));
        //Salto de renglon
        dSaltoRenglon += 15;
        poObjeto.Add(pfRadioCurva - 4.5, posY + dSaltoRenglon, new RepLine(pen, poObjeto.rWidth - lArc + 7.8, 0));
        //Salto de renglon
        ...
        //Linea Cuentas corriente y Reserva
        double dPosX = 238;//237
        double dAnch = 246;//246.5
        poObjeto.Add(pfRadioCurva + dPosX, lArc - 24, new RepLine(pen, lArc + dAnch , 0));
	...
	/// <returns></returns>
    private RepImage GenerarCodigoBidimensional(ref byte[] imgCodigo)
	{
    ...
    /// </summary>
    /// <returns></returns>
    private MemoryStream fnImagenCliente(string sNombreLogo)//int id_contribuyente, int s_Id_Rfc)
    {
        MemoryStream ms = new MemoryStream();
