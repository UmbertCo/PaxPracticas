...
public class clsOperacionConsultaPdf
{
    ...
    private void fnTotales(XmlNamespaceManager nsmComprobante, XPathNavigator navPie, StaticContainer Pie, int nNumPag, int nTotPag, string sColor)
	{
        ...
        subtotal = total = moneda = sello = timbre = formaDePago = Regimenfiscal = metodoPago = version = string.Empty;
        ...
        if (nNumPag == nTotPag)//Si es ultima pagina
        {
            ...
            fnAgregarMultilinea(Pie, fnTextoImporte(Transformacion.fnFormatoRedondeo(total), moneda), fPropChica, tamCodigo + horPadding, altoRenglon * 2, 80, true);
	...
	private List<RepString> fnDividirRenglones(string psTexto, int pnTamRenglon, FontProp pFuente, bool bBuscarEspacio)
	{
        ...
        int nSeguro = 0;
        //int nLongTot = 0;
        //if (!(psTexto.IndexOf("\r") == -1))
        //{
        //    while (!(nUltimoEspacio == -1))
        //    {
        //        nLongTot = nlongitudTexto;
                
        //        nUltimoEspacio = psTexto.Substring(nCursorCadena, nlongitudTexto).IndexOf("\r");

        //        if (!(nUltimoEspacio == -1))
        //        {
        //             renglones.Add(new RepString(pFuente, psTexto.Substring(nCursorCadena, nUltimoEspacio).TrimStart()));
        //             nCursorCadena += nUltimoEspacio + 2;
        //             nlongitudTexto = nLongTot - (nUltimoEspacio + 2);
        //        }
        //        else
        //        {
        //            renglones.Add(new RepString(pFuente, psTexto.Substring(nCursorCadena, nlongitudTexto).TrimStart()));
        //        }       
