...
public class clsPlantillaFoxconn
{
    ...
    private StaticContainer fnCrearDetalle(List<DetalleFoxconn> detalles, System.Drawing.Color sColor)
    {
        ...
        for (int i = 0; renglonActual <= maxRenglones; i++)
        {
            ...
            else
            {
                ...
                Col3 = fnAgregarMultilineaDetalle(areaDetalle, d.descripcion + " " + fnObtenerConceptoAddenda(i), fPropNormal, posColumna3, posRenglon, 42, true);
            }
    ...
    private void fnTotales(XmlNamespaceManager nsmComprobante, XPathNavigator navPie, StaticContainer Pie, int nNumPag, int nTotPag, string sColor)
    {
        ...
        if (nNumPag == nTotPag)//Si es ultima pagina
        {
            ...
            fnAgregarMultilinea(Pie, fnTextoImporte(TransformacionFoxconn.fnFormatoRedondeo(total), moneda), fPropChica, horPadding, altoRenglon * 1.7, 80, true);

            fnAgregarMultilinea(Pie, "Forma de Pago:", fPropNegrita, horPadding, altoRenglon * 3.2, 80, true);
            ...
            if (version == "3.2") //Si es version 3.2 muestra el siguiente contenido
            {
                ...
                {
                    fnAgregarMultilinea(Pie, "Método de Pago:", fPropNegrita, horPadding, altoRenglon * 5, 80, true);
                    fnAgregarMultilinea(Pie, metodoPago, fPropChica, horPadding, altoRenglon * 6, 80, true);

                    //Número de cuenta
                    if (!string.IsNullOrEmpty(sNumCtaPago))
                    {
                        fnAgregarMultilinea(Pie, "Número de Cuenta:", fPropNegrita, horPadding, altoRenglon * 7, 80, true);
                        fnAgregarMultilinea(Pie, sNumCtaPago, fPropChica, horPadding, altoRenglon * 8, 80, true);
                    }
                }
                else
                {
                    ...
                    if (!string.IsNullOrEmpty(sNumCtaPago))
                    {
                        fnAgregarMultilinea(Pie, "Número de Cuenta:", fPropNegrita, horPadding, altoRenglon * 5, 80, true);
                        fnAgregarMultilinea(Pie, sNumCtaPago, fPropChica, horPadding, altoRenglon * 6, 80, true);
                    }
                }
            }
        }
        ...
        fnCrearPanelRedondeado(Pie, 0, tamCodigo, Pie.rWidth, Pie.rHeight - (tamCodigo * 2) - altoRenglon, grosorPen, radioCurva, false, ColorT);
    ...
    private RepImage fnMarcaAguaCorpus()
    {
        ...
        {
            clsLog.Escribir(Settings.Default.LogError + "LogError", ex.Message);
        }
