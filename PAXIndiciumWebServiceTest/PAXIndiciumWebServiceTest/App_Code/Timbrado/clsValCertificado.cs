...
public class clsValCertificado
{
    ...
    /// <returns></returns>
    public bool fnVerificarSelloPAC(string psCadenaOriginal, string psSello, string noCertificado, byte[] bCertificado)
    {
        try
        {
            ...
            X509Certificate2 cert = new X509Certificate2(bCertificado);
    ...
    {
        if (DateTime.Now.AddMinutes(65).CompareTo(pdFechaComprobante) < 0 || DateTime.Now.AddDays(-3).CompareTo(pdFechaComprobante) > 0)
            return false;
